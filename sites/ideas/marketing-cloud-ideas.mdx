...
`DeliveryTime` is [`_Job` Data View](../../../docs/config/sfmc-system-data-views/#_job) field that stores timestamp for the email delivery (when it successfully reached the target Email Service Provider). It is currently impossible to get delivery time for emails sent via Journey Builder.

All other engagement data points are available (`EventDate` field for [_Sent](../../../docs/config/sfmc-system-data-views/#_sent), [_Open](../../../docs/config/sfmc-system-data-views/#_open), [_Click](../../../docs/config/sfmc-system-data-views/#_click), [_Bounce](../../../docs/config/sfmc-system-data-views/#_bounce)), but `DeliveryTime` from [`_Job` Data View](../../../docs/config/sfmc-system-data-views/#_job) returns `NULL` for Journey Builder emails.
