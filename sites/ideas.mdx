...
`DeliveryTime` is [`_Job` Data View](../docs/config/sfmc-config-system-data-views#_job) field that stores timestamp for the email delivery (the moment when it successfully reached the target Email Service Provider). It is currently impossible to get delivery time for emails sent via Journey Builder.
...
2. Create a Custom Behavioral Trigger Content Block - now we will be able to leverage direct links from `IGO_PRODUCTATTRIBS` add tracking to them in the product's HTML template. Okay, but not easy and make you loose click data for Einstein Recommendations.
