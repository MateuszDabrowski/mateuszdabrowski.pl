...
Salesforce Marketing Cloud offers many different Waione basic is stillis still one basic missing - Wait Until Time Range.
...
2. Wait by Duration is closest to the solution, as it allows you to delay sending to a specific hour (for example, extend the wait du. Still, unfortunately, it means that if someone enters Journey at 10 AM, they will have to wait a whole day. We want to send emails normally in a given time range.
3. Update Contact adding Timestamp to Data Extension, and Decision Split deciding what should happen (send or Wait by DurmanyNice, but a lot of steps, configuration, performance penalty and awful Journey readability for longer Journeys.
