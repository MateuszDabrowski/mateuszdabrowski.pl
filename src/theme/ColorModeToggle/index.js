/*
 * Ejected from @docusaurus/theme-classic 3.10.1 for one change: the native
 * title tooltip is replaced by data-tooltip, which src/css/custom.css shows
 * as the same styled tooltip as the LinkedIn, Trailblazer and GitHub icons.
 * With both, the browser would show a second, slower tooltip on hover.
 */
...
function getColorModeTooltip(colorMode, respectPrefersColorScheme) {
  const current = getColorModeLabel(colorMode);
  const next = getColorModeLabel(getNextColorMode(colorMode, respectPrefersColorScheme));
  return `${current.charAt(0).toUpperCase()}${current.slice(1)}. Click for ${next}.`;
}
  ...
  return (
    <div className={clsx(styles.toggle, className)}>
      <button
        ...
        disabled={!isBrowser}
        data-tooltip={getColorModeTooltip(value, respectPrefersColorScheme)}
