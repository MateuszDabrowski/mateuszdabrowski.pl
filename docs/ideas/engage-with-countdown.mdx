...
3. [ShareLeverage dynamic content for even better results](#take-advantage-of-dynamic-content)
...
There is much more fun in the JavaScript part. Worry not - you don't need to know the language at all. Let's have a look on the lines to understand what is happening on the general level and then focus on personalisation options.
...
In Salesforce Marketing Cloud you can use either [AMPScript personalization string](https://developer.salesforce.com/docs/atlas.en-us.noversion.mc-programmatic-content.meta/mc-programmatic-content/personalizationStringsAMPscript.htm) or SSJS personalization string:
...
Wondering what will happen with visitors that don't have this date value? With just personalization string such users will see the counter set to zero right from the start. Of course, we would rather not have it that way. Thankfully SSJS personalizations tring allows us to add default value very easily:

```js
const deadline = new Date('<ctrl:field name=DateTypeAttributeName default='2077-07-20T20:17:00+00:00' />').getTime();
```

You can also easily create dynamic content block that will be visible only when the date from personalization string is soon enough, by leveraging a simple `if/else` logic of SSJS.
