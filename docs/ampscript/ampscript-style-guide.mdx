...
Letter case is just one element of syntax convention that helps with readability. Another important ones are indentation and spacing. AMPScript requires neither, but both can help you write better code.

### Consistent Indentation

__Use indentation to highlight code relationships.__

Indentation in AMPScript is even more important than in SSJS. Without curly brackets, it is the last hope for readability. Indetation will help you quickly understand the relationships between the code lines.

It is straightforward to apply the correct indentation to simplicity of AMPScript. There are just two elements that require it. `IF` statements and `FOR` loops - add indentation level inside of those and get readable code.
...
Whenever you build more complex AMPScript code, I recommend using three types of comments: table of contents, section and documentation. For short scripts, the last type will be enough.
...
Before even starting to write AMPScript, describe the desired outcome and steps you plan to code to get there. It can be high level; it can be a draft. But it will help you focus on the best way to write your script and consider potential roadblocks or opportunities for optimisation.
...
6. Use `UPPERCASE` for loops, if blocks and variable decalarations [🔗](#ampscript-loops-ifs-and-variable-declarations)
