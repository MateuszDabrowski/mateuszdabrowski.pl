...
we can use a clean pseudo-ternary:

```js {2} title="Same logic changed to a pseudo-ternary"
for (var customer in customers) {
...
You can leverage similar logic to trigger functions or [debugging](./debugging-ssjs#debugging-variable):
