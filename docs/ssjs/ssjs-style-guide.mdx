...
__Use names that provide context.__

As [John F. Woods](https://groups.google.com/g/comp.lang.c++/c/rYCO5yn4lXw/m/oITtSkZOtoUJ?pli=1) said, `always code as if the guy who ends up maintaining your code will be a violent psychopath who knows where you live`. Meaningful and consistent names are great starting point to live long (coding) life with limited amount of frustration and regret when someone asks you about your year-old code to adapt it for a new project.

Applying rules listed below is really easy when you write the code and remember its logic, but can be a challenge after you reset your memory writing few other scripts. Make your SSJS better with meaningful variable and function names.
...
4. When the variable is an array, use plural name (`optIns`, `favouriteProductCategories`).
...
2. Use a verb (`get`, `post`, `update`, `send`) as a first part of the name to tell what the function is doing.
...
### JS Variables, Functions & Methods Leter Case
...
Readability counts. Having less lines is not worth mental gymnastics required to read a condensed code. It is also much easier to find unclosed brackets.
...
- Around operators.
- Before opening curly bracket and after closing curly brakcet.
- After commas.
...
With SSJS, you can use either single quotes (`'`) or double quotes (`"`). Don't follow random approach of Salesforce SSJS documentation - choose one and be consistent.
...
4. Use `camelCase` for all elements of JS syntax [🔗](#js-variables-functions--methods-case)
5. Use `PascalCase` for all elements of SSJS syntax [🔗](#ssjs-functions-case)
...
Looking for even more Marketing Cloud style? Check out my [SFMC SQL Style Guide](../sql/sfmc-sql-style-guide.mdx).