...
__Use names that suggest the purpose.__
...
1. Use descriptive function names. It is better to have a long explicit one (`getEmailAddress`) than a short mysterious abbreviation (`geteml`).
2. Always use a verb (`get`, `post`, `update`, `send`) as a first part of the name to tell what the function is doing.
