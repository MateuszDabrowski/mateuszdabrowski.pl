...
Some also use Email Studio for testing various scripts, but I recommend against it. It doesn't support SSJS Core Library. And even if you don't need it for your script, it is handy for debugging.
...
Once you debug your code using the above methods, it is still a good idea to program defensively and leverage the Try/Catch block even on the production environment. It will allow you to handle the errors for the customers (for example, redirect to an Error Landing Page) and to log the errors to a data extension (it will provide you with a history of problems along with a description of what happened). You can even build something more sophisticated like automatically create a bug ticket in JIRA.
...
<script runat=server>
    ...
    function handleError(error) {
        if (debugging) {
            ...
            Platform.Function.InsertData('Data_Extension_Name', ['id', 'errorSource', 'errorFunction', 'errorMessage', 'errorDescription'], [GUID(), 'Name of the place where the script runs', 'Name of the function within the try/catch', Stringify(error.message), Stringify(error.description)]);
        }
    }
