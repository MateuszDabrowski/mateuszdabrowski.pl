...
```html {10,12} title="GET request to invalid URL within Try/Catch block"
<script runat="server">
    Platform.Load('Core','1');
    var debugging = true;
    ...
    function handleError(error) {
        if (debugging) {
            ...
            Platform.Function.InsertData(
                ...
                [GUID(), 'Name of the place where the script runs', Stringify(error.message), Stringify(error.description)]
            );
        };
    };
