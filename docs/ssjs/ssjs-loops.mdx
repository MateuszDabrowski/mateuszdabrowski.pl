...
As [for...of](js-loops#for-of) loop is not available in SSJS, you will use this one in most scenarios.
...
In SSJS however, the [for...of](js-loops#for-of) loop is not available. `Object.keys()` or `Object.entries()` also does not exist. This makes the `for...in` the only solution for iterating over objects.
