...
On top of that, standard Roles are silently changing from release to release as [permissions](../../config/sfmc-permissions.mdx) come and go, so keeping track of them is crucial to have a clear picture of what your users can do.
...
Check out my [article on SFMC Permissions](../../config/sfmc-permissions.mdx).
...
As the [default System Roles permissions](../../config/sfmc-permissions.mdx) in Salesforce Marketing Cloud can be edited freely and [currently there is no easy way to revert those changes](../../../sites/ideas/marketing-cloud/#restore-default-permissions-for-sfmc-standard-system-roles), I created a backup repository for you with all out-of-the-box configurations.
