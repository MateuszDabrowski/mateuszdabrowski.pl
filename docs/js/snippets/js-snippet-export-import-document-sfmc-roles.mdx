...
On top of that, standard Roles are silently changing from release to release as [permissions](../config/sfmc-config-permissions.mdx) come and go, so keeping track of them is crucial to have a clear picture of what your users can do.
