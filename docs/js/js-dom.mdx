...
This approach allows you to implement custom validation or data processing while benefiting from the HTML5 form backbone. Perfect when you want to push your form submission to a [Code Resource](../usecase/sfmc-code-resource.mdx#good-form-backend-with-code-resource) asynchronously.
...
1. [Tailor with Data](./snippets/js-snippet-tailor-with-data.mdx) - leverage DOM manipulation and personalisation strings to create dynamic experiences for your customers.
2. [Export, Import & Document SFMC Roles](./snippets/js-snippet-export-import-document-sfmc-roles.mdx) - Backup, clone and move cross-account the Salesforce Marketing Cloud roles. In seconds, thanks to DOM.