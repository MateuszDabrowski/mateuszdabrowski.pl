...
3. Loopability: outcomes of `getElements` selection cannot be looped using [`forEach()`](/docs/js/js-loops.mdx#foreach).
...
The `.classList` property lets you directly list all classes assigned to the selected element (in the form of DOMTokenList). It is excellent when you want to [loop](/docs/js/js-loops.mdx) through to find a specific class or [manipulate the DOM](#manipulating-the-dom).

On the other hand, when you want to do a simple [check or condition](/docs/js/js-if-and-switch.mdx), `.className` is a great shorthand returning all classes as a string.
...
You can either [loop](/docs/js/js-loops.mdx) through those or pick a specific child with an index. Helpful here can be `.childElementCount`, which will show you the number of elements selected.
...
This approach allows you to implement custom validation or data processing while benefiting from the HTML5 form backbone. Perfect when you want to push your form submission to a [Code Resource](/docs/salesforce/marketing-cloud-engagement/config/code-resource.mdx#good-form-backend-with-code-resource) asynchronously.
...
1. [Tailor with Data](/docs/js/snippets/tailor-with-data.mdx) - leverage DOM manipulation and personalisation strings to create dynamic experiences for your customers.
2. [Export, Import & Document SFMC Roles](/docs/salesforce/marketing-cloud-engagement/config/export-import-document-sfmc-roles.mdx) - Backup, clone and move cross-account the :product[Salesforce Marketing Cloud]{of=mce as-written} roles. In seconds, thanks to DOM.