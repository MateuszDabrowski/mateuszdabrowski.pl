...
While it is already useful, those properties shine when you want to [manipulate your page](#changing-attributes). More on that later.
...
As with other properties mentioned previously, those true shine when [manipulating the DOM](#changing-attributes).
...
With mouseetner and mouseleave you can trigger events when user moves the pointer in or out of specific element. The caveat - there needs to be a pointer, so the solution is not working for touchscreen users.

Still, it can be used to capture which part of the website is getting hovered over with mouse (and for how long) or to create those annoying pop ups trying to guilt trip you into staying on the page:

```js
document.documentElement.addEventListener('mouseleave',
    event => alert('Sad Alert')
)
```

Those are just few key events that you can capture with `.addEventListener`. There is much more available, like keydown & keyup, select, scroll, paste, error. Check out the full list on [MDN](https://developer.mozilla.org/en-US/docs/Web/Events#event_listing).

### Changing attributes

Events are awesome, but there is something even better. Attribute manipulation. With it you can change your website in real time according to your logic. How? By going one step further with what you already learned above.
...
But probably the most useful feature is possibility to alter the properties:
...
You can do the same with [`.innerText` and `.innerHTML`](#innerText-and-innerHTML), [`.style.display`](#hidden-and-style) and more.

Finally, you can mix DOM manipulation methods and change attributes on specific [events](#event-types):
