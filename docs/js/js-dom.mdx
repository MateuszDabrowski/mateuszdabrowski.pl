...
const divsInForm = form.querySelectorAll('div'); // Returns only div tags within your form
```
...
document.querySelector('form').querySelectorAll('div');

/* ❌ Chain from a collection to a collection - will throw TypeError */
document.querySelectorAll('form').querySelectorAll('div');

/* ✅ Chain from a single element (thank you, index) to a collection - same outcome as previous code snippet */
document.querySelectorAll('form')[0].querySelectorAll('div');
```

:::

#### How to use getElements and querySelectors?
