...
We can also add a logging solution that will store session and log app use history. We will need a Data Extension (one can serve multiple Cloud Apps) and a bit more code around the above snippets.
...
By considering all the above, we get a robust and secure template for our Cloud Apps:
...
<script runat='server'>
    ...
    if (!state && !errorMessage) {
        state = GUID();
        Platform.Response.Redirect('https://' + clientBase + '.auth.marketingcloudapis.com/v2/authorize?response_type=code&client_id=' + clientID + '&redirect_uri=' + appURL + '&state=' + state)
    /* 2. Authentication with REST Token */
    } else if (state) {
        var code = Platform.Request.GetQueryStringParameter('code');
        var payload = {
            grant_type: 'authorization_code',
    ...
    /* If there is an error, redirect to Error Page with details */
    } else {
        handleError(errorMessage, errorDescription)
    }
</script>

<!-- The Cloud App micro SSO solution will secure anything you create below -->
```
...
%%[
    ...
    /* Capture State, Error and Token for Authorisation purposes */
...
1. __Logging Data Extensions__
    1. In Content Builder or Email Studio create a [Cloud App Logging Data Extension](#capture-your-cloud-app-usage).
    2. In Content Builder or Email Studio create an [Error Logging Data Extension](../ssjs/debugging-ssjs#logging-to-data-extension).
    3. In both cases, one is enough for multiple Cloud Apps. If stored in the Shared Data Extensions folder, both can serve multiple Business Units.
2. __Cloud Page__
...
5. __Secured Template__
    1. Copy the [template](#secured-cloud-app-template) in the prefered language to the Cloud Page from step 2.i.
