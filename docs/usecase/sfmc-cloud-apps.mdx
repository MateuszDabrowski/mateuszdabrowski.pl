...
UpsertData(@LogDE,1,'session',@State,'appName',@AppName,'token',@Token,'tokenExpire',DateAdd(Now(),18,'MI'))
```
...
By merging above parts, we get powerful secured template for our Cloud Apps.
...
Any reload done within the app (Reload/Redirect/Post via Form) should point back to Apps Cloud Page URL with `'s=' + state` appended to it as a query parameter (remember to use `?` if it will be first parameter or `&` in other case).

You can also leverage the session to lookup the token for API calls: `%%=Lookup(@LogDE,'token','session',@Session)=%%`
...
<!-- Authorization START -->
%%[
    VAR @AppName, @ClientID, @ClientSecret, @ClientBase, @MID, @AppURL, @ErrorURL, @RequestURL, @Payload, @APIStatus, @Response, @Token, @Session, @Code, @LogDE

    /* Add clientID, Client Secret, Client Base (part of API Base URL) and MID for all further calls*/
    SET @ClientID = 'CLIENT_ID'
    SET @ClientSecret = 'CLIENT_SECRET'
    SET @ClientBase = 'API_BASE_URI'
    SET @MID = 'BUSINESS_UNIT_MID'

    /* URLs and Scope (space separated) for the API */
    SET @AppName = 'CLOUD_APP_NAME'
    SET @AppURL = 'APP_CLOUD_PAGE_URL'
    SET @ErrorURL = 'ERROR_CLOUD_PAGE_URL'

    /* Data Extension for Authorization Logging */
    SET @LogDE = 'CloudAppLoggingDE'
    ...
    IF NOT Empty(@Session) THEN
        ...
        IF Lookup(@LogDE,'appName','session',@Session) != @AppName THEN
            Redirect(@AppURL)
        ENDIF
