...
```js title="JavaScript functions on the form Cloud Page"
/**
 * @function request
...
```js title="Example of pushing data to backend"
pushData({
    action: 'createLead',
    payload: formData,
    sourceName: '<ctrl:var name=sourceName />',
    subscriberKey: '<ctrl:var name=subscriberKey />'
}).then(response => {
...
```html title="Example processing structure on the Code Resource"
<script runat="server">
...
And then there is more! Code Resource is available not only for Cloud Pages - you can use exactly the same approach for any form hosted on your CMS or even within internal systems. As long as your front end can make an AJAX call, you can target your Code Resource and make use of Marketing Cloud power.
