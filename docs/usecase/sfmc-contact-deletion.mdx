...
You can however add this using Query Activity in Automation Studio. For email channel it's straightforward:
...
If you are using also other channels, you can also include their [System Data Views](../config/sfmc-config-system-data-views.mdx) to the query.
...
First, to remove unused Leads and Users you must first stop them from synchronizing via Marketing Cloud Connect. Otherwise, they will come back to SFMC at the next scheuled synchronization. The best way to do it is to create a boolean field on those objects (for example called `ShouldSyncToSFMC`) that would be `true` only for records you need in your system. Depending on your use case, it might be neither (so just set `false` as default value) or it might be complicated (calculated field will be your best friend). With such boolean you will be able to select it as a filter for your Marketing Cloud Connect object synchronization.
...
If you have Mobile Studio in yoru SFMC licence, you have a nice workaround:

1. Go to Contact Builder.
2. Click All Contacts in top menu.
3. Select Mobile List tab.
4. Click Create Mobile List button in top right.
5. Leave MobileConnect as the channel as select Filtered List as a type. Click Ok.
...
9. Click Save as Filter button in top right.
10. Provide sensible name (like "AllContactsList") and click Save.

:::note You Should Know

All Contacts view show many cool data fields, but unfortunately, most of them are no longer available after the export to the List.
...
It will create and populate the list with all your Contacts. Save its name - it will be useful for the Query.
...
You might wonder what is happening within the [`WHERE`](../sql/sfmc-sql-where.mdx) fitler above - this is magic that is possible thanks to Salesforce ID structure. The ID is not completly random, instead it has specific structure.
...
For full list, check [Daniel Ballinger's cheat sheet](http://www.fishofprey.com/2011/09/obscure-salesforce-object-key-prefixes.html).

:::

If you want to remove only those Leads / Users that are no longer synchronized via filtered Marketing Cloud Connect, you will need to add comparision similar to the Contacts use case.
...
Another group ripe for deletion are Contacts that you cannot contact. There are multiple subgroups that fill that definition:

1. Supressed contacts
2. Hard bounced contacts
3. Contacts that does not have any channel information (like email address or mobile phone)
4. Bad quality contacts (test records, fake emails, contacts that failed Email Studio List Detective)

Before deletion, it might be worth to save some of this data for the future (especially suppressed contacts for compliance or hard bounced contacts for future deliverability).
...
I know, it might be hard to delete opted-in contacts when obtaining new ones is a hard work, but after some time you need to finish this toxic relationship. Zombie contacts skew your analytics, put unncesary strain on your instance, degare your IP Reputation and create costs. All this without any value to your business. You will be far better without them.

Align with your busienss what should be the treshhold for marking a contact as a zombie. It will be different from company to company, but my starting point recommendation is a contact that (1) Was sent at least 6 emails (2) within at least 3 different months and (3) did not open or click any email.

1. Makes sure that you tried enough times to make your opinion based in solid facts
2. Makes sure that it's not matter of out-of-office period, but a constant issue
3. Makes sure that temporary problem is not leading to deletion of otherwise valuable contact

Before making the final decision, I recommend checking few versions of those rules against your database to see which variation gives you best results without beeing too lax.

----

## Contact Deletion Archive

Deleting contacts is super fun, but sometimes you might need to know who you have deleted. Or be able to debug recurring deletions to fix a leak in your data pipeline. Contact Deletion Archive is the way to go - it allows you to keep details of your deletions in a privacy-first way.

It is build with two additional elements on top of the standard Contact Deletion process described above.

### Contact Deletion Archive Data Extension

It will store the information about past deletions allowing you to check multiple things securily:

1. Subscriber Key of deleted contacts.
2. Hashed Email of the deleted contacts.
3. Reason for the deletion.
4. Date of first and last deletion (and therefore information whether the SubscriberKey was deleted more then once).
...
The Archive Data Extension will be filled with new Query Activity that moves the data from the [Contact Deletion Pending Data Extension](#contact-deletion-pending-data-extension).

What is important, it will leverage `HASHBYTES` function to hash the Email Address. It will on one hand protect privacy of the deleted contacts and make sure they won't be used for any send, on the other - it allows you to check list of deleted contacts against known email by hasing it the same way.
...
This new Query Activity should be put in the Contact Deletion Automation between the [Contact Deletion Query Activity](#which-contacts-should-be-deleted) that selects the contacts to-be-deleted and the [Contact Deletion Script Activity](#contact-deletion-script-activity) that does the deed.