...
The bad? It is suitable for relatively simple apps. Think about a single front-end page powered by an SSJS or AMPScript backend on a [Code Resource](./sfmc-code-resource.mdx). Of course, you can try to build something more complex, but you may quickly get into a place where the whole endeavour would have been easier with an outside-hosted application.
...
1. __Logging Data Extensions__
    1. In Content Builder or Email Studio create a [Cloud Page App Logging Data Extension](#capture-your-cloud-app-usage).
