...
SELECT LEFT(FirstName, 1)    AS FirstNameInitialLetter,
...
```sql {2} title="Dynamically pull part of the Institution Name value"
SELECT CASE
...
```sql {3} title="Perfect for preparing your data for personalization"
SELECT SubscriberKey,
       EmailAddress,
...
```sql {3} title="Same as above, but with the plus notation"
SELECT SubscriberKey,
       EmailAddress,
...
```sql title="Noone likes useless whitespace in their database"
SELECT LTRIM('   data from webform ')        AS CleanLeftSide      /* Output: 'data from webform ' */
