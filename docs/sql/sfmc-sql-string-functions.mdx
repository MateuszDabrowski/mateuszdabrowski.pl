...
All string functions in SQL use a 1-based index instead of 0-based indexing known from, for example, JavaScript. The first character starts at index 1. It is crucial for both providing the correct value to the parameters and correctly interpreting the results.
...
In the example above, I used the optional starting index, but you will not be using it in most cases. The `CHARINDEX` defaults to starting from the beginning so that we can write the above line shorter as `CHARINDEX("Manager", JobTitle)`.
...
1. With the plus notation, if at least one provided value is `NULL`, the whole outcome becomes `NULL`. `CONCAT` function treats such values as empty strings.
...
The output structure is the same as for the SSJS `Platform.Function.GUID()` or AMPScript `GUID()` functions - five alphanumerical strings joined by hyphens: `XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX`.
