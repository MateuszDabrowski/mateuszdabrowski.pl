...
SELECT GETUTCDATE()                  AS TrialStartDate,
...
However, if you are changing the date with the 31st day of the month by months and the outcome month is shorter, SQL will account for that and return the last day of a given month.
...
```sql {4} title="Find all Subscribers that joined within the last month"
...
A very similar date calculation is also available with `DATEADD()` AMPScript function. Depending on the use case, consider whether you want to manipulate the date in the Data Extension or only in the communication.
...
```sql {4} title="Find all Subscribers that joined in 2020"
SELECT SubscriberKey,
       EmailAddress
  FROM ENT._Subscribers
...
It outputs an integer, so be sure you are using a number when making comparisons.

:::note You Should Know

There are also three shortcuts for the most popular use cases: `YEAR()`, `MONTH()` and `DAY()`.

```sql {4} title="Find all Subscribers that joined in 2020 - shorter approach"
SELECT SubscriberKey,
       EmailAddress
  FROM ENT._Subscribers
...
You can reference date parts both by their full name, as well as an abbreviation.
...
```sql {2} title="Check how long someone was a subscriber"
SELECT SubscriberKey,
...
There is a limit to how big the difference can be. If you go down to the `SECOND` [date part](#date-parts), the query's maximum time difference is 68 years, 19 days, 3 hours, 14 minutes and 7 seconds. For most use cases, this shouldn't be an issue in Marketing Cloud use cases.

:::

## DATENAME

The `DATENAME` functions return the value of the specified [datepart](#date-parts) from a given date.

```sql {2} title="Capture the full day's name"
SELECT SubscriberKey,
       DATENAME(WEEKDAY, DateJoined) AS DayJoined
  FROM ENT._Subscribers
```

It might be useful to quickly cut the date to just an important part (like a year for anniversary personalisation or capturing anniversary bins). However, its standout feature is returning the name of the day for a given date, thanks to the `WEEKDAY` [datepart](#date-parts).
...
SELECT CONVERT(DATETIME2(0), '2020-01-01T18:00:00') AT TIME ZONE 'Central Standard Time' AS CST, /* Output: 2020-01-01 18:00:00 -06:00 */
...
SELECT TODATETIMEOFFSET(CONVERT(DATETIME2(0), '2020-01-01T18:00:00'), '-06:00') AS CST,     /* Output: 2020-01-01 18:00:00 -06:00 */
...
SELECT CONVERT(DATETIMEOFFSET(0), '2018-01-01 18:00:00 -06:00') AT TIME ZONE 'Central European Standard Time' AS CSTtoCET /* Output: 2020-01-01 01:00:00 +01:00 */
```
...
SELECT CONVERT(DATETIME2(0), '2020-01-01T18:00:00') AT TIME ZONE 'Central Standard Time' AT TIME ZONE 'Central European Standard Time' AS DateToCSTtoCET, /* Output: 2020-01-01 01:00:00 +01:00 */
```

It works because the function's first execution [adds the offset with `AT TIME ZONE`](#add-offset-with-at-time-zone). Second, uses the output with an offset applied and [change the timezone](#change-timezone-with-at-time-zone).
...
SELECT TODATETIMEOFFSET(GETDATE(), '-06:00') AT TIME ZONE 'Central European Standard Time' AS SystemDateToCSTtoCET,
```
...
SELECT FORMAT(CONVERT(DATE, '10/30/2020'), 'd', 'en-GB') AS DateOrderedProperly,
