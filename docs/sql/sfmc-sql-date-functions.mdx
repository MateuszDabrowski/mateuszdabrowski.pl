...
`DAYOFYEAR`, `DAY`, and `WEEKDAY` sounds similar to each other, but they return a bit different value. `DAYOFYEAR` will tell you which day of the year it is (f.e. `257`). `DAT` returns number of the day within the month (f.e. `14`). `WEEKDAY` returns number of the day within a week (f.e. `2` for Monday).
...
Not all available [timezones](#available-timezones) observe Daylight Saving Time. For example `Central Standard Time` observes DST, while `Central America Standard Time` does not. Always double-check behaviour of the timezone you are selecting for your query.
...
Of course, for Salesforce Marketing Cloud system dates, we still need the either of the [workarounds](#at-time-zone-vs-todatetimeoffset):
