...
You can see the 24-hour clock format and milliseconds and UTC-6 offset at the end. This last information will be helpful for timezone conversions with [`AT TIME ZONE`](#at-time-zone) function.
...
`DATEDIFF` can output both positive and negative numbers depending on provided values and their order.
...
```
 ...
 It is shorter than [`DATEADD`](#dateadd) but with a different level of control. Because `DATEDIFF` operates on [Date Parts](#date-parts), it will take into consideration only the chosen part. For example:
...
It might be good or bad depending on the use case, so be sure to select the correct function for your needs.
