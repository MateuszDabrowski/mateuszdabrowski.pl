...
You can get the datetime with appropriate offset information by using `SYSDATETIMEOFFSET()` function. It will return the same datetime as `GETDATE()` but in a format showcasing the timezone offset: `2020-12-28 15:08:01.7239173 -06:00`.

You can see the 24-hour clock format and milliseconds and UTC-6 offset at the end. This last information will be useful for timezone conversions with [`AT TIME ZONE`](#at-time-zone) function.
...
However, you can limit its output to either date or time part by using [`CONVERT`](./sfmc-sql-conversion-functions.mdx#case--convert) or [`FORMAT`](#format-with-dates) Functions.
...
```sql {4} title="Get everyone that hit one year on the subscription list"
SELECT
    SubscriberKey
FROM ENT._Subscribers
...
It takes into consideration the leap years. If you provide dates with [offset](#at-time-zone), it will even include the timezone differences and Daylight Saving Time.

:::note You Should Know

There is a limit of how big the difference can be. If you go down to `SECOND` [date part](#date-parts) the maximum time difference for the query is 68 years, 19 days, 3 hours, 14 minutes and 7 seconds. For most use cases, this shouldn't be an issue in Marketing Cloud use cases.

:::

## DATENAME

The `DATENAME` functions return value of specified [datepart](#date-parts) from a given date.

```sql {3} title="Capture full name of the day"
SELECT
    SubscriberKey,
...
It might be useful to quickly cut the date to just an important part (like a year for anniversary personalisation or capturing anniversary bins). However, its standout feature is returning the name of the day for a given date thanks to `WEEKDAY` [datepart](#date-parts).

```sql {4} title="Create an automation that will capture weekend meetings for Friday reminder"
SELECT
    SubscriberKey
FROM SalesMeetings
...
It's must-have for multi-country senders and beneficial for others planning to personalize or calculate using date data.
...
In the above snippet, we [`CONVERT`](./sfmc-sql-conversion-functions.mdx#convert) the dates to datetime without offset. Then the `AT TIME ZONE` function not only adds the offset - but it also calculates the Daylight Saving Time offset change for you.
...
Salesforce Marketing Cloud uses Central Standard Time (UTC-6) __without__ changes between standard and daylight savings time. It means that the excellent feature of `AT TIME ZONE` is working against us.

There is, however, another function that might solve this issue - `TODATETIMEOFFSET`. It is a simplified version of the `AT TIME ZONE` that adds arbitrary offset without any logic on Daylight Saving Time. Just as we would like for the SFMC system dates.

To use it, just provide a datetime and an offset (for example `'-06:00'` or `'+01:00'`):

```sql {2,4} title="No Daylight Saving Time magic"
SELECT
...
Of course, for any datetime values that include the Daylight Saving Time - pushed from website or your data warehouse - [`AT TIME ZONE`](#add-offset-with-at-time-zone) will be better solution.
...
```sql {2} title="Change CST datetime with offset to CET"
SELECT
...
```sql {2} title="First, apply timezone. Then change the timezone"
SELECT
    CONVERT(DATETIME2(0), '2020-01-01T18:00:00') AT TIME ZONE 'Central Standard Time' AT TIME ZONE 'Central European Standard Time' AS DateToCSTtoCET,
    -- Output: 2020-01-01 01:00:00 +01:00
```

It works because the first execution of the function [adds the offset with `AT TIME ZONE`](#add-offset-with-at-time-zone). Second uses the output with an offset applied and [change the timezone](#change-timezone-with-at-time-zone).

Of course, for Salesforce Marketing Cloud system dates, we still need the [workaround](#at-time-zone-vs-todatetimeoffset):

```sql {2} title="No Daylight Saving Time magic but still changing the timezone"
SELECT
    TODATETIMEOFFSET(GETDATE(), '-06:00') AT TIME ZONE 'Central European Standard Time' AS SystemDateToCSTtoCET,
```
