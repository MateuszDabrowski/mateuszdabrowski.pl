...
You can, however, limit its output to either only date or time part by using [`CONVERT`](sfmc-sql-conversion-functions#case--convert) or [`FORMAT`](#format-with-dates) Functions.
...
`DATEADD` is especially useful when used with the [`WHERE`](sfmc-sql-where) statement, as it allows to filter records dynamically:
...
The last line outputs `Freitag, 7. Februar 2020`. Neat for running it with [`CASE`](sfmc-sql-select#conditional-values-with-case) to change your short date into an extended date personalized on user language.
