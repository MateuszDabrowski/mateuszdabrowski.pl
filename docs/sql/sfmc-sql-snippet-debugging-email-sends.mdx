...
My favourite way to start debugging email sends is by using a SQL Query. Open your [Query Studio](../config/sfmc-config-appexchange-solutions.mdx#query-studio) and copy-paste the below code:
...
As they say, with great power comes great complexity. And that's definitely true for finding the root cause of the undelivered email. The deliverability by itself is already beautiful mess and then there are various features and options you can use directly in Salesforce Marketing Cloud. Let's dive in.

### 1. Query System Data Views

I like to start with the query described in this guide - filtered down to problematic subscriber - to get the high level picture of the situation. Free Salesforce [Query Studio](../config/sfmc-config-appexchange-solutions.mdx#query-studio) is the best place to execute that little exploratory analysis.

Once you see the outcome, there are few possibilities:

- __There is no data__ » It's time go to to the [next step](#2-check-send-log).
- __BounceDate is filled__ » Either the email is invalid (hence the bounce) or the email was blocked by the recipient's inbox Admin (they sometime use fake bounces to fight of spam - especially in enterprise B2B and public markets).
- __OpenDate or ClickDate is filled__ » It's time to compare their timestamps with DeliveryDate. If those are within a single second and there are no later subsequent opens/clicks, the email has been checked by antispam bot and flagged for no delivery to final recipient (again, B2B and Public). If the timestamp are spread out (especially engagement ones from the DeliveydDate), the email got delivered to the right person and they just forgot or lost it.
- __DeliveryDate is filled__ » If there is no Bounce, Open or Click timestamps, but Delivery is filled in, the email should be in subscriber's Spam folder or Promotions tab.
- __DeliveryDate is not filled__ » It's time go to to the [next step](#2-check-send-log).

### 2. Check Send Log

 [Send Log](../usecase/sfmc-enhanced-send-log.mdx)
