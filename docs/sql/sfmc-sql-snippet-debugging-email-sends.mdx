...
```sql
SELECT s.SubscriberKey     AS SubscriberKey,
  ...
  FROM _Sent s
       LEFT JOIN _Job job ON job.JobID = s.JobID
       ...
       LEFT JOIN _Unsubscribe u ON s.JobID = u.JobID
           ...
           AND u.IsUnique = 1
       LEFT JOIN _JourneyActivity ja ON ja.JourneyActivityObjectID = s.TriggererSendDefinitionObjectID
       LEFT JOIN _Journey j ON j.VersionID = ja.VersionID
       LEFT JOIN _Subscribers sub ON sub.SubscriberKey = s.SubscriberKey
