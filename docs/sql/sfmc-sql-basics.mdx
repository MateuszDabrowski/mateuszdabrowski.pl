...
However, there is much to learn to unlock full power of SQL in SFMC. Check out deep-dive docs on SQL Statements ([`SELECT`](sfmc-sql-select.mdx), [`FROM`](sfmc-sql-from.mdx))and [SFMC System Data Views](sfmc-sql-system-data-views.mdx).
