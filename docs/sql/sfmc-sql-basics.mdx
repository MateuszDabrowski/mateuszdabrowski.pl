...
This query can be written without filter on `sent.EventDate` but with `INNER JOIN _Sent` instead. Above version makes it easier for changing the query for the bonus below.
...
On this level you learn how to use the most important filter of all - date filter. You also get to know basic date functions and what to do when you don't have the right data type for your use case.
...
Write a query that will find Subscriber Key for all Subscribers in your All Subscribers list that joined the list within last year and had a bounce on Google or Microsoft email between the first day of current month (write that date manually, as a string) and today.

For the purpose of this exercise we will assume that Google users have emails `something@gmail.com`, Microsoft users - `something@outlook.com` and Yahoo - `something@yahoo.com`.
...
2. When writing date comparision filter, take a pen and paper and draw a timeline with area you want to capture. Seeing it makes it much easier to write it correctly.
3. When deciding on whether to use shorthands - go with the most readable option.
...
On this level you will be able to leverage data from multiple tables - the only limitations will be creativity and autokill function crashing queries after 30 minutes of execution ;)

#### Learning materials

- [JOINs](./sfmc-sql-join.mdx#multiple-joins) - yup, everything. The Cheat Sheet at the end should be useful to better visualize multi-JOIN fun.
...
Find some active Journey from the last quarter that already had some email sends with engagement. Note name of the Journey and Email Acitvity.
...
1. Think which tables are needed for the task. The less, the better.
2. Think what [types of JOINs](./sfmc-sql-join.mdx#join-types) will be needed. To make it easier - take a pen and paper and draw a diagram. Seriously, it's super helpful as amount of tables grow.
3. Think on what columns you can join the tables in a way that will protect you from the fan out (one to many relationship that results with more rows then needed in the output).
...
On this level you graduate from just playing with rows and start calculating the data to see patterns, outliers and problems. This is especially useful for SFMC Administrators and Consultants performing analysis on the platform data.
...
- [Order of operations](#order-of-operations) - this is the moment when it becomes tricky.
...
Find the number of sends, unique bounces and the bounce rate (percent of bounces vs sends) per domain from deliveries attempted in the last month. Filter the records to only those who have at least 100 sends. Order by the bounce rate descending and limit to 20 records.

If you have huge send volumes, feel free to change the timeframe from last month to last week for the purpose of the exercise.
...
9. When calculating Bounce Rate, do it so that the outcome is integer equal to a percent.

Important: Because of the Query Studio bug, you cannot have spaces around the multiplication symbol (`*`) as otherwise you will be seeing an error related to bad practice SELECTs.
...
Bonus: This query might be performance heavy. One option to make it perform better is to shorten the timeframe in WHERE fitler. Another would be to split this query into two - one getting cumulative counts and second responsible for doing math for Bounce Rate.
...
More levels will be added once I finsih writing supporting learning materials :)
