...
However, there is much to learn to unlock the full power of SQL in SFMC. Check out deep-dive docs on other SQL Statements ([`LIKE`](./sfmc-sql-like.mdx)), [String](./sfmc-sql-string-functions.mdx), [Date](./sfmc-sql-date-functions.mdx), [Number](./sfmc-sql-numeric-functions.mdx), [Conversion](./sfmc-sql-conversion-functions.mdx) Functions and [SFMC System Data Views](../config/sfmc-config-system-data-views.mdx).
