...
However, there is much to learn to unlock the full power of SQL in SFMC. Check out deep-dive docs on SQL Statements ([`SELECT`](sfmc-sql-select), [`FROM`](sfmc-sql-from), [`JOIN`](sfmc-sql-join), [`WHERE`](sfmc-sql-where)) and [SFMC System Data Views](sfmc-sql-system-data-views).
