...
* [`SELECT`](sfmc-sql-select): tell the query what data you want to leverage
* [`FROM`](sfmc-sql-from): tell the query where the data is in Marketing Cloud (name of Data Extension or System Data View)
* [`JOIN`](sfmc-sql-join): tell the query how you want to merge data from multiple sources
* [`WHERE`](sfmc-sql-where): tell the query which part of the above data is interesting to you

However, there is much to learn to unlock the full power of SQL in SFMC. Check out deep-dive docs on other SQL Statements ([`LIKE`](sfmc-sql-like)), [String](sfmc-sql-string-functions), [Date](sfmc-sql-date-functions), [Number](sfmc-sql-numeric-functions) Functions and [SFMC System Data Views](sfmc-sql-system-data-views).
