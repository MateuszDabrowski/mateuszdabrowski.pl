...
1. You can use Query Activities in Automation Studio. It will be much less user-friendly. For each change in the query, you will have to update the target Data Extension manually and go through multiple steps across the platform to see the results. But it will let you train using the data you have in your platform.
2. You can also use [MC Snippets SQL Playground](https://mcsnippets.herokuapp.com/sql) created by Salesforce MVP Pato Sapir. It allows you to learn Marketing Cloud SQL even if you don’t have access to the SFMC at all. It’s limited to the Data Views (on Pato’s instance) and a Data Extension you can create in the Playground itself, but as you can spin it quickly and have everything in one screen it’s perfect tool for quick experimentation or sharing tested transformations on sample data with colleagues. 

My approach is to always start in [Query Studio](../config/sfmc-appexchange-solutions.mdx#query-studio), and only once my SQL Query is complete and validated do I move it to Automation Studio.

Once you have the right tool, it’s time to get the right knowledge. There are few formats available. If you like text and small knowledge checks - this page is the right choice. But if you prefer video or a challenge - Cam Roberts and Sabuhi Yahya got you covered:
