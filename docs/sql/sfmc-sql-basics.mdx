...
The most important two SQL commands that you will be using in every single query are `SELECT` and `FROM`.

### SELECT

`SELECT` is starting each query and defines to columns of data extension that will be used within the query.

The simplect version of this statement is `SELECT *`, which targets all available columns from the source, but it is not recommended approach. It is awful for both readability and performance of your query.

The prefered method is targeting columns by their names:

```sql
SELECT emailaddress, [Favorite Colour]
```



* `FROM`: location where data reside within Marketing Cloud (usually a data extension)
* `JOIN`/`ON`: allows the query to search multiple tables and/or data extensions
