...
`CASE` statement is an awesome tool, but can be overkill for simple conditions. This is where `IIF` comes to the rescue.

It takes three arguments:

1. Condition
2. Value returned if the condition evaluates to true
3. Value returned if the condition evaluates to false
...
Both `CASE` and `IIF` can be taken to the next level with dynamic values. You don't have to use a string, integer or boolean. You can pass a column name.

Use case? Think of a Master Data Extension that stores huge amount of contact information, including preferred channel (Email or Mobile). With dynamic values and `IIF` in just few lines we can create optimised Data Extension with crucial information:
...
Now, even with just three columns in your Entry Point, you can create Journey that adapts the channel to customer preferences.

:::

## TOP

`TOP` clause can be added to your `SELECT` statement to limit the number of rows returned by your query.
...
`TOP` clause is not limited to an integer, you can also ask for specific percent of rows with `PERCENT` modifier:
...
`TOP` clause best practice is to always use it with `ORDER BY`. Only with sorted data you will know what is the logic behind selecting rows.

To sort outcomes of your query, add `ORDER BY` at the end along with column(s) that should be used for sorting.
...
`ORDER BY` sorts Text fields alphabetically. This might surprise you if you store some numbers in such field type, as it will order `10` before `2`.

The best solution is to use appropriate field type for numbers. However, if for whatever reason you cannot do it, leverage [`CONVERT`](./sfmc-sql-conversion-functions.mdx#convert):

```sql {4} title="Top 10% with highest Quiz Score if QuizScore is Text field"
SELECT TOP 10 PERCENT
      SubscriberKey
    , QuizScore
...
`ORDER BY` can operate not only on selected column. You already saw above that you can use expression (even [`CASE`](#conditional-values-with-case)!) to calculate the sorting value, but it's much more flexible.

You can sort using a column that was not selected:
...
SQL will sort using first provided column and then, while mantaining the order of the previous column, will sort again using the next value. This may sound complicated, so let's jump to an example:
...
Query will first sort using QuizScore to find people with the most points. Next, it will apply second sort on the date of quiz completion. As an outcome, we will have the highest scorers sorted from first to last within each high score.
...
What can we do to capture everyone with the score good enough for the Top 3 status? `WITH TIES` will help:
...
With those two words added after the `TOP` clause, our query will return all rows that match the sorted value in the last row. This, however, means that the number of outcomes is not set in stone, so be sure to prepare your next steps for that uncertainity.
...
`DISTINCT` clause is the simplect deduplication tool.

When added to the `SELECT` statement, it will check all available values and leave only unique ones.
...
In the above case you will get separate row for each Subscriber Key - Domain pair. It means that you might receive multiple rows with the same Subscriber Key (but each with different Domain). And you will certainly see multiple rows with the same domain (but each with different Subscriber key).
