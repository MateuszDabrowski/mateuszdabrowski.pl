...
```sql {3} title="Let's add another column with whitespace in its name"
SELECT ContactKey,
       EmailAddres,
       [External Contact Id]
```
...
```sql {3-4} title="Improve readability and consistency with labelling"
SELECT ContactKey,
       EmailAddress,
...
```sql {5-6} title="Add custom column labelled 'ImportDate' with a value equal to the UTC of the script execution for each record"
SELECT ContactKey,
       EmailAddress,
       [External Contact Id] AS ExternalContactId,
       Gender__c             AS Gender,
...
```sql {3} title="Let's create a concatenated column for easier personalization"
...
```sql {2-6} title="Let's calculate what channel we should use in our campaign for a given user"
SELECT ContactKey,
       CASE
...
```sql {2-15} title="Let's add another dimension of membership level to differentiate available channels"
SELECT ContactKey,
       CASE
...
```sql {3} title="Great way to split your database between Internal and External users for different communication"
SELECT ContactKey,
       CASE
...
```sql
SELECT _Subscriber.SubscriberKey
