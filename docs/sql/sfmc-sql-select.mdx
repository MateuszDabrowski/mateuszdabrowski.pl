...
The simplest version of this statement is `SELECT *`, which targets all available columns from the source. I do not recommend this approach, as it is terrible for both readability and performance of your query. As Salesforce Marketing Cloud caches the source Data Extension column names when you save SQL Query Activity, you won't have added value of flexible selection. You will still have to edit and save the activity. Also, [Query Studio](sfmc-sql-basics#query-studio) does not support this shortcut. Don't use `*`.
...
It becomes mighty with the [SQL String functions](sfmc-sql-string-functions).
...
`LIKE` is not only useful in the [`WHERE` Statement](sfmc-sql-where#like-operator). It is also an awesome tool empowering `CASE` statement. It allows you to build flexible conditions on free text values:
...
In the `SELECT` statement, the `LIKE` operator is especially useful for fixing data coming from the free text fields. To learn more about the possibilities, check out the [`LIKE` guide](sfmc-sql-like).
...
Fortunately, it does support (selected) functions. Learn more about them in SFMC SQL Functions documentation pages related to [Strings](sfmc-sql-string-functions), [Dates](sfmc-sql-date-functions), [Numbers](sfmc-sql-numeric-functions).