...
If you want to search for a hyphen '-' within a grouping operator that uses a range, just add it in the very beginning. `LIKE '[-0-9]` will look for either `-` or any digit.

:::note You Should Know

You can mix and match groups, ranges and wildcards.

It is great tool to create basic categorization solutions for uncontrolled data. For example, to clean up free text job title coming from web forms.
...
There is one more operator that is sometimes useful - the exclusion operator `^`. By adding it to the beginning of your group, you are telling the SQL to look for any character not like the ones within the group. For example, `LIKE '[^0-9]'` will look for any character that is not a digit.

This operator is especially useful for the use in `WHERE` statement. Best example? SQL for finding with Subscriber Key not being correct Salesforce ID:
...
This simple and short query will look into each SubscriberKey and check whether in there is any character other the letter or digit (case-insensitive) or whether the length is not equal to 18. All such records have incorret value and are eligible for cleanup or investigation. Perfect for Automation with deletion SSJS script or Verification Activity to alert administrator about issues.
