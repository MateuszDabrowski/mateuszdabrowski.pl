...
However, in Salesforce Marketing Cloud, it is much better to use `CONVERT` function instead. It covers all features of `CAST` plus adds quite a lot more. It is also better supported by the [Query Studio](./sfmc-sql-basics.mdx#query-studio) (for example, you cannot use `CAST` in a `SELECT` part of the query there).
