...
You can also leverage style codes to format your dates during conversion to string, without the need to use [`FORMAT`](sfmc-sql-date-functions#format-with-dates) function on top.
...
| BIGINT | Big integer number | When your numer is bigger than 2,147,483,647, you need BIGINT |
...
Just as with data types - there are many style codes available. Here I will cover just the most popular ones (if you don't understand some formats codes, check [date formats guide](sfmc-sql-date-functions/#custom-date-formats)):
