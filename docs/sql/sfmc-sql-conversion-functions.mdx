...
```sql
SELECT CONVERT(NVARCHAR, GETDATE(), 101) AS DateFormat1,   /* Output: 10/30/2020 */
...
```sql
SELECT CONVERT(DECIMAL, '123.99')      AS RoundedConversion /* Output: 124 */
...
SELECT CAST(6.9 AS INT)      AS CastToInt,       /* Output: 6 */
...
```sql
SELECT CONVERT(NVARCHAR, GETDATE(), 101)              AS DateAsString,
