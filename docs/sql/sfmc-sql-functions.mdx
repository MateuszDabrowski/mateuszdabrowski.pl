...
2. Format to be applied (more on that later)
...
First that comes to mind when dealing with US-centric SFMC is to change the date format. For personalization. For comparision between system and imported dates outside of the query. For preparing data that will be exported to external system.
...
In the example above, first two `FORMAT` functions return date in the same format - `30/10/2020`. First using manually provided one. Second by taking current UTC date. The last line outputs `Freitag, 7. Februar 2020`. Use case? Run it with [`CASE`](sfmc-sql-case) to change your basic date into language-personalized expanded date.
