...
With [`GROUP BY`](./sfmc-sql-aggregate-functions.mdx#group-by) and [`HAVING`](./sfmc-sql-aggregate-functions.mdx#having) we simplify the output to show only the duplicate values with corresponding counts.
