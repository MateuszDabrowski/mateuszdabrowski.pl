...
```sql {2,5} title="This query will show you which Email Addresses exists on more than one contact"
...
```sql {9-10} title="Returns Subscriber Key, Email Address and Duplicate Count of your database"
SELECT s.SubscriberKey,
       s.EmailAddress,
       counted.EmailCount
  FROM _Subscribers AS s
       INNER JOIN (
           SELECT s.EmailAddress        AS EmailAddress,
                  COUNT(s.EmailAddress) AS EmailCount
             FROM _Subscribers AS s
            GROUP BY s.EmailAddress
           HAVING COUNT(s.EmailAddress) > 1
       ) AS counted ON counted.EmailAddress = s.EmailAddress
```
...
```sql {5-6} title="Get a Subscriber Key and Points of all the customers that have the biggest amount of points"
SELECT comp.SubscriberKey,
       comp.CompetitionPoints
  FROM CompetitionDataExtension AS comp
 WHERE comp.CompetitionPoints = (
...
```sql {1-2} title="For Datetime values, it will output 19 characters"
SELECT MAX(LEN(c.Id))    AS SubscriberKey,
...
SELECT FORMAT(1234.56, 'N')  AS WithoutCustomPrecision,   /* Output: 1,234.56 */
...
```sql {2} title="From '48520935815' to '(48) 520 935 815' in one line"
SELECT s.SubscriberKey,
       FORMAT(CONVERT(BIGINT, ea.MobileNumber), '(###) ### ### ###') AS FormattedPhone,   /* Output: (48) 520 935 815 */
  FROM _Subscribers AS s
       JOIN EnterpriseAttribute AS ea ON ea._SubscriberID = s.SubscriberID
```
...
SELECT FORMAT(VIPDiscount, 'P')       AS StandardFormattedVIPDiscount,      /* Output: 33.00 % */
