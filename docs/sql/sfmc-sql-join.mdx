...
All you need to do is write your alias after a data source name. For the readability purposes, I recommend using the `AS` keyword between. Now you can leverage the shortened name to prefix the selected columns. It's instrumental when pulling data from multiple Data Extensions.
...
```sql {7-11} title="To the list of contacts, add the product details we want to use in the email content along with special offer description"
SELECT
    up.SubscriberKey,
    up.EmailAddress,
    prod.ProductName,
