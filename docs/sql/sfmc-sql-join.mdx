...
Apart from the `JOIN` there is also an `UNION` operator that serves similar purpose. It concatenates outcomes of multiple `SELECT` statements.
...
As you can see in the above example, `UNION` doesn't require any relationship between the two queries. There is no [`ON` element](#joining-on). On the other hand, `UNION` cannot add any new columns.
...
The names does not have to be the same, however it is best practice to match the aliases for readability purposes.
...
By default `UNION` will ignore duplicates. If you want to keep them, use `UNION ALL`.
...
If you are using deliverability monitoring suite, you probably are using seedlist. If not - a seedlist is a list of bot email addresses that are used to estimate inbox placement of your communication.

In most cases you will store your seedlist in a separate Data Extension and will have only Email Address and fake Subscriber Key columns filled in. How to push them easily into your segment? With `UNION`, of course:
...
In the example above not only we are adding the Seedlist to your main segment, but also we are providing static placeholders for columns that are used in personalisation.

:::note You Should Know

You can mix and match both `JOIN` and `UNION` to create your perfect segment. And you can union arbitrary data without a [`FROM`](./sfmc-sql-from.mdx).
