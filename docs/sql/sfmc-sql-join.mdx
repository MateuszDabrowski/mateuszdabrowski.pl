...
```sql {5} title="Check who opened the email and when by querying the _Open System Data View"
SELECT wel.SubscriberKey,
       wel.EmailAddress,
       o.EventDate AS OpenDate
  FROM WelcomeCampaignSegment AS wel
       JOIN _Open AS o ON o.SubscriberKey = wel.SubscriberKey
```
...
```sql {5} title="The same query, but this time joined with LEFT JOIN"
SELECT wel.SubscriberKey,
       wel.EmailAddress,
       o.EventDate AS OpenDate
  FROM WelcomeCampaignSegment AS wel
       LEFT JOIN _Open AS o ON o.SubscriberKey = wel.SubscriberKey
```
...
```sql {5-6} title="Example of exclusion applied to LEFT JOIN"
SELECT wel.SubscriberKey,
       wel.EmailAddress,
       o.EventDate AS OpenDate
  FROM WelcomeCampaignSegment AS wel
...
```sql {5} title="Not a best use case for the FULL JOIN"
SELECT wel.SubscriberKey,
       wel.EmailAddress,
       o.EventDate AS OpenDate
  FROM WelcomeCampaignSegment AS wel
       FULL JOIN _Open AS o ON o.SubscriberKey = wel.SubscriberKey
```
...
```sql {4} title="Self-join using INNER JOIN will output only categories that have a parent category"
SELECT c.CategoryName  AS Category,
       pc.CategoryName AS ParentCategory
  FROM WristwatchesDE AS c
       INNER JOIN WristwatchesDE AS pc ON pc.ParentCategoryID = c.CategoryID
```
...
```sql {7-9} title="Flattening three levels of Salesforce Account hierarchy"
SELECT a1.Id   AS Level1AccountId,
  ...
  FROM Account_Salesforce AS a1
       LEFT JOIN Account_Salesforce AS a2 ON a2.Id = a1.ParentId
       LEFT JOIN Account_Salesforce AS a3 ON a3.Id = a2.ParentId
```
...
SELECT _Sent.SubscriberKey,
       _Job.EmailName
  FROM _Sent
       LEFT JOIN _Job ON _Job.JobID = _Sent.JobID
```
...
All you need to do is write your label after a data source name. For the readability purposes, I recommend using the `AS` keyword between. Now you can leverage the shortened name to prefix the selected columns. It's instrumental when pulling data from multiple Data Extensions.
...
```sql {5-7} title="Contacts used for three different ToFu campaigns that are ready for a MoFu campaign"
SELECT wel.SubscriberKey,
       wel.EmailAddress,
       nsl.PhoneNumber,
       ebo.EbookName
  FROM WelcomeCampaignSegment AS wel
       INNER JOIN NewsletterPromoCampaignSegment AS nsl ON nsl.SubscriberKey = wel.SubscriberKey
       INNER JOIN EbookDownloadCampaignSegment AS ebo ON ebo.SubscriberKey = wel.SubscriberKey
```
...
```sql {6-8} title="To the list of contacts, add the product details we want to use in the email content along with special offer description"
SELECT up.SubscriberKey,
       up.EmailAddress,
       prod.ProductName,
       prod.ProductPrice,
       promo.Offer
  FROM UpSellCampaignSegment AS up
       LEFT JOIN ProductDetailsList AS prod ON prod.ProductId = up.OfferedProduct
       LEFT JOIN CustomOfferList AS promo ON promo.PromotionId = up.OfferedPromotion
```
...
```sql {6-8} title="Same query as above, but this time with a mix of INNER JOIN and LEFT JOIN"
SELECT up.SubscriberKey,
       up.EmailAddress,
       prod.ProductName,
       prod.ProductPrice,
       promo.Offer
  FROM UpSellCampaignSegment AS up
       INNER JOIN ProductDetailsList AS prod ON prod.ProductId = up.OfferedProduct
       LEFT JOIN CustomOfferList AS promo ON promo.PromotionId = up.OfferedPromotion
```
...
SELECT jan.SubscriberKey,
       jan.EmailAddress,
  FROM JanuaryEventParticipants AS jan

 UNION

SELECT feb.SubscriberKey,
       feb.EmailAddress
...
```sql {8,16} title="AutoKill is the limit of your imagination"
SELECT wel.SubscriberKey,
       wel.EmailAddress,
       wel.FirstName,
       i.Interest
  FROM WelcomeCampaignSegment AS wel
       LEFT JOIN InterestsDataPoint AS i ON i.SubscriberKey = wel.SubscriberKey
...
SELECT '123456789987654321' AS SubscriberKey,
