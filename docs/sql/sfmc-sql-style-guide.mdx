...
2. Above is also very beneficial during difference checking and when leverging git for query version control. Thanks to comma in the beginning, addition and deletion of a Column in both those places is displayed correctly as a single line change.
3. Having a comma in the beginning makes it easier to quickly assess where is the beginning of a new Column definition. This is huge readability improvement when you are using more complex structures, like [`CASE` in `SELECT`](./sfmc-sql-select.mdx#conditional-values-with-case).
4. It's also easier to quickly assess whether you have a comma for each Column. With lines of various length it is much harder with comma at the end.

Notice, that for the sake of readability I also indent first Column by two spaces, so that it is in line with the following ones. Similarly, I indent `END` after [`CASE` in `SELECT`](./sfmc-sql-select.mdx#conditional-values-with-case) by two spaces for the same reason.
