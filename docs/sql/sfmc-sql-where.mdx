...
It is also an excellent tool for database cleaning by finding incorrect values within automation. To learn more about the possibilities, check out the [`LIKE` guide](sfmc-sql-like).
...
Sometimes you want to rather exclude some records instead of searching for them. In many cases, you can do it just adjusting the operators (for example `<>` instead of `=`). There are, however, some situations where the `NOT` operator is the best tool for the job.
...
It works, but in most cases, you can write it better with other means mentioned above. This query was previously coded with single-line `WHERE` utilizing `NOT IN` statement.
