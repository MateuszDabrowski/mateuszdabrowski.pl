...
You can do better even with the [search version of `CASE`](./sfmc-sql-select.mdx#search-version-of-case), as it allows you to use `AND`/`OR` within the `CASE`:

 ```sql {5-8} title="All subscribers NOT with unsubscribed, bounced or temporarily held status"
SELECT
    SubscriberKey,
    EmailAddress
FROM _Subscribers
WHERE 1 = CASE
        ...
        WHEN Status = 'held' OR Status = 'unsubscribed' OR Status = 'bounced' THEN 0
    END
```
