...
The [`SELECT`](sfmc-sql-select) statement alone won't do anything. Once you have selected columns you want to work with, you also need to pick the table (Data Extension or System Data View) `FROM` which the query will take the right columns. The code for this is simple:
