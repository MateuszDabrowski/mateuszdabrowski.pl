...
5. This Data View along with [`_EnterpriseAttribute`] does not have data retention.
...
5. This Data View along with [`_Subscribers`] does not have data retention.
...
It gets even more interesting when you consider that which unsubscribe level will be triggered depends on email configuration, Email Studio configuration, preference center configuration and subscriber's behaviour. Oh, and there is no out-of-the-box unsubscription from Data Extensions, which are the most popular segmentation and targeting tool.
...
Additionally, this Data View stores data only for six months, so retention-less `_Subscribers` Data View might show more unsubscribed records if you are not cleaning them from SFMC on regular basis.
...
3. The most useful data here are:
...
If the subscriber clicks "Unsubscribe from All" on out-of-the-box unsubscription center, it will perform Master Unsubscribe from all Lists. As it is single event, it will be stored as a single record in the `_Unsubscribe` Data View with random ListID assigned to it. For a full picture you will need to check [`_ListSubscriber`](#_listsubscriber) Data View.
...
Email Studio allows you to choose whether unsubscribes should happen from all Business Units or just on single Business Unit Level.

If you select to have separate subscription status for each Business Unit, your [Ent._Subscribers](#_subscribers) `Status` will not reflect subscription change happening on the child BU.

To find subscribers in your account and their child business unit unsubscribe data, use the _BusinessUnitUnsubscribes data view in Marketing Cloud Automation Studio. You can only run the data view on the parent account only and not on child business units.
...
UnsubReason | The reason listed is a custom value configured by your administrator. | Number | X
...
Name | Description | Data Type | Nullable
-- | -- | -- | --
AddedBy | The ID of the user who ran the process that added the subscriber | Number |
...
SubscriberID | The subscriber ID for the affected subscriber | Number |
SubscriberKey | The subscriber key for the affected subscriber | Text |
SubscriberType | The type of subscriber | Text | X
...
View data regarding spam complaints from subscribers about email sends from your account by querying the _Complaint data view. Records dating back six months from the day the query runs are available.
