...
5. This Data View along with [`_EnterpriseAttribute`](#_enterpriseattribute), [`_BusinessUnitUnsubscribes`](#_businessunitunsubscribes) and [`_ListSubscribers`](#_listsubscribers) does not have data retention.
...
5. This Data View along with [`_Subscribers`](#_subscribers), [`_BusinessUnitUnsubscribes`](#_businessunitunsubscribes) and [`_ListSubscribers`](#_listsubscribers)  does not have data retention.
...
`_Complaint` Data View stores Email Service Providers Feedback Loop (FBL) data. It means that if someone clicks Report Spam button on yout email and there is FBL in place, this information will be added to the Data View. This will also result in an unsubscribe event and be visible in the [`_Unsubscribe`](_unsubscribe).
...
You can additionally [register for the Yahoo Feedback Loop](https://help.salesforce.com/s/articleView?id=000312472&type=1). Gmail also has their [Feedback Loop system](https://help.salesforce.com/s/articleView?id=000353530&type=1) but it works differently and cannot be added to `_Complaint` Data View.

This Data View along with [`_Bounce`](#_bounce) should be monitored on regular basis to assess health of your email marketing.
...
2. Use `_Complaint.IsUnique = 1` in [`JOIN`](../sql/sfmc-sql-join.mdx) or [`WHERE`](../sql/sfmc-sql-where.mdx) to focus on first occurrences of each event.
3. The `Domain` is useful for aggregating the Complaint data to see any domain-specific problem with targeting your communication.
...
It starts with 4 levels of unsubscriptions:

1. List unsubscribe = no more sends from a particular list / publication list.
...
And it gets even more interesting when you consider that triggering specific unsubscripction level depends on email configuration, Email Studio configuration, preference center configuration and subscriber's behaviour.
...
To give us at least a chance to make sense of that mess, Salesforce created multiple System Data Views related to (un)subscription.

### _Unsubscribe

`_Unsubscribe` Data View stores data about unsubscribe events. The wording here is crucial, as it explains the difference from [`_Subscribers`](#_subscribers) Data View unsubsciption counts.

In `_Subscribers` you can check what is the __current subscription status__ for All Subscribers list. In `_Unsubscribe` you see the __unsubscription events__ happening from List-Unsubscribe Header, `LogUnsubEvent` API call, Reply Mail Management responses, Complaints and out-of-the-box Preference Center - all linked to a specific [`_Job`](#_job). It is also not limited to All Subscribers, but also covers unsubscribes from other types of lists.

This means that any non-job related status changes like Import Activity, direct API change, manual change in the Email Studio will be reflected in the `_Subscribers` Data View, but not in the `_Unsubscribe`. On the other hand, `UnsubEvents` does not necessarily change the Status in the `_Subscribers`. For example, taking care of List-Unsubscribe Header clicks requires dedicated automation that will get this data from `_Unsubscribe` Data View and push to other parts of your consent data model.

This Data View stores data only for six months, so retention-less `_Subscribers` Data View might show more unsubscribed records if you are not cleaning them from SFMC on regular basis.

All of this makes `_Unsubscribe` much better source of knowledge about unsubscription.
...
2. Use `_unsubscribe.IsUnique = 1` in [`JOIN`](../sql/sfmc-sql-join.mdx) or [`WHERE`](../sql/sfmc-sql-where.mdx) to focus on first occurrences of each event.
3. If you query this Data View on child Business Unit, it will return only unsubscribe events from that specific BU.
4. The most useful data here are:
...
If the subscriber clicks "Unsubscribe from All" on out-of-the-box unsubscription center, it will perform Master Unsubscribe from all Lists. As it is single event, it will be stored as a single record in the `_Unsubscribe` Data View with random ListID assigned to it. For a full picture you will need to check [`_ListSubscribers`](#_listsubscribers) Data View.
...
[`_Subscribers`](#_subscribers) Data View might give you information about current state of subscriptions on All Subscribers list. But if in Email Studio you configure Business Unit Level unsubscribes, this information will not be useful at all, as the data stored there will not reflect the subscription status in a specific Business Unit.
...
Dspite `_BusinessUnitUnsubscribes` Data View being focused on Business Units, it can be queried only from Enterprise level (so on Parent BU or using [`Ent.` prefix](../sql/sfmc-sql-from.mdx#enterprise-system-data-views)).
...
1. Remember that it stores only currently unsubscribed contacts. Not unsubscribe event data (that's in [`_Unsubscribe`](_unsubscribe)) and not other statuses (like bounced, held). It will also not show unsubscribed users that were deleted or resubscribed.
...
3. The real treat in this data view (apart from finally having easy option to see Business Unit unsubscribes) is `UnsubReason` field that can tell you a bit more about the unsubscribe.
4. This Data View along with [`_Subscribers`](#_subscribers), [`_EnterpriseAttribute`](#_enterpriseattribute) and [`_ListSubscribers`](#_listsubscribers) does not have data retention.
...
list status is the current status of the subscriber specific to the list.

Query this data view in Automation Studio to find subscribers on lists in your Marketing Cloud account. This view includes subscribers found on the Triggered Send Managed List.

View subscribers on lists in your account by querying the _ListSubscribers data view.

:::note You Should Know

All Subscribers List ID is shared across whole account. If you selected "Unsubscribe from this Business Unit Only" option in Email Studio,
...
CreatedDate | The date the subscriber was added to the list | Date | X
...
1. TODO
2. This Data View along with [`_Subscribers`](#_subscribers), [`_EnterpriseAttribute`](#_enterpriseattribute) and [`_BusinessUnitUnsubscribes`](#_businessunitunsubscribes) does not have data retention.
...
2. Monitor this data view and blocklist numbers with big `BounceCount` values to clean up your database, save money and protect from spamming potential future owners of the recycled phone number.
3. It's not possible to know real delivery status for SMS sends - Marketing Cloud reports only whether SMS carrier accepted the message. It has no knowledge of what happens with it after carrier takes over. However, most of the SMS messages accepted by carrier are delivered.
