...
What all the players do is putting an invisible 1px wide image in the email body that is unique for every single email you send. When someone opens an email, its images are being downloaded from the Marketing Cloud server. When that happens with that unique image, SFMC treats it as a proof that the email was opened. And in most cases it is right. But...

The images are not always rendered. Many users block images from loading by default. It can be because of comapny policy, the privacy concerns or limited internet bandwith.
