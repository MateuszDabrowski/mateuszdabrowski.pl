...
Name | Description | Data Type | Nullable
-- | -- | -- | --
_SubscriptionDefinitionID | | Text(200) |
_MobileNumber | | Text(15) |
_OptOutStatusID | | Text | X
_OptOutMethodID | | Text | X
_OptOutDate | | Date | X
_OptInStatusID | | Text |
_OptInMethodID | | Text | X
_OptInDate | | Date | X
_Source | | Text | X
_CreatedDate | | Date |
_CreatedBy | | Text | X
...
When working with `_MobileSubscription` Data View:
...
OptOutStatusID | Whether the subscriber has opted out of SMS messages. See key for values. | Number | X
OptOutMethodID | Method the subscriber used to opt out of SMS messages. See key for values. | Number | X
OptOutDate | Date the subscriber opted out of SMS messages | date | Date | X
OptInStatusID | Whether the subscriber has opted in to SMS message. See key for values. | Number |
OptInMethodID | Method the subscriber used to opt in to SMS messages. See key for values. | Number | X
OptInDate | Date the subscriber opted in to SMS messages | Date | X
Source | The subscription source | Number | X
CreatedDate | Date the message was created | Date |
ModifiedDate | Date the message was modified | Date |
...
Sent | If the message was sent (1 = true, 0 = false) | Number |
Delivered | If the message was delivered (1 = true, 0 = false) | Number | X |
Undelivered | Whether the message was delivered successfully or not (1 = true, 0 = false) | Number | X |
Unsub | If the subscriber unsubscribed (1 = true, 0 = false) | Number X |
OptIn | Whether the subscriber has opted in to SMS messages (1 = true, 0 = false) | Number | X |
OptOut | Whether the subscriber has opted out of SMS messages (1 = true, 0 = false) | Number X |
Outbound | If the message was outgoing (1 = true, 0 = false) | Number X |
Inbound | If the message was incoming (1 = true, 0 = false) | Number X |
