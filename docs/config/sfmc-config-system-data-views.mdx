...
OptIn | Whether the subscriber has opted-in to the SMS messages | Boolean | X |
...
5. `ResponseToMobileMessageTrackingID` is an excellent way to check whether the customer responded. You can make [Self-Join](../sql/sfmc-sql-join.mdx#self-join) to output response data in the same line.
6. Despite Marketing Cloud officially supporting this Data View, many columns seem to be hit or miss. `IsValid` isn't working for me at all. `SendID`, `SendSegmentID`, `SendJobID`, `SendGroupID`, `SendPersonID` are entirely random in whether they will be filed in or not. It changes even for consecutive sends of the same SMS to the same person. Also, `SubscriberID` and `SubscriberKey` are random. Sends to the same, unmodified Contact may fill both, one or neither column. For the time being, I do not recommend using those columns for production purposes.
7. `SMSStandardStatusCodeId` and `Description` are great for understanding your SMS send's current status.
