...
MobileConnect Data Views are unique, as most of them are no longer supported by Salesforce. But works. And in some cases - have awesome features, that are not available in any other way.

:::note You Should Know

Phone number format information #TODO

:::

### _MobileAddress

`_MobileAddress` Data View is officially not supported by Salesforce. But it's too good not to use it. And there is still no replacement for it. So let's dive in.
...
_FirstName | | Text | X
_LastName | | Text | X
_UTCOffset | Defaults to 0 | Number | X
_IsHonorDST | Defaults to false | Boolean | X
...
It contains nearly all the same fields (sans `_CreatedBy` and `_ModifiedBy`) and adds even three more (`LogDate`, `SubscriberKey` and `MovileSubscriptionID`). However, be aware that there is different logic for OptIn/OptOut fields in both Data Views. `_MobileSubscription` shows the data #TODO.
...
_OptInStatusID | Whether the subscriber has opted in to SMS message. | Text |
...
You shouldn't be working with `_MobileSubscription` Data View.

---

### _UndeliverableSMS

`_UndeliverableSMS` Data View is officially not supported by Salesforce. But it is still working and might provide additional information on top of what is available in the newer [`_SMSMessageTracking`].
...
`_SubscriberSMS` Data View is a __legacy table__. It is neither supported nor updated and you won't have records there.
...
TransactionalOptInDate | | Date |
TransactionalOptOutDate | | Date | X
MarketingOptIn | | Boolean |
MarketingOptInDate | | Date |
MarketingOptOutDate | | Date | X
SMSCarrierName | | Text | X
...
SubscriptionDefinitionID | The unique keyword ID | Text |
...
OptInStatusID | Whether the subscriber has opted in to SMS message. | Number |
OptInMethodID | Method the subscriber used to opt in to SMS messages. | Number | X
