...
8. There are new fields, `JBDefinitionID` and `JBActivityID`, that let you cleanly match SMS data to [`Journey`](#_journey) and [`_JourneyActivity`](#_journeyactivity) Data Views using `VersionID` and `ActivityID` respectively. No longer you are limited to a fuzzy [`JOIN`](../sql/sfmc-sql-join.mdx) using `Name` field.
