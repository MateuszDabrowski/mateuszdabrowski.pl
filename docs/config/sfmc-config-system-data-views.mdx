...
1. If you are using the query in the child Business Unit, always check whether you are querying local or enterprise Data View ([`Ent.`](../sql/sfmc-sql-from.mdx#enterprise-data-extension) prefix).
...
```sql
SELECT
      _SubscriberID                     AS SubscriberID
    , ProfileAttributeWithoutSpaces
    , [Profile Attribute With Spaces]   AS ProfileAttributeWithSpace
...
5. This Data View, along with [`_Subscribers`](#_subscribers), [`_BusinessUnitUnsubscribes`](#_businessunitunsubscribes) and [`_ListSubscribers`](#_listsubscribers) , does not have six-month data retention.
...
3. If you have triggered a send for an email and after a few minutes you don't see it here, in most cases, it means that the send was blocked and never left Marketing Cloud. Check [enhanced Send Log](../sql/sfmc-sql-snippet-enhanced-send-log.mdx), applied personalisation, data source, `RaiseError` scripts.
...
SELECT
    ...
    , MobileNumber
    , TransactionalOptIn
    , TransactionalOptInDate
    , TransactionalOptOutDate
    , MarketingOptIn
    , MarketingOptInDate
    , MarketingOptOutDate
    , SMSCarrierName
