...
This pattern covers one of the most popular Multi-Org setups where you have up to five production Business Units connected to the production Salesforce CRM Org but also want to have a full testing suite with dedicated Dev & Test Business Unit connected to Dev Org. The closest you can get to CI/CD with the lack of dedicated SFMC sandboxes.

It is much cleaner approach than the [API Extended Single-Org Setup](#api-extended-single-org-setup) and as long as long as you have up to five production Business Units (soft but highly recommended limit per single Integration user) doesn't have many cons.
