...
What Behavioral Triggers does in each case is push relevant data (items left in the cart, browsed without conversion or added to wishlist) to the Einstein Backend and - after configured time - to Marketing Cloud Data Extension.

Then you can leverage this data extension as both Journey Entry, as well as a source of personalisation using built-in Behavioral Trigger Content Block. The latter allows you to use clean UI to configure dynamic content using the obtained engagement data.
...
Behavioral Triggers seems to be easy to implement, but a lot is going in the back end. It is good to understand the basics of it, as it might help a lot when triaging a problem or customising the solution.
...
2. Customer clicks on a product page - you trigger [Page View data layer](#page-view-data-layer) with link to that product (Abandoned Browse scenario).
3. Customer finds an exciting product and adds it to the wishlist - you trigger [Wishlist data layer](#wishlist-data-layer) (Abandoned Wishlist scenario).
4. A customer decides on one of the products and adds it to the cart - you trigger [Cart data layer](#cart-data-layer) (Abandoned Cart scenario).
5. Customer goes through the purchase process and buys the product - you trigger [Cart data layer](#cart-data-layer) with explicit parameter (This blocks Abandoned Cart from launching communication).
...
9. Once there is an event ripe for reengagement, Behavioral Trigger will put it into a system created Data Extension (one per scenario) with three data points: subscriber key, timestamp and the encrypted data.
...
Next step allows you to configure Custom Profile Attributes. It is beneficial for general Einstein purposes, but it is also crucial for Behavioral Triggers if you want to personalise your win-back email for unknown (to Marketing Cloud) customers.
...
Last step here is to enable Einstein Data Extensions. To do this:
...
Select the one you want to start with, and you will be able to set supression rules - one related to multiple triggers and the second to a recent purchase. For initial configuration, I recommend setting both to 0. It will make testing much more manageable. Once you go to production, change those appropriately as per your business requirements.

When you confirm the setup, Marketing Cloud creates the system Data Extension that will store Behavioral Trigger data (one per scenario).
...
Currently, it is not working at all - it does not save the selected setting, and it always show s1 hour. Until fixed, you can manage this by going to Einstein » Einstein Overview » Email Recommendations » Admin » Implementation and clicking Advanced Settings on the left pane. There will be Session Length option with the same range. It is the same setting.
...
The second data layer that will be a key for the sending Behavioral Triggered communication pushes the data about the customer itself. Remember that this layer must be added to your Collect.js before any tracking layer for proper attribution.
...
The base function is pushing the unique identifier of your customer (don't be intimidated by the `email` in that code - you do not have to pass the email address there). Depending on your use case, it might be either:
...
You can leverage the same automation also to be a trigger for the Journey Entry to be sure that all records in the Journey already exist as Subscribers.
...
In many cases, all those domains might be the same, but if you are using Content Delivery Network or different subdomain for backend, those might differ. If you cannot click `Register`, be sure you are passing just the domain - the bolded part of https://**domain.name**/folder/structure?query=string.
...
Remember that this data layer updates the Catalog. Even if you push it in the context of specific subscriber and along the [Track Cart](#track-cart) data layer, it will still update the global Catalog for everyone.
...
2. The `price` parameter here is different from `price` or `sales_price` available in [Catalog Update](#catalog-update-streaming). The Catalog ones are global - the same for all your customers. The `price` pushed within Track Cart is unique for this particular customer and might be different from the global ones (think VIP status or a coupon used). It won't update your Einstein Catalog.
3. Always push the whole cart. If your customer has more then one item in it - put all of them in this data layer. If customer deletes item from the cart - push the data layer without the deleted one. The last version pushed is considered the final state of the cart for personalisation purposes.
4. Always be sure to clear the Track Cart data whenever the cart data is no longer needed. If the customer purchased the products, be sure to use [Track Purchase](#track-purchase) data layer. If the customer deleted all the products from their cart, use the Clear Cart parameter:
...
Now you can go forward with Behavioral Triggers. [Collect.js](#collectjs) will push the required data to Einstein backend, and after Session Timeout Limit it will evaluate it against Suppression Rules as per your [configuration](#Behavioural-triggers-configuration). Then, the Behavioral Triggers adds Abandoned Engagement data to the respective [Data Extension](#Behavioural-triggers-configuration). Now it's time to use it.

The standard way the leverage this information is to use those abandonnment Data Extensions as a Journey Entry Source. The real magic, however, will happen in the email you will be sending in this Journey. And that is thanks to the out-of-the-box Behavioral Trigger Content Block.
...
2. You are limited only to the fields that you [configured for the Product Catalog](#catalog-and-product-attributes). You cannot, for example, use the individual price that you passed in the [Track Cart](#track-cart) data layer. You can vote for it on [IdeaExchange](https://trailblazer.salesforce.com/ideaView?id=0874V0000003lCKQAY).
3. There is no information on how to get access to the data outside of the Content Block - but you can learn about it [later in this article](#6-response-parsing).
4. You cannot leverage the full potential of the Einstein API, as the hard-coded request does not have parameter responsible for pulling the [Einstein Custom Profile Attributes](#user-and-profile-attributes). You can vote for it on [IdeaExchange](https://trailblazer.salesforce.com/ideaView?id=0874V0000003lB7QAI).
...
As far as you can fix the first issue with good CSS knowledge, the rest of them require a bit more work.
...
1. In **Original Code**, you will find full code of the built-in Behavioral Trigger Content Block. Shown configuration displays a product image with name and description plus regular/sale price. The number of items is limited to four in a two-column layout.
2. In **Code Highlights** tab, I focus on some part of the code that I found exciting and deserve some attention.
...
Current version of Behavioral Trigger Content Block is not rendering correctly on Yahoo, AOL and Windows Outlook. Learn more [here](../../../ideas#behavioral-trigger-content-block-rendering-on-yahoo-aol-windows-outlook).
...
Two first lines here are pulling data from the AMPScript described in first highlight. As mentioned, it can be optimised by making the personalisation here and removing AMPScript alltogether.
...
1. For each element of the `products` key, it creates a variable build with `item_` prefix, value name and `_X` suffix, where X is the counter of the item (so 1 for the first item, 2 for the second and so on). It means you can get the first link to the first item by calling `@item_link_1` variable. To get the name of the second item goes with `@item_name_2`.
2. If the response contains `current_cart` key (it is available only if the [Track Cart](#track-cart) was triggered), then it also creates variables for it. The structure is nearly the same - just the prefix changes to `cart_`. The most compelling use case here? You can get the `price` information that you pushed via [Track Cart](#track-cart) (the one unique to specific cart, not coming from the Product Catalog) by calling `@cart_amount_1` for the first item.
...
4. All other keys of the response are also created with just their name. So to get information whether the cart converted you can call `@purchased` variable.
...
3. If you want to use more then one Abandonment Engagement in one email, you will have to alter the prefixes in this code not to overwrite the AMPScript variables.
...
The rest of the code is just taking care of proper display of the information in the email and stopping the send with `RaiseError` if the customer purchased the cart or the API did not respond. It is added to the email using `%%=treatascontentarea('BT Trigger', @content)=%%`.
...
To fully leverage the customisation options of the Behavioral Trigger Content Block, below, you can check the sample responses from Einstein Backend (IgoDigital). They will help you know what AMPScript variables can be available after the Content Block.
...
During Behavioral Trigger implementation there are at least three key phases when you might need to debug your solution. As you will be merging multiple systems and working on time-sensitive data using a fresh feature, possibility to look behind the curtain will be crucial. Thankfully, there are few options that we can use.

### Debugging Collect.js

First thing to check is your front end Collect.js implementation. To quickly check, whether Collect.js is available on your website open Developer Tools in your Browser and go to Console tab. In there, write `_etmc`.

If you see `ReferenceError: _etmc is not defined`, it means that the script is not available. Make sure that you are checking for it on a page that is supposed to have Einstein / Behavioral Triggers implemented and that the script is added correctly.

If the script is implemented, you will see a huge Object containing over a dozen of keys - many of which are the functions that pass the data from your website to IgoDigital.

Let's leverage its power by writing in the console `_etmc.debug = true` to enable debugging mode of Collect.js. Once completed, it will log every change to applied to the dataLayer. Make a tracked action - like adding a product to a cart - to see it work.
...
You can check how Collect.js works and what are the other features of this script by reviewing its contents. To do this, go to the link contained in your base Collect code. It should be: `http://YOUR_MID.collect.igodigital.com/collect.js`.

:::

Another option to observe what Collect.js is sending to Einstein IgoDigital backend is to go to Developer Tools » Network and look for calls made to `igodigital.com` address. You will see mirror data in form of API requests. For example, above JSON will be displayed there as a payload:
...
2. It is correctly assigned to the events happening on you website you want to track
3. It captures necessary information, like price, name of the product, or id
4. It passes the correctly structured JSON with this data to IgoDigital

Once all of the above is validated, it means that the correct data should be available to Salesforce Marketing Cloud Einstein and Behavioral Triggers.
