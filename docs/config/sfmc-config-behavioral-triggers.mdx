...
Behavioral Triggers seems to be easy to implement, but a lot is going in the back end. It is good to understand its basics, as it might help a lot when triaging a problem or customising the solution.
...
You can leverage the same automation to be a trigger for the Journey Entry to ensure that all records in the Journey already exist as Subscribers.
...
2. The `price` parameter here is different from `price` or `sales_price` available in [Catalog Update](#catalog-update-streaming). The Catalog ones are global - the same for all your customers. The `price` pushed within Track Cart is unique for this particular customer and might differ from the global ones (think VIP status or a coupon used). It won't update your Einstein Catalog.
...
2. You are limited only to the fields that you [configured for the Product Catalog](#catalog-and-product-attributes). For example, you cannot use the individual price that you passed in the [Track Cart](#track-cart) data layer. You can vote for it on [IdeaExchange](https://trailblazer.salesforce.com/ideaView?id=0874V0000003lCKQAY).
...
As far as you can fix the first issue with good CSS knowledge, the rest require a bit more work.
...
5. You receive status 200 on the API call in Network tab
...
1. Go to Journey Builder » Behavioral Triggers and check whether there is a required Trigger Type and its Status is Active.
2. Go to Status tab and check whether Status next to Collect Tracking Code Type is Ok. Don't worry here about the numbers in Collected Today - those are frequently not aligned with reality.
3. Go to Audience Builder » Contact Builder » Data Extensions and in the main local folder look for `IGO_PRODUCTS` and `IGO_PROFILES` Data Extensions. You should see the captured users and products details that can be used by Einstein solutions. It validates that the data is flowing through Collect.js.
4. Finally it's time to check in the same main local folder the Data Extension related to the Behavioral Trigger Type you have activated. It should be named `abandoned_TYPE_01234` (with `TYPE` being wishlist, cart or browse depending on what you enabled).
...
If you do not see data in the above places - most probable cause is a problem with Collect.js script implementation. Go to previous debugging step and check whether everything is set up correctly. If you are certain that all is good and after waiting the required amount of time, contact Salesforce Support for help.

### Debugging BT Content Block

Once the above steps are done, it's time to play with [Behavioral Trigger Content Block](#behavioral-trigger-content-block).

The easiest way to check whether everything works fine is adding the default one and checking whether the data makes sense. The hard part starts when there is an issue with the outcome (wrong data, missing data or just no preview).
...
This is very basic script, but helps with finding the source of an issue. As you can see, a lot of it is just a copy paste of the original [Behavioral Trigger Content Block](#behavioral-trigger-content-block). But instead of displaying beautiful abandoned cart message, it will show you Subscriber Key, Email Address, raw data that is being shot to IdoDigital Backend and the response from it.

Using those information you can validate whether the raw data, email address and subscriber key all belong to the same Contact and whether the response is aligned with what you were pushing during your testing on the website.

If there is anything wrong with those information - check again, whether you have a correct Collect.js implementation, whether it captures the correct product information and whether it assigns it to the correct Contact data.

If the response is correct, but there is still some issues with what the Behavioral Trigger Content Block displays - welcome to the new adventure - a Custom Behavioral Trigger Content Block creation.