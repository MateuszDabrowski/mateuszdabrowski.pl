...
1. In **Original Code** you will find full code of the built-in Behavioral Trigger Content Block. It is configured to show product image with name and description plus regular/sale price. Number of items is limited to four and they are shown in two-column layouyt.
...
Those two variables store all options that you have selected when configuring the Behavioral Trigger Content Block. You can alter them outside of what is possible in the UI. You can also change the HTML that is being generated for the products.

Remember however, that the classes visible in the `markupFragments` are used by the Behavioral Trigger Content Block CSS, so changes might break the styling.
...
More interesting, however, is the third line. You can configure localization for your products in [Email Recommendations Configuration](#user-and-profile-attributes), but it is not used by the Behavioral Triggers - officially. Thanks to this little line you can - even with the built-in standard Content Block - decalre the locale source for the Subscriber and set it to `@event_locale` AMPScript variable. Bam, your Behavioral Triggers are now localized.
...
Just change the change the values after the equal sign to the exact names of your [Custom Profile Attributes](#user-and-profile-attributes) that you want to have available in your code.
...
TODO: Validate whether those AMP variables works and update first highlight based on the answer
