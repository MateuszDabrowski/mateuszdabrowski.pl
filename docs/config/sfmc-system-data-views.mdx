...
System Data Views are **built-in backend Data Extensions** that you can use to find information about your subscribers and sends. You **cannot change them**, but you can leverage their data in SQL queries for quick reports in [Query Studio](./sfmc-appexchange-solutions.mdx#query-studio) or outputting to the standard Data Extension. They are excellent fuel for SSJS logic.
...
Be sure to check the excellent [Dataviews.io](https://dataviews.io) - an interactive system tables relationship diagram created by [Zuzanna Jarczyńska](https://sfmarketing.cloud/author/zjarczynska/). It will for sure help you with multi-view [`JOIN`s](../sql/sfmc-sql-join.mdx).
...
3. If you are performing A/B tests or want to analyse the subject line's impact on performance, be sure to pick `EmailSubject`. Pair it with [`CASE`](../sql/sfmc-sql-case.mdx#conditional-values-with-case) to create a great knowledge source for your content team.
...
7. You can also leverage `JobID` field to [`JOIN`](../sql/sfmc-sql-join.mdx) your email sending activities directly with [`_Job_`](#_job) and pull send-specific information from engagement Data Views.
