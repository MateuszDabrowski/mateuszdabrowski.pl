...
DESelect Segment is the golden standard for a middle ground between the very limited Filtered Data Extensions and yes-code SQL Queries. Actually, "middle ground" is not fair. Over time it has been polished by the DESelect team to cover nearly all use cases that in the past were possible only with SQL.

In a fully drag and drop experience DESelect Segment can:

- Join multiple datapoints (both Data Extensions and Data Views and even on multiple relationships)
- Create complex, multilevel filters (including dynamic dates, aggregates and checks against other Data Extensions)
- Build Data Extensions based on the segment created in the tool
- Select values you need in the outcome, including creating custom values based on any logic
- Deduplicate using specified conditions
- Preview the outcomes to validate the configuration

And they didn't stop there - there are even few features that are not possible with any of the out-of-the-box solutions:

- Picklists creation to mitigate typo issues when working with values
- Relationships to make joining tables a breeze
- Data Sets to prepare templates multi-table relationships for quick drag and drop
- Cross Business Unit segmentation templates to speed up rolling solutions

And all that can be made available to you within an hour directly in the Marketing Cloud main menu.

#### The Bad

It's not free. But it is nicely priced for the power you get. It's up to you whether you want to invest in SQL competencies or no-code solution that can do the same, faster.

It's complex. Sure, it's a clean drag-and-drop interface, but because of all the available features, it has some learning curve (and training offered by DESelect).
...
While I'm a big fan of SQL, I know that it's not for everyone and makes scaling the marketing team much harder. DESelect Segment is currently the best solution to solve that problem and, with ongoing updates, it gets better and better at it.
