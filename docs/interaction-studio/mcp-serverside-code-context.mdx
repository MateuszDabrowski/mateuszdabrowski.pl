...
`context.user.currentCart` is a single object with the same structure as each of the orders stored in [`context.user.orderHistory`](#contextuserorderhistory). But because it is still a cart, not an order, most fields will be `null`/`0`.
