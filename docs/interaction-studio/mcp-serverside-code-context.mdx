...
Requires an ActionStatsRequest (`{actionName: 'Name of the action'}`) and returns the total of provided action triggers for the current user. You can also extend ActionStatsRequest with `start` and `end` date properties to limit the timeframe of action count.
...
Requires an ItemStatsRequest (`{itemType: 'CatalogName', statType: 'StatTypeName'}`) and returns the count for that stat for a given Catalog. You can find available statTypes at the [bottom of this article](#itemstattype). ItemStatsRequest can also be extended with another optional property `itemId: 'id'` to limit the result just to a specific Item within a given Catalog (itemType). Finally, as with [actionCount()](#contextuseractioncount), you can also use `start` and `end` timeframe boundaries.
...
Keep in mind that the meaning of returned value will differ depending on the selected statType - it can be count, miliseconds or money.
...
Requires StatsRequest (`{start: Date, end: Date}`) and returns the count of pages viewed in that timeframe. Both timeframe bounds are required.
...
Similar to [pageViewCount()](#contextuserpageviewcount) it requires StatsRequest (`{start: Date, end: Date}`) but returns the count of visits instead of specific pages.
...
Similar to [pageViewCount()](#contextuserpageviewcount) it requires StatsRequest (`{start: Date, end: Date}`) but returns the number of miliseconds user spent on your website in a specified timeframe.
