...
1. `context.event.fields.pageType` returns the name of the currently viewed page type as configured in the Sitemap (f.e. `'Home'`). Useful when you want to change the serverside payload for the campaign based on page type, when there are multiple sharing the content zone.
2. `context.event.fields.action` returns the name of the currently pushed action as configured in the Sitemap (f.e. `'Viewed Home'`, albeit it is much more interesting for custom actions that can drive different campaign experiences despite being triggered on the same page type as you can then dynamically modify the payload based on that action).
...
`context.services` is the most powerful part of the `context` obejct as it is packed to the brim with functions that give you access to Marketing Cloud Personalization data or let you create new recommendations.
...
It's a nested object, so we will go through it - property by property - to discuss each group of functions.
