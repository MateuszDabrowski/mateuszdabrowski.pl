...
In Template Preview you will only be able to use [`context.event.fields.item`](#eventfields) - a single property with a stringified object containing details of the Item selected in top right Simulate section):
```
"{\"type\":\"ITEM_TYPE\",\"_id\":{\"label\":\"ITEM_LABEL\",\"value\":\"ITEM_ID\"}}"
```
You can find example usage of it in my [MCP Catalog ETL Metadata Viewer](./snippets/mcp-catalog-etl-metadata-viewer.mdx).
...
The `services.catalog` object contains a set of lookup methods that help you find Items in your Marketing Cloud Personalizaiton Catalogs. It opens a big range of use cases using related Items for cross-sell and up-sell purposes.
...
Most of those methods returns Item object - the content of it will differ depending on what type of item it is. By default, it will contain `id` string, and `attributes` object that has `name`, `imageUrl`, `description`, `promotable` and `archived` properties. Optionally, Item can also have `dimensions` object with RCO's data and `location` object. Finally, there are some properties that are related to a specific type of an Item - like `categories`, `skus`, `modifiedTime` or `subtitle`.
...
The `findItem()` is the most strightforward catalog lookup method. It requires a Catalog Type (f.e. 'Product', 'Category', or your custom Catalog) and Item ID (as string) to return the full Item detail.
...
// Leverage those details to pull IDs of the hardcoded upsell and crosssell products
...
// Pass those IDs to yet another findItem method to get full details to fill in the handlebars with data
...
Of course, doing so many `findItem()` calls is not optimal - and MCP offers another method as a solution for that.

#### context.services.catalog.findItems()

The `findItems()` method allows you to request details for multiple Items in a single call. Cleaner and more performant. The baseline way of calling this functions works just as [`findItem()`](#contextservicescatalogfinditem), but requires an array of IDs instead of a single ID and returns an array of Item detail objects instead of single Item object.

```ts title="Return multiple Items details"
const itemsDetails = context.services.catalog.findItem('Product', ['19542', '12524', '91324']);
```

A neat use case for that method is pulling currently viewed Item details with [`findItem()`](#contextservicescatalogfinditem) to get values from a custom attribute containing alternative product IDs or a dedicated accessories for it and then pass all those found product IDs into `findItems()` to get a full detail required for a personalization showcasing those related items.
...
It can be further extended that custom catalog based approach by passing an ItemSort as a second argument. However, despite trying my luck with `sortByActivity` and `sortByPublishedDate` properties, I haven't been able to make it work. Too bad, as sorting by activity could have been interesting when working with custom catalog Items.

#### context.services.catalog.findClosestItems()

If you are using locations details for your Items, `findClosestItems()` methods will work wonders for you as it allows you to get items based on provided longitude and latitude within defined distance:
...
1. You can try to get longitude and latitude from [`context.user.location.geographicPoint`](#userlocation). This allows you for magic-like recommendations even for anonymous, first time vists. However, confidence might be limited depending on the market specifics.
2. You can capture user-provided location (be it based on the shipping details or the city they are filtering for on your website) and pass calculated long/lat to a user attribute. Then user that user attribute as source of data for `findClosestItems()` call to provide location-aware recommendations for your customers.

#### context.services.catalog.dimensionFilter()

As mentioned in [`findItems()`](#contextservicescatalogfinditems), `dimensionFilter()` method lets us create an ItemFilter that can be leveraged to find Items from a given custom catalog (used to be called a Dimension, hence method name). Just pass the catalog name as a string in there. That's it.
...
The `services.recommendations` object contains four methods related to Item recommendations. Two of them are absolutely great for delivering highly customized use cross-selling use cases wheareas the other two doesn't seem to work. A nice mixed bag. Let's dive in.
...
The `recommend()` method is a great solution for generating Item recommendations in a programmatic and highly customized way. It requires `currentItemId` and `currentItemType` as an anchor for the Recipe, `maxResults` to limit the number of outcomes, `recipeId` to apply the correct recommendation model and `userId` to pull correct affinity data.
...
It's great feature to deploy recommendations that shouldn't be changed during campaign configuration, cross-sell offerings based on currently viewed item (using its custom attributes) or item filter based on negative recipes.

:::note You Should Know

This `recommend()` method works just as the `recommend` that you can import in the template along with `RecommendationsConfig` from `recs` module. The imported one is better suited for the user friendly module that marketer can setup according to the business needs during campaign configuration phase. Context method is better for deploying hardcoded logic programatically without any input from the marketer - the key value being the possibility to provide custom Item anchor that might be different from currently viewed one.
...
The `recommendIdsOnly()` method works exactly the same as [`recommend()`](#contextservicesrecommendationsrecommend) but instead of returning an array of objects with Item details it returns an array of Item Ids as strings. Just as `recommendIdsOnly` that you can import along with `RecommendationsConfig` from `recs` module.

```ts {9} title="Return IDs of items recommended from a recipe"
const userId = context.user.id;
const recommendationsRequest = {
    currentItemId: '1017847',
    currentItemType: 'Product',
...
It can be useful if you only need to pass IDs to the e-commerce platform for it to render the campaign experience or when you want to filter out specific IDs returned by a recipe from another recommendation you deploy.

#### context.services.recommendations.smartSearch()

The `smartSearch()` method seems to be currently limited to the MCP backend purposes - I wasn't able to find the proper structure of the `query` argument it requires to make it work. In the Evergage times it enabled personalized search results when interacting with the search box on the website.

#### context.services.recommendations.smartSort()

The `smartSort()` method requires an object with `itemIds` array, `recipeId` and `userId`. It was part of the same Evergage mechanism as [`smartSearch()`](#contextservicesrecommendationssmartsearch) as it sorted the returned search recommendations to best meet the affinities of the user.

Unfortunately it seems to be unavilable as well. Each time I try to implement it, I get `System service exception` despite having proper structure. Too bad, as it would be a cool feature for [`context.services.catalog`](#servicescatalog) set of functions - ordering Item Ids pulled from a custom attribute or a catalog retrieve based on current user interest.

```ts {4} title="Item sorting that doesn't want to work :("
const userId = context.user.id;
const itemsToBeSorted = ['1018913', '60365', '1017847', '1105'];
const smartSortRequest = {itemIds: itemsToBeSorted, recipeId: '7H5OK', userId: userId }
