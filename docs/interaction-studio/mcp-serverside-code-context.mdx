...
The `smartSearch()` method allows you to get recommendations based on search query string and a recipe (for example, to recommend items when user interacts with a search box on the page):
...
The `query` parameter is looking for a match in the name (and description?), then filters and orders found Items using the provided Recipe ID.

:::note You Should Know

`smartSearch()` can return more than 12 recommendations, but as it returns whole Item details, it can impact performance. It might be helpful to filter the outcome just to the information required for your purpose.
...
The `smartSort()` method requires an object with an `itemIds` array, `recipeId`, and `userId`. It was part of the same Evergage mechanism as [`smartSearch()`](#contextservicesrecommendationssmartsearch), as it sorted the returned search recommendations to best meet the user's affinities.

Unfortunately, it seems to be unavailable. Each time I try to implement it, I get a `System service exception` despite having the proper structure. Too bad, as it would be a cool feature for the [`context.services.catalog`](#servicescatalog) set of functions—ordering Item IDs pulled from a custom attribute or a catalog retrieve based on current user interest.

```ts {2} title="Item sorting that doesn't want to work :("
const itemsToBeSorted = ['1018913', '60365', '1017847', '1105'];
const smartSortRequest = {itemIds: itemsToBeSorted, recipeId: '7H5OK', userId: context.user.id }
const smartSortedItems = context.services.recommendations.smartSort(smartSortRequest);
```

### services.smartTrends

The `smartTrends()` method is a backend tool for the MCP Trends feature. Unfortunately it doesn't seem to work in the Serverside code with `This expression is not callable` and `Unknown identifier` errors dropping despite proper `SmartTrendsRequest` argument structure (`{itemIds: ['1018913', '60365', '1017847', '1105'], itemType: "Product", lookbackMinutes: 9999 }`).
...
For now, if you want to leverage MCP Trends, you need to use the client-side code API approach based on the [Trends Global Template](https://github.com/MateuszDabrowski/mcp-campaign-templates/tree/main/Global%20Templates/SalesforceInteractions/Trends).
