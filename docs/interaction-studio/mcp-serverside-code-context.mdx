...
The `smartSort()` method requires an object with `itemIds` array, `recipeId` and `userId`.

```ts {3} title="Return smar- sorted items"
const itemsToBeSorted = ["1018913", "60365", "1017847", "1105"];
const smartSortRequest = {itemIds: itemsToBeSorted, recipeId: "7H5OK", userId: "6690fc1c49e87274" }
const smartSortedItems = context.services.recommendations.smartSort();
```
