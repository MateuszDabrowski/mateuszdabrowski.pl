...
Decorators are a TypeScript feature that can change or extend behaviour of properties in the MC Personalization's Serverside Code. You already seen some of them: [`@options`](#-select-string) used to provide picklist values, [`@richtext`](#-rich-text-string) that changes string input or [`@range`](#-datetime-range) that modifies date selection into range selection. But there is so much more available.

---

Coming soon: Imports, run() and context object.
