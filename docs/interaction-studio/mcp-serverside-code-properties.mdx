...
When [`@title` and `@subtitle`](#title--subtitle) are not enough for the context you want to provide, you can use `@markdown` decorator to go wild with text, styling and even links.
...
The `@header` will be in the same font size as input labels and `@headerSubtitle` will match [`@subtitle`](#title--subtitle) style. Oh, and passing a space - `@header(' ')` - will add a bit of whitespace. Perfect for those of us with OCD that can't stand that uneven spacing between inputs.

### @hidden & @shownIf

`@hidden` & `@shownIf` are one of the most important decorators as they allow you to control visibility of the inputs. Huge if you want to provide nice and clean campaign configuration UI that won't overhelm the marketer.

There are two ways to use them.

First, with basic `true` boolean argument, makes sense only for `@hidden`. It is perfect for data you will calculate in the [run() block](./mcp-serverside-code-basics.mdx#2-run-block) or just don't want to show to marketer.
...
Technically in both above scenarios you can use `@hidden` and `@shownIf` interchangeably after flipping the boolean appropriately. However, I find using `@hidden` only with `true` boolean argument and `@shownIf` with function argument easier to grasp when reading the code.
...
`@optional` decorator was great tool to set required and non-required inputs. Was. It no longer works. Whether you add it to the code or not, nothing will change in the UI or on saving. And I doubt it will start working, as bringing the functionality back would break all templates created with it not working in mind. So to sum up - there is no way to enforce filling an input.

---

Imports and custom properties coming soon :)