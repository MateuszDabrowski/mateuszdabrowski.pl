...
2. `div` with name and price personalisation strings (we will talk about the difference in a moment)
3. `div` with a button (we don't need any link, because whole final image will be a link)

That's it. You can add more personalisation strings - including those using attrubutes defined on the catalog object. You can of course also write nicer HTML with separate CSS style section. But in the end - it will be just a boilerplate for an image that you can preview directly in the editor.
