...
image: img/og/og-image-sf-mcp-serverside-code-basics.png
...
Whether you are creating a Web, Serverside or Triggered Campaign Template in the :product[Marketing Cloud Personalization], you start with the Serverside Code tab. It is the true heart of the :product[MCP] platform responsible for delivering data-driven personalisations.
...
Open Time Email Campaigns are using different, older system. I covered its details in a [separate guide](/docs/salesforce/marketing-cloud-personalization/open-time-email.mdx).

:::

When you open the Serverside Code tab in a new Template creation screen for Web, Serverside or Triggered Campaign within the :product[Marketing Cloud Personalization], you will be greeted by a bit intimidating interface. Let's start by breaking it down section by section:
...
If you don't see the configuration form preview (3) updates despite making changes in the serverside code (1), :product[MCP] probably froze. Check whether you are still logged in to the MC Personalization (:product[MCP] session length is shorter than :product[MCE]{bare}'s). If yes, cut (`ctrl`/`cmd` + `x`) your whole serverside code, provide any symbol (like `a`) to overwrite the cache and replace it back by pasting the code (`ctrl`/`cmd` + `v`).
...
The field configuration lets you add properties that will become part of the campaign configuration form used by marketers to set the experiences. It is the primary data input; :product[MCP] will directly pass all properties you add here to the campaign response.

<Button
    link='/docs/salesforce/marketing-cloud-personalization/serverside-code-properties/'
    text='Learn more'
    newTab='true'
/>
...
The run block execution works differently during the template creation and campaign configuration. When the marketer configures the campaign, :product[MCP] applies the changes executed within the run in real time - this may lead to content changes or even the removal of field input. You shouldn't transform the data in the fields directly but rather do it on new variables:
...
With the basics of Serverside Code processing, you are ready to start coding. Next step - build :product[MCP] Campaign Template inputs with [Serverside Code Properties](/docs/salesforce/marketing-cloud-personalization/serverside-code-properties/).
...
Here, I want to share a few of my favourite sources focusing on :product[MCP] Serverside Code and its use in Campaign Templates:

1. My [:product[MCP] Campaign Templates and code snippets](/docs/category/salesforce/marketing-cloud-personalization/snippets/)
...
Found other inspiring :product[MCP] Serverside Code examples? Let me know!