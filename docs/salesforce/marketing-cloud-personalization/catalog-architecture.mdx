...
image: img/og/og-image-sf-mcp-catalog-architecture.png
tags: [Marketing Cloud Personalization, Personalisation, Setup, Data Management]
...
The purpose of the :product[Marketing Cloud Personalization] Catalog is to store your asset data along with interconnecting relationships for personalisation and machine learning purposes.
...
However, some quirks and features require more consideration to make the most out of :product[MCP] capabilities. Especially as some wrong moves can be tough to reverse. Let's dive in.
...
Mixing both sources is very tempting as it sounds like the best of both worlds. Unfortunately, it is not recommended approach due to :product[MCP]'s backend limitations. Sending the same Items through both channels impacts performance, creates concurrency and can lead to incorrect and not-so-easy-to-fix issues with your Catalog.
...
Cons:
- Require you to export data in a particularly formatted .csv to :product[MCP] SFTP.
- Not real-time (but every 15 minutes is pretty damn close, come on!).
- Doesn't support [drill-down Category hierarchy](/docs/salesforce/marketing-cloud-personalization/ideas#full-mcp-category-hierarchy-support-for-etl) in the UI<sup>[*](#category-catalog-unique-attributes)</sup>.
...
In most cases, your systems already use flat files and FTPs for communication, and you can extend the architecture to cover :product[MCP]. If not, many tools can help you transform your data cleanly into a nice .csv and add it to :product[Marketing Cloud Personalization] Feed with the correct naming convention. You can even leverage :product[Marketing Cloud Engagement]{bare}'s Automation Studio to prepare and deliver the files in the proper format! But sometimes data is just not there - and then, ETL is a no-go.
...
*Diagram: choose the Sitemap if ETL updates every 15 minutes are too slow or you cannot schedule a .csv export to :product[MCP] SFTP, and choose ETL otherwise.*
...
Published and Expiration fields (as all :product[Interaction Studio]{as-written} date type fields) use ISO-8601 datetime format. You must pass them as `2022-04-12T10:23:37Z`.

If you want to push a datetime to :product[MCP] using [ETL](#2-mc-personalization-with-etl), you can prepare the data with SQL using clean [FORMAT](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx#format-with-dates): `FORMAT(YourDateField,'yyyy-MM-ddTHH:mm:ssZ')` and output it to a String field type (Text, if we are talking :product[MCE]{bare} Data Extension).
...
List Price | Decimal | Full sticker price of the product | Useful for price drop personalization and Product Price Reduction Triggered Campaigns. Unfortunately, there is no Price Reduction Recipe [yet](/docs/salesforce/marketing-cloud-personalization/ideas#price-reduction-ingredient-in-einstein-recipes).
...
Depending on where you look in the :product[Marketing Cloud Personalization], you might still see "Dimensions" - an old name for related Catalog Objects.
...
There are some gotchas you can encounter when working with :product[MCP] Catalog. Here I gathered solutions to frequent problems and some undocumented tricks that can make your life easier.
...
2. Upload that file to the testing folder on the :product[Marketing Cloud Personalization] SFTP
...
8. Here you have it; the full :product[MCP] Catalog exported to a .csv file. Two notes: fields you pushed with the `relatedCatalogObject` prefix will return with the `dimensions` prefix, and Multistring fields will return as Arrays instead of being pipe-delimited.

That's dirty, but there is currently no other option for that. I created an [Idea to change that](/docs/salesforce/marketing-cloud-personalization/ideas#full-mcp-catalog-export).
