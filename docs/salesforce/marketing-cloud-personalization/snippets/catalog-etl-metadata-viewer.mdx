---
id: catalog-etl-metadata-viewer
...
image: img/og/og-image-sf-mcp-catalog-etl-metadata-viewer.png
tags: [Marketing Cloud Personalization, Personalisation, TypeScript, Data Management, Snippet]
...
:product[Marketing Cloud Personalization] can pull [product, content and related catalog data](/docs/salesforce/marketing-cloud-personalization/catalog-architecture.mdx) from multiple sources and does it continuously. That's perfect when all works great, but debugging features are minimal once you see some data discrepancies.
...
A Serverside Template that leverages built-in Simulated Payload Preview available in Template Editor to display hidden changelog for ETL updates: attribute's current value, timeframe it was first introduced and first CSV that provided it to :product[MCP].
...
Additionally, it's a great way to showcase some [:product[MCP] Serverside Code](/docs/salesforce/marketing-cloud-personalization/serverside-code-basics.mdx) approaches that can be useful for building standard campaign templates.
...
While this solution can benefit [:product[MCP] Catalog Object](/docs/salesforce/marketing-cloud-personalization/catalog-architecture.mdx) debugging, there are a few significant limitations.
...
While `context.event.fields.item` is perfect for the use case described here, don't try to use it in standard Campaigns. Unfortunately, it is filled in only in the Simulation Preview and will not be available when deployed to a website. However, for production purposes you can use [`context.event` methods](/docs/salesforce/marketing-cloud-personalization/serverside-code-context.mdx#event-methods) to get the same information dynamically from the item viewed by user.
