...
tags: [Salesforce, Marketing Cloud Engagement, Marketing Automation, Best Practice]
...
Marketing Cloud is a complex tool within a complex Salesforce ecosystem focused on solving complex business needs. But how you operate and develop MCE should be as straightforward as possible. It will make your solution robust, agile and easily understandable.
...
When building in Journey Builder, it is easy to nest Decision Splits to send various communication based on some criteria, but this approach will quickly become [complicated](#complex-is-better-than-complicated) and (with how MCE renders multiple splits) [ugly](#beautiful-is-better-than-ugly).
...
In terms of code it means proper [debugging and error handling](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx). Especially [error logging](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#error-logging) is good idea to keep up with the potential issues in matured Marketing Cloud solution. You can also protect your solution by creating last resort checks in messaging with the `RaiseError` function.
...
There are multiple examples of this risk. SSJS is not-fully-supported ES3 limited by the .NET backend, so you never know which feature will work. SQL is a partial SQL Server 2016 with different behaviour in Script Activity and Query Studio. There are inconsistent approaches within the same solution (for example [shared folder permissions](/sites/ideas/salesforce/marketing-cloud-engagement-ideas/#content-builder-shared-folders-permissions)). Unexpected bugs (like incorrect rendering of [Behavioral Triggers](/sites/ideas/salesforce/marketing-cloud-engagement-ideas/#behavioral-trigger-content-block-rendering-on-yahoo-aol-windows-outlook)). Temporary issues that change the SSJS function responses for few days.
...
There is a lot of contradictory recommendations above. It's intentional. Zen of Marketing Cloud is not a list of rigid rules - it would be impossible with the number of differences between Marketing Cloud customers. Instead, it's a set of flexible guidelines. Treat them as friendly best practices, not absolute rules and adapt them to improve your work.
