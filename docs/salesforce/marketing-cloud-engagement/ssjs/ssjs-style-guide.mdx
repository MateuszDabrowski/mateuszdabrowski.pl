...
description: Script with style. Readable code is a few rules & best practices away. Naming, letter case, indentation, semicolon and comment rules for SSJS in Marketing Cloud Engagement.
image: img/og/og-image-sf-mce-ssjs-style-guide.png
tags: [Marketing Cloud Engagement, SSJS, Style Guide, Best Practice]
...
First things first: this Salesforce :product[Marketing Cloud Engagement] SSJS style guide is highly subjective. You may use it as it is, implement only some parts of it, or ignore it altogether. There are only two rules that I believe are a must-have:
...
For all of those I'm using `camelCase`. It's the most popular convetion, so it will make it easier to work with other programmers, leverage code snippets and onboard new-to-SSJS people to :product[MCE] development.
...
Not only it allows to differentiate JavaScript native and :product[MCE] custom elements, but it also follows the official SSJS documentation (and therefore most of the snippets you might find).
...
- Don't end complex statements with a semicolon. Think [`if`](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-if-and-switch.mdx#if-statements) statements, [loops](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-loops.mdx), `functions`, `try/catch` blocks.
- But... you need a semicolon after the [`do/while` loop](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-loops.mdx#do-while). Because JavaScript (and yeah, it is different from [`while` loop](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-loops.mdx#while) when considering the statement blocks). Oh, and you will need to add them in your [`for` loop](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-loops.mdx#for) iteration rules.
...
4. :product[MCE] ecosystem already leverages it in :product[Interaction Studio]{as-written}.
...
- [:product[MCE] SQL Style Guide](/docs/salesforce/marketing-cloud-engagement/sql/sql-style-guide.mdx)