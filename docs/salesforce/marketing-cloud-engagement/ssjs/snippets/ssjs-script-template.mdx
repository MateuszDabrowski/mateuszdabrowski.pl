...
image: img/og/og-image-sf-mce-ssjs-script-template.png
tags: [Salesforce, Marketing Cloud Engagement, Marketing Automation, SSJS, Snippet]
---
...
Read more on that approach in my [SSJS Style Guide](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-style-guide.mdx#1-table-of-contents-comment).
...
There is also a second reason - defining them before the try/catch block, even without any value, will allow accessing its value after that block. Read more about that in [Debugging & Error Handling](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#try-to-catch-the-error).
...
- controlling the script's behaviour (like configurational ones or the `debugging` variable described in detail [here](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#debugging-variable))
...
To learn more about them, check out detailed descriptions of [`debugValue`](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#debugging-variable) and [`handleError`](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#error-handler-function) functions.

:::note You Should Know

If the script will be used in [JSON Code Resource backend](/docs/salesforce/marketing-cloud-engagement/config/code-resource.mdx) I modify those two functions a bit by:
...
In many cross-cloud scenarios I also use [AMPScript runner](/docs/salesforce/marketing-cloud-engagement/ssjs/snippets/ampscript-in-ssjs.mdx), but as it is more use case specific solution, I don't keep it in base template.

I prefix all functions with [documentation comments](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-style-guide.mdx#3-documentation-comments).
...
The final part of the template is where I'm adding the main part of the script. It's wrapped in try/catch (more on that [here](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#try-to-catch-the-error)) and - if the script is long - split into [sections](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-style-guide.mdx#2-section-comments).