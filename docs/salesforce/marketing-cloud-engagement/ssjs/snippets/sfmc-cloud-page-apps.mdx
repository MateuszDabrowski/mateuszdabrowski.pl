...
description: Create micro applications tailored to your organisation needs using only Salesforce Marketing Cloud Engagement features.
image: img/og/og-image-sf-mce-solve-with-cloud-page-apps.png
tags: [Marketing Cloud Engagement, SSJS, AMPScript, Snippet, Cloud Page, App, API]
...
<LeadText content="Create micro applications tailored to your organisation needs using only Salesforce Marketing Cloud Engagement features." />

## Cloud Page Apps: what and why

### When out-of-the-box features are not enough

Salesforce :product[MCE] offers a multitude of options. With all the Studios, Builders, and other Salesforce-ecosystem cross-sells, you can deliver nearly anything. Nearly.
...
Depending on the use case and company resources, you can overcome such challenges in two ways. By shopping at :product[AppExchange]{as-written} - Salesforce App Store with 3rd Party solutions - or by building a custom application connected to :product[MCE] with APIs (for example, leveraging yet another Salesforce product - Heroku).

:product[AppExchange]{as-written} might be a great idea if you can find a product solving your problem, but it will add a recurring cost to your budget. On the other hand, a fully-fledged custom application might deliver precisely what you need but create considerable up-front cost (in either money for external developers or time for an in-house team) and require maintenance.

There is, however, the third path. One that can be perfect if you have a necessary but relatively small requirement for internally used power-up and would instead go lean on both budget and time — :product[MCE] micro application.

### Out-of-the-box approach to out-of-the-box feature

:product[Marketing Cloud Engagement] micro application offers AppExchange-style deep integration with :product[MCE] platform and solution flexibility of custom development. In short, it is an application hosted on a Cloud Page and available via the main menu of :product[MCE]. A Cloud Page App.

The good? You can use :product[MCE] programmatic languages (SSJS and AMPScript) with all their nifty built-in functions to control the assets and behaviours of the :product[MCE] platform and connected Sales & Service Clouds. You can also leverage the APIs to gather even more power and pull external logic and data. All this without having to care about hosting, scaling and integration. Available directly from the :product[MCE] menu. And cost? Just a super message for loading a Cloud Page.
...
- :product[MCE] Asset Viewer that allows you to quickly locate, for example, a Data Extension or Content asset.
...
- Deliverability Dashboard pulling data from :product[MCE] and external deliverability monitoring and alerting tools.
- Documentation or FAQ related to your company processes and best practices that everyone should follow when building customer experience through :product[MCE].
- Templated Journey creator to deploy tested flows for most used campaigns in a single click.

Anything related to :product[MCE] that you wish could be a bit more automated or easy can be a good idea for a micro application approach.
...
3. Go to :product[MCE] Setup -> Apps -> Installed Packages and create a New Package.
4. Add a `Marketing Cloud App` Component to the Package and paste the Cloud Page URL in both Endpoint fields and Save.

That's all. You just created your first :product[Marketing Cloud Engagement] micro application. Refresh :product[MCE] to see it available in the main menu under the :product[AppExchange]{as-written} icon. Now it's time to build up your solution to solve the business need.

### A Good application is a secure application

Your micro application is now available from the :product[MCE] menu. But unfortunately, it can also be accessed from any other place, as it is still just a simple Cloud Page. It is, of course, not ideal. You don't want random people to access your API-fueled solution.
...
After you create your Installed Package, three base URI's are available - Authentication, REST and SOAP. After the `https://`, all of them have a common alphanumeric string with hyphens unique to your :product[MCE]. I copy only this part to build any endpoint necessary for the application.
...
With the above pre-work completed, you can use SSJS or AMPScript on your Cloud Page to redirect to the `v2/authorize` endpoint of your Authentication Base URI before loading. It will require any visitor to log into your :product[MCE] to load your micro application. What's even better, it will automatically load it for users already logged in (micro Single Sign-On).
...
This way, you just added a micro Single Sign-On feature that is as secure as your :product[MCE]. And that's not all! As now only your :product[MCE] users can access the application, you can leverage the Installed Package Access tab to control who will see your solution in the main menu.

Now you have a secure, SSO-enabled micro application deeply integrated with :product[MCE] ready to solve your business needs and power up your Marketing Team.
...
The session id is the credential of your Cloud Page App: anyone who knows a live one can use the app as its owner. Mint it with `GUID()` after the token exchange and never reuse the OAuth `state` for it. Whoever starts a sign-in knows its `state`, and could send the authorize link to a colleague who is logged in to :product[MCE], then use the colleague's session.
...
This single-page template signs the visitor in on every load. Any reload done within the app (Reload/Redirect/Post via Form) should point back to the plain Cloud Page URL: the visitor is still logged in to :product[MCE], so the sign-in completes without a login screen. Do not pass `state` along to stay signed in, because whoever starts a sign-in knows it.
...
You can also leverage the session to lookup the token for API calls: `%%=Lookup(@authDE,'token','session',@session)=%%`. A session stays valid for 18 minutes, the life of its token; after that the page signs the visitor in again, without a login screen when they are still logged in to :product[MCE].
...
3. __Installed Package__
    ...
    4. :product[Marketing Cloud]{of=mce as-written} App Component.
...
4. __Menu Unlock__
    1. Refresh tab with :product[MCE].
    2. On the main menu pane of :product[MCE], hover over :product[AppExchange]{as-written} to see your application.
...
1. Webinar recording on [Cloud Page Apps](/docs/salesforce/marketing-cloud-engagement/config/webinars/mce-cloud-page-apps.mdx).
2. Webinar recording on [Architecting Web Solutions in :product[MCE]](/docs/salesforce/marketing-cloud-engagement/config/webinars/mce-architecting-web-solutions.mdx).
3. Article on [Power of :product[MCE] Code Resources](/docs/salesforce/marketing-cloud-engagement/config/code-resource.mdx).
4. Article on [:product[MCE] :product[AppExchange]{as-written} Solutions](/docs/salesforce/marketing-cloud-engagement/config/appexchange-solutions.mdx).
