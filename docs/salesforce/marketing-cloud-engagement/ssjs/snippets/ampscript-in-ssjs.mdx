...
tags: [Salesforce, Marketing Cloud Engagement, Marketing Automation, SSJS, AMPScript, Snippet]
---
...
As we are using `Variable.GetValue` here, you will need to import Salesforce Marketing Cloud Engagement SSJS Core library at the top of your script.
...
Be sure only to use this approach if there is no equivalent SSJS solution, as mixing languages will [impact performance](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-vs-ampscript-performance.mdx). It is crucial for code in the messages that MCE will execute on send time.
