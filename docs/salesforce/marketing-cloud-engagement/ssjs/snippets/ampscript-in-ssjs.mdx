...
image: img/og/og-image-sf-mce-ampscript-in-ssjs.png
tags: [Marketing Cloud Engagement, SSJS, AMPScript, Snippet]
...
SSJS language has many features missing from AMPScript - [try/catch blocks](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#try-to-catch-the-error), arrays and objects to mention the most used ones. But some things are available only in AMPScript.
...
As we are using `Variable.GetValue` here, you will need to import Salesforce :product[Marketing Cloud Engagement] SSJS Core library at the top of your script.
...
Be sure only to use this approach if there is no equivalent SSJS solution, as mixing languages will [impact performance](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-vs-ampscript-performance.mdx). It is crucial for code in the messages that :product[MCE] will execute on send time.
...
To use any Salesforce Object functions, you will need to have Marketing Cloud Connect implemented to tie in your :product[MCE] and Sales or Service Cloud.
...
It is even more useful when you have variability in what you want to create. For example, let's consider a use case where you want to create Campaigns in Salesforce from :product[MCE], but only some of your campaigns have an End Date known from the beginning.
