...
Be sure only to use this approach if there is no equivalent SSJS solution, as mixing languages will [impact performance](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-vs-ampscript-performance.mdx). It is crucial for code in the messages that Marketing Cloud will execute on send time.
