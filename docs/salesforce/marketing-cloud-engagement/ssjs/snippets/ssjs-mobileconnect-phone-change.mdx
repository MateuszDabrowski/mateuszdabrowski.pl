...
image: img/og/og-image-sf-mce-ssjs-mobileconnect-phone-change.png
tags: [Salesforce, Marketing Cloud Engagement, Marketing Automation, SSJS, SMS]
---
...
There is a lot to like about the Salesforce Marketing Cloud Engagement Mobile Connect, but Contact management is not the one.

One of the problems is managing the Mobile Phone Number. It is a crucial data point because we are using it to send the SMS and because it is the key value for many of the [MobileConnect Data Views](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx#mobileconnect-data-views).
...
The official approach is to import the contact with the new Mobile Number. It, however, won't overwrite the previous phone. It will merely add a new one to the record with higher [priority](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx#_mobileaddress). Not perfect.

So what you can do? Leverage the [`_MobileAddress` Data View](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx#_mobileaddress) along with simple SSJS snippet and force MCE to do what you want it to do.

:::note You Should Know

The solution shown below is working perfectly fine right now. However, remember that Salesforce considers [`_MobileAddress` Data View](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx#_mobileaddress) as legacy and unsupported, so be sure to make the first run on test data to check whether it is still working correctly.
...
That's it. Running this snippet - even in a Cloud Page or [Code Resource](/docs/salesforce/marketing-cloud-engagement/config/code-resource.mdx) - will change the Mobile Number assigned to the Contact. Just be sure to publish the page and load it. The Web Studio Preview won't work.
...
You are not limited to Mobile Number change in MobileConnect with this solution. You can leverage [`_MobileAddress` Data View](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx#_mobileaddress) to even merge MobileConnect contacts.
...
You can also leverage the snippet within an Automation Studio using Data Extension as a batch data source and Script Activity for processing. Pack the snippet within a [`for` loop](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-loops.mdx) and make a more significant change to your MobileConnect data.

:::note You Should Know

Technically you can use this solution to modify any field of [`_MobileAddress`](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx#_mobileaddress). Due to the unsupported status of this Data View, I would recommend using Contact Builder whenever it is enough for your use case. It allows you to change everything but the Mobile Number and Contact ID.
