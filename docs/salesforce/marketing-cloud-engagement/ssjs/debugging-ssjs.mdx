...
description: To catch, or not to catch, that is debugging. Where to test SSJS, how to trace a 500 error, try/catch, debug output and error logging.
image: img/og/og-image-sf-mce-ssjs-debugging-and-error-handling.png
tags: [Marketing Cloud Engagement, SSJS, Debugging, Cloud Page, Email]
...
When you start working with programmatic languages in Salesforce :product[Marketing Cloud Engagement], you will quickly become close friends with the Error 500 page. And if you beat it and go forward with the code and official documentation, you will fall in love with unexpected errors in functions and API responses. How to survive those?

----

## SSJS Testing Ground

Before diving deep into errors, let's talk about where to write the SSJS. The language is useful in many places within the :product[MCE] ecosystem. Among else in:
...
The first one can be done directly in :product[MCE] but is a bit of a brute-force approach. You block half of your code from running by enclosing it in a multiline comment (`/* here goes the code */`) and check again. If it works, you have half of the SSJS validated (from the 500 error perspective). If not, you split the remaining code in half and comment it out. Rinse, repeat. This way, you can quickly find the few lines that are the source of the issue and focus on validating them.
...
Whenever you create a script for the long-term, it is good to keep your debugging `Write` functions. There might be new requirements. Data sources change. :product[MCE] too. And sometimes, those things might break your code.
...
You are not limited to strings with the `throw`. You can also pass objects to mimic the :product[MCE] errors. It will make error handling much easier:
...
The easiest way to log your errors is to leverage :product[MCE] Data Extensions.
...
Most of the :product[MCE] Errors are objects with two keys: `message` and `description`. Putting those elements into separate columns of the Data Extension make it easier to read the log.
