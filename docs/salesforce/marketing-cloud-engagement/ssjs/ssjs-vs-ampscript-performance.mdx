...
image: img/og/og-image-sf-mce-ssjs-vs-ampscript-performance.png
tags: [Marketing Cloud Engagement, SSJS, AMPScript, Performance]
...
There is much knowledge shared on blogs, social networks, and Stack Exchange in the Salesforce :product[Marketing Cloud Engagement] world. Frequently you will see some strong opinions on which scripting approach is best.
...
The testing toolset is minimal by the nature of SSJS and AMPScript. I'm using a Cloud Page and `new Date().getTime();` to capture the execution's beginning and end. Within this scope, I run a [loop](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-loops.mdx) with the code I want to test. Finally, I divide the total time by the number of loop iterations to get an average run time.

This approach means that the millisecond values I provide are in no way the exact times you can use to calculate the speed at scale. But they should be standardised enough to allow for meaningful comparison between various scenarios. The actual times also change a bit when we run the test multiple times, as the strain on :product[MCE] servers at the very moment of execution also has an impact. Focus less on exact milliseconds count and more on relations between the values.
...
For the final scenario I decided to break out of AMPScript context within an AMPScript loop, as this is popular way for creating dynamic content in :product[MCE].
...
What should you use when you want to change a string to Proper Case? As there is no single function in SSJS for that, I compared two pure SSJS approaches (`split`/`join` & `replace` with RegEx) with [AMPScript in SSJS](/docs/salesforce/marketing-cloud-engagement/ssjs/snippets/ampscript-in-ssjs.mdx) and (nearly) pure AMPScript.
...
3. Although [AMPScript in SSJS](/docs/salesforce/marketing-cloud-engagement/ssjs/snippets/ampscript-in-ssjs.mdx) is doing levels-of-magnitude worse than pure AMPScript, it will be a better choice then pure SSJS for long sentences (10+ words).
...
2. Pure AMPScript execution time still is better, but due to constant time in both languages and much faster SSJS speed, there is no longer any reason to use [AMPScript in SSJS](/docs/salesforce/marketing-cloud-engagement/ssjs/snippets/ampscript-in-ssjs.mdx) or [break out of SSJS code block](#code-block-breaking).