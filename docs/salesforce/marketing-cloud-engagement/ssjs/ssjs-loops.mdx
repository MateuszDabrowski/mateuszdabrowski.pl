...
image: img/og/og-image-sf-mce-ssjs-loops.png
...
As ES6 [for...of](/docs/js/js-loops.mdx#for-of) loop is not available in SSJS, you will use this one in most scenarios.
...
In SSJS however, the ES6 [for...of](/docs/js/js-loops.mdx#for-of) loop is not available. `Object.keys()` or `Object.entries()` also does not exist. This makes the `for...in` the only solution for iterating over objects.
