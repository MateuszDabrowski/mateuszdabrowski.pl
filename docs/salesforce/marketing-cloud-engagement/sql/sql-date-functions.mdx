...
Salesforce :product[Marketing Cloud Engagement] SQL implementation does not support user-defined functions. There are, however, multiple built-in functions that are useful on a day-to-day basis when working with queries.
...
`GETDATE()` gets server time. For :product[MCE], it means Central Standard Time (UTC-6) without changes between standard and daylight savings time. It cannot be changed, even with timezone and culture settings in Setup.
...
However, you can limit its output to either date or time part by using [`CONVERT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx#cast--convert) or [`FORMAT`](#format-with-dates) Functions.
...
For 99% of :product[MCE] needs, you will be fine with `YEAR`, `MONTH`, `DAY`, `HOUR` and `MINUTE`. Using only those with the full name will make your queries readable and straightforward for everyone.
...
`DATETIMEFROMEPARTS` is a sibling function that requires more data but outputs a fully-fledged datetime value. On top of the year, month and day, add an hour, minute, seconds and milliseconds. You won't be able to see the time part in the :product[MCE] UI (Contact Builder or Email Studio), but it will be available for calculations.
...
There is also a third function: `TIMEFROMEPARTS`. It requires an hour, minute, seconds, fraction and precision as arguments. Its usefulness in :product[MCE] is close to none.
...
`DATEADD` is especially useful when used with the [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) statement, as it allows to filter records dynamically:
...
If the date field is nullable, be sure to use [`COALESCE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx#coalesce) or add a [`NULL` checking condition](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx#null).
...
You may use it in the [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) statement:
...
You may use it in the [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) statement:
...
It is crucial for creating correct comparisons within [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx).
...
There is a limit to how big the difference can be. If you go down to the `SECOND` [date part](#date-parts), the query's maximum time difference is 68 years, 19 days, 3 hours, 14 minutes and 7 seconds. It shouldn't be an issue in :product[MCE] for most use cases.
...
`AT TIME ZONE` always outputs `DATETIMEOFFSET` data type. If you need it in another type/format, use [`CONVERT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx#convert) or [`FORMAT`](#format-with-dates).

:::

### Add offset with AT TIME ZONE

When you work with timezones, you want to have offset for the datetime fields you are using. Unfortunately, in most cases, it is missing. Neither :product[MCE] system timestamps nor the popular [`GETDATE`](#getdate--getutcdate) function provides offset information.
...
In the above snippet, we [`CONVERT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx#convert) the dates to datetime without offset. Then the `AT TIME ZONE` function adds the offset and calculates the Daylight Saving Time offset change for you.
...
As astonishing as it might seem, taking Daylight Saving Time into consideration is a problem when working with :product[Marketing Cloud Engagement] system dates (including the [`GETDATE`](#getdate--getutcdate)).

:product[MCE] uses Central Standard Time (UTC-6) __without__ changes between standard and daylight savings time. It means that `AT TIME ZONE`'s excellent feature is working against us.
...
2. `TODATETIMEOFFSET`. It is a simplified version of the `AT TIME ZONE` that adds arbitrary offset without any logic on Daylight Saving Time. Just as we would like for the :product[MCE] system dates.
...
Of course, for :product[MCE] system dates, we still need either of the [workarounds](#at-time-zone-vs-todatetimeoffset):
...
It is a perfect tool to manipulate any dates (or date-like strings with the help of [conversion](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx)) available in your Data Extensions. For personalization. For comparison between system and imported dates outside of the query. For preparing data to export it to an external system with specific format requirements.
...
The last line outputs `Freitag, 7. Februar 2020`. Neat for running it with [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#conditional-values-with-case) to change your short date into an extended date personalized on user language.
...
As highlighted in the table header - the examples are for the :product[MCE] default US culture formating. If you add a specific culture code as a third parameter, the outcome will be different, as various countries have different defaults of elements order and separator style.
