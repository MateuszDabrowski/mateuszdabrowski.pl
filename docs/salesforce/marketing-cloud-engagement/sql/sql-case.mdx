...
title: MCE SQL Case
sidebar_label: Case
description: Empower your Marketing Cloud Engagement queries with conditional SQL CASE logic.
image: img/og/og-image-sf-mce-sql-case.png
tags: [Salesforce, Marketing Cloud Engagement, Marketing Automation, SQL]
---

import { LeadText } from '/src/components/LeadText.js';

<LeadText content='Empower your Marketing Cloud Engagement queries with conditional SQL CASE logic.' />

## Conditional values with CASE

`CASE` statement evaluates a list of conditions and returns one of the multiple possible result expressions. In Marketing Cloud Engagement SQL CASE statements, it is especially useful when you want to select a column and modify the values within it. Perfect for translating numeric codes to descriptive ones for personalisation purposes. Excellent for standardising data (change those `1`, `true` and `on` values of opt-in to a single format).
...
Feel free also to leverage [aliases](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx#aliasing-with-as) to manipulate the column name at the same time:
...
In the `SELECT` statement, the `LIKE` operator is handy for fixing data coming from the free text fields. To learn more about the possibilities, check out the [`LIKE` guide](/docs/salesforce/marketing-cloud-engagement/sql/sql-like.mdx).
...
The `CASE` statement is not only useful in a [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) part of the query. You can use it also within the [`WHERE` to apply conditional filtering](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#case-operator).