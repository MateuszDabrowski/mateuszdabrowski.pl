...
image: img/og/og-image-sf-mce-sql-from.png
...
The [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) statement alone won't do much. In most cases, once you have selected data you want to work with, you also need to pick the table (Data Extension or System Data View) `FROM` which the query will take the required columns. The code for this is simple:
...
There are, however, two cases specific to Salesforce Marketing Cloud Engagement that adds a twist. System Data Views and Enterprise Data Extensions.
