---
id: sql-numeric-functions
title: Marketing Cloud Engagement SQL Numeric Functions
...
image: img/og/og-image-sf-mce-sql-numeric-functions.png
...
How to find the culprit? With `MAX` and [`LEN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx#len) functions:
...
The above example might not look like much value, but with the help of [`DATEDIFF`](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx#datediff) and [`CONVERT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx#convert) functions you can make it much more interesting:
...
However, `AVG` shows its true power with [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) filtering:
...
`AVG` function is also beneficial as a [part of an aggregate query](/docs/salesforce/marketing-cloud-engagement/sql/sql-aggregate-functions.mdx#avg).
...
It is helpful nearly only as a [part of an aggregate query](/docs/salesforce/marketing-cloud-engagement/sql/sql-aggregate-functions.mdx#sum).
...
It is a perfect tool to manipulate any numbers (or numeric strings with the help of [conversion](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx)) available in your Data Extensions.
...
As you can see, we had to use the [`CONVERT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx#cast--convert) function with the `BIGINT` parameter, as the MobileNumber is a Text field. Thanks to conversion, we can use numeric functions to format it quickly. It could get even better with [`LIKE` in `CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#like-in-case) used to create different formats depending on the country code.
