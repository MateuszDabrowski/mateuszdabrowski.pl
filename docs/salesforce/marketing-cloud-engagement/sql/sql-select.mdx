---
id: sql-select
title: MCE SQL Select
sidebar_label: Select
description: Selected information on the SELECT statement in Marketing Cloud Engagement SQL.
image: img/og/og-image-sf-mce-sql-select.png
...
<LeadText content='Selected information on the SELECT statement in Marketing Cloud Engagement SQL' />
...
The simplest version of this statement is `SELECT *`, which targets all available columns from the source. I do not recommend this approach, as it is terrible for your query's readability and performance. As :product[MCE] caches the source Data Extension column names when you save SQL Query Activity, you won't have added value of flexible selection. You will still have to edit and save the activity. Also, [Query Studio](/docs/salesforce/marketing-cloud-engagement/config/appexchange-solutions.mdx#query-studio) does not support this shortcut. Don't use `*`.
...
There are also some exceptional cases related to selected [System Data Views](/docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx) in :product[MCE], where you need to use an underscore prefix before a name (for example, when taking Subscriber ID from Enterprise Attributes):
...
It becomes mighty with the [SQL String functions](/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx).
...
The best solution is to use the appropriate field type for numbers. However, if for whatever reason you cannot do it, leverage [`CONVERT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx#convert):
...
For example, it will be needed when you `ORDER BY` [`COUNT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-aggregate-functions.mdx#count) within [`GROUP BY`](/docs/salesforce/marketing-cloud-engagement/sql/sql-aggregate-functions.mdx#group-by).

`ORDER BY` will also sort dates in :product[MCE] alphabetically using month (start with `Apr`, follow up with `Aug` and so on). You can fix it by using [`CONVERT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx#convert) to `DATE` (if you to order only by date) or `DATETIME2` (if time is important for ordering as well), just as in previous example.

:::

`ORDER BY` can operate not only on the selected column. You already saw above that you can use expression (even [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#conditional-values-with-case)!) to calculate the sorting value, but it's much more flexible.
...
I'm covering this topic in-depth in [`JOIN` statement documentation](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#table-name-prefix).

## Flow control

Despite official [:product[MCE] SQL documentation](https://help.salesforce.com/articleView?id=mc_as_sql_reference.htm&type=5) stating that it supports the `IF` constructs, I could not make it work.

## Variables & Functions

Unfortunately, :product[MCE] SQL does not support variables.

Fortunately, it does support (selected) functions. Learn more about them in :product[MCE] SQL Functions documentation pages related to [Strings](/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx), [Dates](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx), [Numbers](/docs/salesforce/marketing-cloud-engagement/sql/sql-numeric-functions.mdx), [Conversion](/docs/salesforce/marketing-cloud-engagement/sql/sql-conversion-functions.mdx), [Aggregation](/docs/salesforce/marketing-cloud-engagement/sql/sql-aggregate-functions.mdx), [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx) and [`LIKE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-like.mdx).