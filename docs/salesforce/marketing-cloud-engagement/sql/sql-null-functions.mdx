...
Because `NULL` is unknown, if you try to do any standard operation on the NULL (/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx#concat) it with a string), it will return unknown (`NULL`):
...
The fun doesn't stop there - When you try to make any [comparision](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#basic-operators) with `NULL`, it will fail, because it is a check against an unknown value. Or, to be more specific, the comparison will evaluate to `FALSE`, impacting your output.
...
When you are building a boolean logic with `AND`/`OR` operators (/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) or in [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx)) treat `NULL` as `FALSE`.
