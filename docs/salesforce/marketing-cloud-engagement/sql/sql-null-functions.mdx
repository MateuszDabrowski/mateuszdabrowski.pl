...
title: MCE SQL NULL Functions
sidebar_label: Null Functions
description: IS NULL, or IS NOT NULL, that is the question. Ways of working with NULL values in Marketing Cloud Engagement (MCE) SQL.
image: img/og/og-image-sf-mce-sql-null-functions.png
tags: [Salesforce, Marketing Cloud Engagement, Marketing Automation, SQL]
---

import { LeadText } from '/src/components/LeadText.js';

<LeadText content='IS NULL, or IS NOT NULL, that is the question. Ways of working with NULL values in Marketing Cloud Engagement (MCE) SQL.' />
...
Because `NULL` is unknown, if you try to do any standard operation on the NULL (like adding an integer, or [concatenating](/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx#concat) it with a string), it will return unknown (`NULL`):
...
The fun doesn't stop there - When you try to make any [comparison](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#basic-operators) with `NULL`, it will fail, because it is a check against an unknown value. Or, to be more specific, the comparison will evaluate to `FALSE`, impacting your output.
...
In the example above, all three comparisons in the [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) statement will return `FALSE` for `NULL`s and the query will not output any rows.
...
`NULL` gets even more problematic when you want to work with [`IN`/`NOT IN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#in-shorthand) or [`BETWEEN`/`NOT IN BETWEEN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#between-shorthand).
...
When you are building a boolean logic with `AND`/`OR` operators (be it in [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) or in [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx)) treat `NULL` as `FALSE`.
...
[Aggregate functions](/docs/salesforce/marketing-cloud-engagement/sql/sql-aggregate-functions.mdx) just ignore `NULL` values. Calculating average? `NULL` will not be taken into consideration at all - which can scew your output a lot.

The only exception is [`COUNT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-aggregate-functions.mdx#count) that has specific `*` argument for counting rows with `NULL`.
...
However, there are other methods - dedicated shorthands of the universally helpful [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx) statement. Let's check them.
...
I personally find the standard [`IIF`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#iif-shorthand) shorthand much more readable and user-friendly, despite being slightly longer than dedicated `NULLIF`:
