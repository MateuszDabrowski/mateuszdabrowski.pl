---
id: sql-debugging-value-length
title: MCE SQL Debugging Value Length
...
image: img/og/og-image-sf-mce-sql-debugging-value-length.png
tags: [Marketing Cloud Engagement, SQL, Snippet, Data Extensions, Debugging]
...
You can make quick manual check with a simple SQL query. Open your [Query Studio](/docs/salesforce/marketing-cloud-engagement/config/appexchange-solutions.mdx#query-studio) and copy-paste the below code:
...
This query will output one row of data with the current maximum length of the values that you have in your Synchronized Data Extension for the selected fields. It uses [`MAX`](/docs/salesforce/marketing-cloud-engagement/sql/sql-numeric-functions.mdx#min-and-max) function on the [`LEN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx#len) outcome to find the single maximum length available across all records. It has two great uses:
...
Of course, as mentioned above, the values might get longer in the future. But as :product[Marketing Cloud Engagement] allows you to change maximum field length up but not down, it might be a good idea to try as low as reasonable and leave space for growth.

### Debugging with MAX LEN

The second use of the above snippet is quick debugging when an error occurs. Execute it in [Query Studio](/docs/salesforce/marketing-cloud-engagement/config/appexchange-solutions.mdx#query-studio) and compare against the column lengths you set up in your Data Extension. If you see in Query Studio any value longer than the maximum you set up in Data Extension - you found a culprit.
...
The first field we [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) is the Unique Identifier (Contact ID / Subscriber Key or other based on the specific object you are validating). It is useful to quickly know which record(s) should you checked when some values lengths exceed the expectations.

After that, you can see the standard [`FROM`](/docs/salesforce/marketing-cloud-engagement/sql/sql-from.mdx) statement. You can, of course, [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) additional data sources.

Finally, there is a [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) statement. In it, we are checking whether at least one field on each record has a value longer than expected. To adapt it - change the number on the left to the length of the field you set up in the target Data Extension / Profile Attribute.
...
Right after this SQL Query Activity add Verification Activity and configure it to check the technical Data Extension mentioned above. If there is any row in it - it should stop the whole Automation and send an alert email to the :product[MCE] administrator.
...
However, we already are coding the expected field length in the [`FROM`](/docs/salesforce/marketing-cloud-engagement/sql/sql-from.mdx) part of the query. Why not use it to make life easier?

Let's change the `LEN(c.Id) AS [Subscriber Key],` in the [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) statement part to something a bit more interesting:
...
As you can see, we changed a simple line to a more complex [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#conditional-values-with-case) statement. There is a good reason for it. Thanks to it, the query will evaluate each field against the expected length and in the Value Length Data Extension display information only for the problematic values.
...
We can go even further with this approach and add a bit more data to the output for those problematic fields by making few calculations in `THEN` part of the [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#conditional-values-with-case) statement.
