...
tags: [Marketing Cloud Engagement, SQL, Snippet, Data Management, Debugging, Integration]
...
Salesforce :product[Marketing Cloud Engagement] is a powerful and flexible tool, but that flexibility can easily lead to messy data if not managed properly. The All Contacts in Contact Builder is a critical component of :product[MCE], as it serves as the central repository for all cross-channel subscriber information. However, over time, this list can become cluttered with invalid or test records, which can lead to increased costs and reduced performance.

This can be a problem for several reasons:
1. **Cost Implications**: The number of contacts in your All Contacts Data list is one of the [:product[Marketing Cloud Engagement] Licence Limits](/docs/salesforce/marketing-cloud-engagement/config/licence-limits.mdx). If you have a large number of invalid or test records, you may be paying for contacts that are not actually engaged with your marketing efforts. And never will be.
...
In this article, I will walk through a step-by-step guide to identify and clean up issues within the All Contacts using SQL queries and :product[MCE] UI investigation. By following these steps, you can ensure that your :product[MCE] instance remains efficient and cost-effective.
...
- Access to [Query Studio](/docs/salesforce/marketing-cloud-engagement/config/appexchange-solutions.mdx) (free SQL editor for :product[MCE]) or at least Automation Studio to run Query Activities.
...
- At least basic understanding of SQL and how to write queries in Salesforce :product[Marketing Cloud Engagement]. If you are new to SQL or not sure on some specific element, I recommend checking out my [:product[MCE] SQL Basics](/docs/salesforce/marketing-cloud-engagement/sql/sql-basics.mdx) article before proceeding.

## Step 1: Access All Contacts List

The bad news is that you cannot directly query All Contacts list in Salesforce :product[Marketing Cloud Engagement].
...
2. If you **don't have Mobile Connect** licence you will need to stitch that information from two sources:
    1. You can access all your email channel Contacts via [`_Subscribers` System Data View](/docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx#_subscribers).
...
My favourite place to start is to look at the Subscriber Key formats. Subscriber Key is the unique identifier for each contact in Salesforce :product[Marketing Cloud Engagement], and it can be in various formats depending on how the contact was created or imported.
...
1. If the number is matching and makes sense volume-wise - all good here.
2. If the number is matching, but is very high - you might check whether you truly need all those Leads in your :product[MCE] instance, and consider [filtering out](/docs/salesforce/marketing-cloud-engagement/config/mcc-integration-patterns.mdx#manage-the-data) those that do not support your marketing use cases.
...
Special points here for checking how are you managing Lead-to-Contact transition in Salesforce CRM and how it should be reflected in your :product[MCE] instance. You can open the following query to check how the post-conversion clean-up of Leads look like in your All Contacts list:
...
A simple way to manage it is to [add a checkbox on your User object](/docs/salesforce/marketing-cloud-engagement/config/mcc-integration-patterns.mdx#manage-the-data) to identify which (if any) Users should be synced to :product[MCE], and then update your integration to only sync those that are relevant for marketing purposes. Then you can clean up the rest of the records from your All Contacts list.
...
Proper Subscriber Keys are better and using email address for it in most cases means that someone is manually adding purchased or scraped email lists to :product[Marketing Cloud Engagement]. And in most of the civilised world this is not something you want to do.
...
<ButtonRow
 linkLeft='/docs/salesforce/marketing-cloud-engagement/config/contact-deletion/#delete-all-contacts-from-list-or-data-extension'
 textLeft="Manual Contact Deletion"
 newTabLeft='true'
 linkRight='/docs/salesforce/marketing-cloud-engagement/config/contact-deletion/#contact-deletion-with-automation-studio'
