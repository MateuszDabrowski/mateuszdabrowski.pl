---
id: sql-debugging-email-sends
title: MCE SQL Debugging Email Sends
sidebar_label: Debugging Email Sends
description: Make finding problems with email delivery in Salesforce Marketing Cloud Engagement (MCE) great again. SQL with Query Studio to the rescue.
image: img/og/og-image-sf-mce-sql-debugging-email-sends.png
...
<LeadText content='Make finding problems with email delivery in MCE great again. SQL with the Query Studio to the rescue.' />

## Problem with straightforward Email Send analysis

Deliverability issues, unfortunately, are a standard both during the :product[MCE] implementation phase as well as after the go-live.

The test you sent to yourself during the :product[MCE] implementation that never arrived in the inbox. The subscriber that asks where is his newsletter with the promo code.
...
My default is checking the sends from the last day. You can change it by modifying the [`DATEADD` function](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx#dateadd).

### Limit to a single subscriber

If you are looking for a problem related to only one specific subscriber, you can add `s.SubscriberKey = 'SubscriberKeyOfThatContact'` or `sub.EmailAddress = 'specific@email.address'` to the [`WHERE` statement](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#logical-operators). Or replace the current one not to be limited by the timeframe.

### Limit to a single domain

If you see problems with deliverability for a particular user, I highly recommend checking whether other users from the same domain also have such issues. Maybe it is related to corporate server policy? Add `sub.Domain = 'Recipients.Domain'` to the [`WHERE` statement](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#logical-operators). Or replace the current one not to be limited by the timeframe.

### Limit to a single email

If the problem is related to just one email asset, you can limit your outcomes by adding `job.EmailName = 'ProblematicEmailName'` to the [`WHERE` statement](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#logical-operators). Or replace the current one not to be limited by the timeframe.

### Limit to a single Journey

If you want to focus on a single Journey, you can limit your outcomes by adding `j.JourneyName = 'ProblematicJourneyName'` to the [`WHERE` statement](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#logical-operators). Or replace the current one not to be limited by the timeframe. You can also pinpoint it further by filtering on `j.JourneyVersion`.

### Limit to single engagement type

If you are interested only in bounces and do not need any other sends, add `b.IsUnique = 1` to the [`WHERE` statement](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#logical-operators).

You can use the same approach to output only opens or clicks by changing the prefix from `b` to the [alias](/docs/salesforce/marketing-cloud-engagement/sql/sql-join/#table-name-prefix) representing the needed [System Data View](/docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx).

## Email Send Debugging Step-by-Step

Now you know the :product[Marketing Cloud Engagement] Email Send Debugging query in and out. But how to work with the outcomes? And what are the next steps?

As they say, with great power comes great complexity. And that's definitely true for finding the root cause of the undelivered email. The deliverability by itself is already a beautiful mess, and then there are various features and options you can use directly in :product[MCE]. Let's dive in.
...
If System Data Views don't have the data or the DeliveryDate is not filled, the email hasn't reached the recipient's inbox. Time to investigate the [Send Log](/docs/salesforce/marketing-cloud-engagement/config/enhanced-send-log.mdx).
...
- __ErrorCode is filled__ » Check the [Error Code value meaning](/docs/salesforce/marketing-cloud-engagement/config/enhanced-send-log.mdx#send-log-errorcode-values) to find the root cause.
- __ErrorCode is not filled__ » The email was sent but not delivered to the recipient - you have deliverability issues. Check your configuration (From Address Management in :product[MCE] + SPF/DKIM/DMARC in DNS) and deliverability metrics (Salesforce offers Reputation Audit Test and Sender Rating as [features-on-demand](/docs/salesforce/marketing-cloud-engagement/config/features-on-demand.mdx#deliverability-features)).

### 3. Check All Subscribers List

If there is no trace of the send in System Data Views and Send Log, it means that :product[MCE] was not able to attempt sending. There are few possible reasons for that. Let's start with potential subscriber issues by searching for problematic Contact in Email Studio All Subscribers.

- __There is no subscriber__ » The most probable reason is subscriber block by :product[MCE] [List Detective](https://help.salesforce.com/s/articleView?id=000335444&type=1) that protects your lists from nasty emails. What frequently trips people is that it scrubs various group addresses like `admin@`, `info@` and [more](https://www.cameronrobert.com.au/sfmc/email-usernames-blocked-by-salesforce-marketing-cloud/).
...
If there are no errors within the Preview, the next potential culprit can be Exclusion or Suppression. There are quite a lot of them, and not all will be relevant for you, depending on the :product[MCE] configuration and method of sending, so be sure to check applicable ones from the Exclusion List, Exclusion Script, Suppression List, Auto-Suppression List.

### 6. Create Support Ticket

If, after all those checks, there is still no answer to the root cause - create a Support Ticket. Sometimes, it can be a master unsubscribe, server-related queue build-up or a bug on the :product[MCE] backend.
