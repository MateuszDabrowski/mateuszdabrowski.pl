---
id: sql-conversion-functions
title: MCE SQL Conversion Functions
...
image: img/og/og-image-sf-mce-sql-conversion-functions.png
...
Here, I will cover only selected conversion functions, that I find most useful for Marketing Automation purposes. They will help you change the value types to enable the use of type-based :product[MCE] User Interface elements and type-specific functions.

## CAST & CONVERT

In :product[Marketing Cloud Engagement], you can use two universal conversion functions - `CAST` and `CONVERT`.
...
You can also leverage [style codes](#convert-date-style-codes) to format your dates during conversion to string, without the need to use [`FORMAT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx#format-with-dates) function on top.
...
There are over 30 data types available for conversion, but there are only a few that are useful on a day-to-day basis in :product[MCE]:
...
| NVARCHAR | Variable-length string with Unicode support | Best option for most :product[MCE] string use cases |
...
There are also two more data types: `FLOAT` and `REAL`. Don't use them. Both are __Approximate__ Numeric Data Types and can lead to unpredicted behaviour, especially when used for equality-based conditions in [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx). `DECIMAL`/`NUMERIC` is the way to go.
...
Just as with data types - there are many style codes available. Here I will cover just the most popular ones (if you don't understand some formats codes, check [date formats guide](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx#custom-date-formats)):
...
For the second option, you have to find Date Style Codes within your string date and convert each separately. This can be easily done with [LEFT & RIGHT](/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx#left--right) functions.
...
With [LEFT & RIGHT](/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx#left--right) functions we split the datetime on the space, then [CONVERT](#convert) each part to finally sum it into a single date that is equal to our original string.

If you have specific parts of your date stored in separte fields, you can leverage [`DATETIMEFROMPARTS`](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx#datefromparts--datetimefromparts) function as well.
