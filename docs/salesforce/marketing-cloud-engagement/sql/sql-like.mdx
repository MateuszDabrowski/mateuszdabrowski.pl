...
title: MCE SQL Like
sidebar_label: Like
description: Get a LIKE and build more flexible Marketing Cloud Engagement SQL queries.
image: img/og/og-image-sf-mce-sql-like.png
...
Suppose you would like to select all contacts with active phone subscription. In that case, you shouldn't use [equality operator](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#equality-operators) (`WHERE ActiveSubscription = 'Phone'`), as it would return only contacts that have just this one subscription. Some of your customers might, however, have multiple (phone and tv or phone and internet plans). For SQL, a field containing `'Phone'` is not equal with the one having a value of `'Phone', Internet'`. `LIKE` operator is here to save the day.
