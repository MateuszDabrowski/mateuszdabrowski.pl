---
id: sql-where
title: Marketing Cloud Engagement SQL Where
sidebar_label: Where
description: Use WHERE to make your query lean and to the point. Less is more.
image: img/og/og-image-sf-mce-sql-where.png
...
With just [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) and [`FROM`](/docs/salesforce/marketing-cloud-engagement/sql/sql-from.mdx) statements, you can already start building your queries. With the addition of [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx), you can do much more than in the Salesforce Marketing Cloud Engagement UI. `WHERE` grants you even more power by allowing you to filter your queries' outcomes.
...
`WHERE` statement is heavily impacted by [`NULL`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx) values - be sure to check [dedicated guide](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx) on how to solve those issues.
...
It is also an excellent tool for database cleaning by finding incorrect values within automation. To learn more about the possibilities, check out the [`LIKE` guide](/docs/salesforce/marketing-cloud-engagement/sql/sql-like.mdx).
...
The `CASE` statement is not only useful in a [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) part of the query. You can use it also within the `WHERE`. There are two approaches to do this.
...
You can do better even with the [search version of `CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#search-version-of-case), as it allows you to use `AND`/`OR`/`IN` within the `CASE`:
...
To learn more about the possibilities and [shorthands](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#iif-shorthand), check out the [`CASE` guide](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx).