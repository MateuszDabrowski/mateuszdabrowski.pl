...
title: MCE SQL Aggregate Functions
...
image: img/og/og-image-sf-mce-sql-aggregate-functions.png
...
It limits the data you can [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) to either `GROUP BY` column(s) or aggregate functions.
...
You can quickly improve this query by limiting the records with [`TOP`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx#top) and sorting [`ORDER BY`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx#top-with-order-by) to leverage Pareto's principle and pick 20% of the records responsible for 80% of the results. In this case - 20% domains covering approximately 80% subscribers.
...
Let's check how the domain numbers are changing in years with the help of the [`DATEPART` function](/docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx#_subscribers).
...
`GROUP BY` will group [`NULL`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx#null) values as if they are equal.
...
For example, in seeing whether you have potential duplicate contacts. Instead of looking for all rows with `COUNT(*)`, we will change the function argument to the specific column to count only its [non-`NULL`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx#null) values.
...
- `COUNT(*)` will return the number of items, including [`NULL`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx#null) values and duplicates.
- `COUNT(FieldName)` / `COUNT(ALL FieldName)` will return the number on [non-`NULL`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx#null) values including duplicates.
- `COUNT(DISTINCT FieldName)` will return the number of [non-`NULL`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx#null) and non-duplicate values.
...
`HAVING` works nearly the same as [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) and allows you to filter outcomes of your [`GROUP BY`](#group-by).
...
You can leverage all filtering tricks from [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx). In practice, I mostly use it along with aggregate functions like [`COUNT`](#count).

### HAVING vs WHERE

I just wrote that `HAVING` and [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) work similarly, and both filter the outcomes. What is the difference?
...
As they are aggregate functions, we can use them in both the [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) part of the query and for filtering with [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) or [`HAVING`](#having).
...
Learn more about `MIN` and `MAX` in [Numeric Functions documentation](/docs/salesforce/marketing-cloud-engagement/sql/sql-numeric-functions.mdx#min-and-max).

## AVG

`AVG` function allows you to find the average of values within the group.

It ignores `NULL` values for the calculation and we can use it in [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx), [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) or [`HAVING`](#having) part of the query.
...
Learn more about `AVG` in [Numeric Functions documentation](/docs/salesforce/marketing-cloud-engagement/sql/sql-numeric-functions.mdx#avg).

:::note You Should Know

`AVG` will ignore rows with [`NULL`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx), which can skew your results. [`COALESCE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-null-functions.mdx#coalesce) will be your best friend to make sense out of imperfect data.
