...
image: img/og/og-image-sf-mce-sql-join.png
...
There are cases where all you need is to pull (and maybe transform) data [`FROM`](/docs/salesforce/marketing-cloud-engagement/sql/sql-from.mdx) a single Data Extension. But the real magic of SQL is visible when you need to combine information from multiple data points. Here comes the `JOIN` statement.
...
A use case I can think of in the world of Salesforce :product[Marketing Cloud Engagement] could be, for example, a Data Extension that stores product categories and subcategories.
...
Thinking about how much easier it would be with a neat visual cheat sheet? Perfect - I prepared one for you [at the bottom](#mce-sql-join-cheat-sheet) :)
...
You can have the same outcome with more control over comparison logic with a mix of [`JOIN`](#join-types) and [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx).
...
All standard SQL JOINs in a single image with name, example :product[Marketing Cloud Engagement]-focused use case, key part of the SQL query and diagrams visualising the output. Ready to bookmark or download. Full size version linked below the preview.

<Image img={cheatSheet} alt='Salesforce Marketing Cloud SQL JOIN cheat sheet. Each join has a Venn diagram, a use case and the key SQL. Two tables: INNER, LEFT, RIGHT, LEFT EXCLUDING, RIGHT EXCLUDING, FULL and FULL EXCLUDING. Three tables: INNER INNER, INNER LEFT, LEFT INNER, LEFT LEFT, INNER FULL and FULL FULL.' />
