---
id: sql-style-guide
title: MCE SQL Style Guide
sidebar_label: Style Guide
description: Query with style. Readable, bug-free code is a few shifts & spaces away.
image: img/og/og-image-sf-mce-sql-style-guide.png
tags: [Marketing Cloud Engagement, SQL, Style Guide, Best Practice]
...
First things first: this Salesforce :product[Marketing Cloud Engagement] SQL style guide is highly subjective. You may use it as it is, implement only some parts of it, or ignore it altogether. There are only two rules that I believe are a must-have:
...
SQL gives you much freedom regarding the type of letter case you will be using for parts of your query. Let's use this freedom to create queries that are readable and aligned in convention to other :product[MCE] programmatic languages operating on the same data.
...
Treat it as a soft recommendation, as it is highly connected to your organisation's conventions. If other connected systems are using a different style, it might be best to follow it in :product[MCE].
...
The most popular SQL convention for Table and Column names is `snake_case`. However, in :product[MCE], the Table and Column names will share the case between your queries and the User Interface. It is essential to make them as readable and human friendly as possible. Additionally, in many cases, you will also leverage those names in AMPScript and SSJS.

Due to the above, I believe `PascalCase` - especially for Column names - is the best choice for :product[MCE] SQL.
...
1. Each [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) Column in a separate line
2. Each [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) condition in a separate line
3. Each [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx) condition in a separate line
4. Each [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) and [`ON`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#joining-on) relationship in a separate line
...
As you can see, the [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx) and [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) keywords have their separate line to mark respective block starting. For readability reasons, I make an exception if there is only one Column or condition to be used:
...
If you are using `DISTINCT` or `TOP`, put it into the same line as [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx). In such a case, even if you work on a single Column, move it to a separate row for better readability.
...
This rule focuses on writing speed. While I prefer the Vertically Aligned Space approach's aesthetics, I don't see any significant readability gains. On the other hand, writing in this style in :product[MCE] is a pain, as there is no autoformatter supporting it in the Query Studio or Query Activity. We would have to add all those additional spaces manually. It's just not worth it.
...
- Selected Column Names are children of [`SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx)
- [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) keywords are children of the initial Table referenced with [`FROM`](/docs/salesforce/marketing-cloud-engagement/sql/sql-from.mdx)
- Joining relationships ([`ON`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#joining-on)) are children of [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx)
- Case conditions are children of the [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx)
- Where conditions are children of the [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx)
- Sub-queries follow the same rules in respective indentation level.

For simplicity I use 4-spaces indent, as it is both standard and can be added in :product[MCE] Query with a single click of the Tab key.
...
The must-have is spacing around any [operators](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx#basic-operators) (equality or comparison) and after commas.

However, there is another place where spacing is beneficial for readability but might require an exception in some scenarios. Spacing for [aliases](/docs/salesforce/marketing-cloud-engagement/sql/sql-select.mdx#aliasing-with-as) and `THEN` keywords of the [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx) statement.

I try to use it whenever possible, but when there is huge imbalance in length (for example one Column requires multi-function calculation or there is a single complex `WHEN` in [`CASE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx)) equal spacing might make the query harder to read. Always decide which approach works best for your query from readability perspective.
...
3. Having a comma in the beginning makes it easier to assess where a new Column definition starts quickly. It is a vast readability improvement when you are using more complex structures, like [`CASE` in `SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#conditional-values-with-case).
...
For the sake of readability, I indent the first Column by two spaces so that it is in line with the following ones. Similarly, I indent `END` after [`CASE` in `SELECT`](/docs/salesforce/marketing-cloud-engagement/sql/sql-case.mdx#conditional-values-with-case) by two spaces for the same reason.
...
__Use full [Date Parts](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx#date-parts) names.__

Yes, abbreviations let you save few characters, but they kill the readability unless someone is fluent in those shortcuts. For :product[MCE] purposes, the time saved during writing is not worth the time lost when reading. Go with the full version.
...
__Use [`INNER JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) name and reference Column from the new Table first after [`ON`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#joining-on).__

Instead of just [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx), write [`INNER JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#inner-join) - it's longer but explicit and in line with the rest of [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) keywords. It makes it easier to scan queries with [multiple various `JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#multiple-various-joins) statements.

Additionally, when defining [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) relationships after [`ON`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#joining-on), firstly reference the Column from the joined Table. The previous Table should be on the right side. It will improve consistency and reading speed.
...
1. If you are working on only one Table without [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) - don't use [aliases](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#table-name-prefix) at all. They are not needed and would make the query less readable.
2. If you have any [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) - always use [aliases and prefixes](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx#table-name-prefix). SQL doesn't always require it, but it is much easier to read the query when the Columns have an explicit Table mention in the prefix.
...
Looking for more :product[MCE] style? Check out my:

- [SSJS Style Guide](/docs/salesforce/marketing-cloud-engagement/ssjs/ssjs-style-guide.mdx)
- [AMPScript Style Guide](/docs/salesforce/marketing-cloud-engagement/ampscript/ampscript-style-guide.mdx)