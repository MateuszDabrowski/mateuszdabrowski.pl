...
Send Log is a table that captures relevant data about sends happening in the platform. In the case of Marketing Cloud Engagement, it is a unique Data Extension that automatically captures the data (no need for AMPScript or SSJS to populate it).
...
Send Log is populated when MCE prepares the send. That's why it can log [ErrorCode](#send-log-errorcode-values) if the send fails. The Data View is populated later when MCE pushes a correctly constructed email out to the Email Service Providers.
...
Unfortunately, it currently does not support Journey Builder Sends. Learn more and vote for a fix [here](/sites/ideas/salesforce/marketing-cloud-engagement-ideas/#_datasourcename-for-entry-data-extension-in-journey-builder).
