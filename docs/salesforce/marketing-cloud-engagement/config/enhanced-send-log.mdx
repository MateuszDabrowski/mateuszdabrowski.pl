...
description: Create, customise and enhance Marketing Cloud Engagement Send Log for debugging and reporting.
image: img/og/og-image-sf-mce-enhanced-send-log.png
tags: [Marketing Cloud Engagement, SQL, Snippet, Email, Debugging, Analytics]

---

import { LeadText } from '/src/components/LeadText.js';

<LeadText content='Create, customise and enhance Marketing Cloud Engagement Send Log for debugging and reporting.' />

## What is Send Log

Send Log is a table that captures relevant data about sends happening in the platform. In the case of :product[Marketing Cloud Engagement], it is a unique Data Extension that automatically captures the data (no need for AMPScript or SSJS to populate it).

:product[MCE] doesn't have Send Log out of the box, but you can make one yourself. On the first screen of Data Extension creation, select `Create From Template` in the `Creation Method` field. You will see a picklist with multiple templates, including `SendLog` (for Emails), [`SMSSendLog`](docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx#sms-send-log) and [`PushSendLog`](docs/salesforce/marketing-cloud-engagement/config/mobile-push-data-views.mdx#push-send-log). If you do not see any of those templates - you might need to request this as a free [feature on demand](docs/salesforce/marketing-cloud-engagement/config/features-on-demand.mdx).
...
If your send was not delivered, but you can see it in the Send Log without any `ErrorCode`, leverage [Email Sends Debugging Query](/docs/salesforce/marketing-cloud-engagement/sql//snippets/sql-debugging-email-sends.mdx) to learn what happened with the message.

:::

### Send Log vs _Sent System Data View

You might wonder - why use email Send Log when [_Sent Data View](docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx#_sent) captures the same data out of the box? There are two reasons.

#### 1. Captured data is not precisely the same.

It's not only about the [`ErrorCode` field](#send-log-errorcode-values) in the Send Log, and multiple additional fields in the [_Sent Data View](docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx#_sent). It's also about when the data is captured.

Send Log is populated when :product[MCE] prepares the send. That's why it can log [ErrorCode](#send-log-errorcode-values) if the send fails. The Data View is populated later when :product[MCE] pushes a correctly constructed email out to the Email Service Providers.

It means that Send Log is an incredible tool to check why some emails did not go out of :product[MCE].

#### 2. You can customise the Send Log

Data Views are set in stone (unless you purchase a [custom one](docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx#custom-data-views)), but you can add fields to the Send Log to align it better with your business needs. Let's dive into how it works and why you should use it.
...
When :product[MCE] prepares the email for sending, Send Log evaluates all available [personalisation strings](https://help.salesforce.com/s/articleView?language=en_US&type=5&id=sf.mc_es_available_personalization_strings.htm), Sendable Data Extension fields and AMPScript variables. Each value that shares a name with Send Log column is automatically filled. Examples?
...
In most cases, you can dry-test automatic filling by adding `%%[Output(Concat('SendLogColumnName: ', @SendLogColumnName))]%%` to the bottom of the email (where `SendLogColumnName` is equal to the Send Log field you want to test). This AMPScript one-liner should output `SendLogColumnName: ` along with the value that :product[MCE] would add to the Send Log. If it doesn't - there might not be an appropriate variable, or it is not assigned a value.
...
Unfortunately, it currently does not support Journey Builder Sends. Learn more and vote for a fix [here](/docs/salesforce/marketing-cloud-engagement/ideas/#_datasourcename-for-entry-data-extension-in-journey-builder).

:::

## Enhanced Send Log

Custom Send Log is already a fantastic tool, but it's more suited for administrators with its technical field names and lack of post-send data. However, you can easily leverage it to create something perfect for your more business-oriented :product[MCE] users.
...
Create Automation with Query Activity that will take data from your technical [template-based Send Log](#send-log-data-extension-template) and [System Data Views](docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx) to populate your Enhanced Send Log DE with actionable data.
...
In the [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) section, we filter the data to pull only updates for sends from the last month to optimise the query performance.

Schedule that Automation nightly and share the Enhanced Send Log with your users.

## Send Log ErrorCode Values

List of email Send Log errorCode values from the [official :product[MCE] Documentation](https://help.salesforce.com/s/articleView?language=en_US&type=5&id=sf.mc_es_email_send_error_codes.htm):
...
| 15 | Unspecified Error | The subscriber didn't pass validation. Contact your :product[MCE] account representative for more information. |
...
| 25 | Subscriber Blacked Out | The subscriber is ineligible to receive the email. :product[MCE] reschedules the triggered send. |
...
| 40 | Subscriber Deleted | Subscriber or contact is deleted and can't be added to :product[MCE]. |
