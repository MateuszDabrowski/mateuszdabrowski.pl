...
description: Learn how to use and abuse Marketing Cloud Engagement Code Resources, the free-of-charge programmatic powerhouse.
image: img/og/og-image-sf-mce-code-resources.png
...
<LeadText content="Learn how to use and abuse Marketing Cloud Engagement Code Resources, the free-of-charge programmatic powerhouse." />

## What are the Code Resources?

Code Resources in Salesforce :product[Marketing Cloud Engagement] are a solution for hosting JavaScript, CSS, JSON, RSS, Text and XML files within your account.
...
- Unlike Cloud Pages, Code Resources have __no HTML structures appended automatically__ by :product[MCE]. It allows you to have complete control over what is returned by the Code Resource.
- Unlike externally hosted files, you can leverage :product[MCE] programmatic solutions - __SSJS and AMPScript__ - to provide a backend logic to your Code Resources.
...
The real power comes with using Salesforce :product[MCE] programmatic languages - AMPScript and SSJS. Those two allow you to execute logic on the backend and impact the front-facing outcome on the Code Resource. It enables some powerful use cases.

#### Dynamic Code Resources

For example, you cannot host fonts in :product[MCE]. But, you can encode them to base64 and, in that format, add them to your CSS Code Resource.
...
What if you need a different font depending on the market to cover various alphabets? Adding all of them to your CSS is not a good idea from the performance point of view. However, you can leverage the power of :product[MCE] programmatic languages to load only the required font. How? By requesting the specific font using a query parameter:
...
Making your CSS or JS dynamic is just the beginning. Code Resources lets you leverage the :product[MCE] Data Model. With SSJS and AMPScript, you can make Lookups and use other functions to adapt the outcome to a specific subscriber.
...
Once :product[MCE] executes SSJS and AMPScript on the backend, it removes that code from the output. However, you can push some data to the frontend with a personalisation string. If that data is an SSJS object, you can convert it to proper JSON with `Stringify()`.

This trick enables many different use cases that make Code Resources shine:

1. AMPScript and [SSJS debugging](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx) - there are many places where you might want to use :product[MCE] programmatic languages. Most of them either doesn't allow for easy debugging or drive SuperMessages costs. Code Resources are free and allow you to output error descriptions.
2. Deploying, cleaning and retrieving scripts - in many situations doing things through :product[MCE] User Interface is not the best option. Deploying dozens of Data Extensions on multiple business units, cleaning hundreds of archival images or pulling assets JSON for version control. We can cover all those use cases with a script that can do the work in a fraction of the time needed for a manual approach. Then, some things cannot be done through UI at all (like targeting AutoSuppression List with Query Activity). Code Resource again will allow you to write, test and run such solutions. For free.
3. Creating custom mini backends - that use case deserves a separate section ;)

### Code Resource Backends

Code Resources can do so much more than just serve some content. Thanks to the Salesforce :product[MCE] programmatic languages, they can become a dedicated backend for various solutions.

The most popular use case for :product[MCE]'s Web Studio is a Cloud Page with a form (for leads, data capture, preference management or whitepaper download). As far as creating the form is a relatively simple task with the modern HTML, the complex part is capturing the submitted data and saving it into the Data Extension.
...
- You can create only limited (frontend JS-based) validation - if you need some checks against the :product[MCE] data, that needs to happen after submission on the second Cloud Page. And if the validation fails, the user will be redirected back to the form page (in many cases, losing the data submitted in the form).
...
With this approach, any interaction (like form submission) is inserted into an object along with optional metadata, pushed through AJAX asynchronously, loaded in the Code Resource and directed to the appropriate processing unit within a switch block. Once :product[MCE] completes the processing, the response is saved into an object and outputted as a JSON. Then it comes back from Code Resource through AJAX to the frontend with the form.
...
And then there is more! Code Resource is available not only for Cloud Pages - you can use precisely the same approach for any form hosted on your CMS or even within internal systems. As long as your front end can make an AJAX call, you can target your Code Resource and use :product[MCE] power. Nifty solution for providing Triggered Sends for 3rd party forms without the need to implement :product[MCE] API on external services.

#### More ideas

Using Code Resource as a form backend is a practical and frequent use case, but it is not the only one. It's worth considering whenever you need a more complex interaction with the :product[MCE] features or data model.
...
The most obvious form of security is the obscurity of the solution. While standard static Code Resources are fully transparent, any programmatic part written in SSJS/AMPScript won't be accessible from outside :product[MCE].
...
If the request comes from :product[MCE] controlled resource (Cloud Page, Email, another Code Resource), you can also leverage Data Extension for even more strict security. Insert some data into the Data Extension in the first step of the process and validate it with Lookup against the payload content within the Code Resource.

### SSO

Finally, if the Code Resource is part of your internal solution created only for :product[MCE] users, you may leverage full-blown SSO to make it as secure as the whole platform. Read more on that approach in the [Cloud Page Apps article](/docs/salesforce/marketing-cloud-engagement/ssjs/snippets/sfmc-cloud-page-apps.mdx#a-good-application-is-a-secure-application).
...
3. Create a [custom Chrome search engine that can find and directly open :product[MCE] Data Extensions right from your URL bar](https://sfmr.dev/ideas/sfmc/tools/data-extension-search-engine) by [Michał Rzepka](https://www.linkedin.com/in/michalrzepka90/).
...
5. Build [Cloud Page Apps](/docs/salesforce/marketing-cloud-engagement/ssjs/snippets/sfmc-cloud-page-apps.mdx) to make your and your team lives easier.
...
1. Webinar recording on [Architecting Web Solutions in :product[MCE]](/docs/salesforce/marketing-cloud-engagement/config/webinars/mce-architecting-web-solutions.mdx).
2. Webinar recording on [Cloud Page Apps](/docs/salesforce/marketing-cloud-engagement/config/webinars/mce-cloud-page-apps.mdx).