...
5. The message will get Disposition `0` (Queued) when it is triggered and stay in this state until it is processed for delivery and left Marketing Cloud (then it will get updated to `1` (Sent)) or if it was not sent at all or is still in queue after the expiration window (then it will get updated to `2` (NotSent)).
