...
description: Dive deep into System Data Views - the backend Marketing Cloud Engagement data. Actionable pearls await.
image: img/og/og-image-sf-mce-system-data-views.png
tags: [Marketing Cloud Engagement, SQL, Analytics, Data Extensions, Email, Journey]
...
<LeadText content='Dive deep into System Data Views - the backend Marketing Cloud Engagement data. Actionable pearls await.' />
...
Be careful, however, as some of the System Data Views can get absurdly big quick, and making copies within your :product[MCE] Instance may lead to performance issues. In such cases, a much better idea might be to export that data on a scheduled basis to an external Data Warehouse.

Another important point related to Data Views is always knowing which Data View you genuinely want to use. For example, in most cases, you will be interested in parent-level [`_Subscribers`](#_subscribers) Data View, which means you will need to use the [`Ent.`](/docs/salesforce/marketing-cloud-engagement/sql/sql-from.mdx#enterprise-data-extension) prefix in your query.

:::note You Should Know

Below you can find selected Salesforce :product[Marketing Cloud Engagement] Data Views focused on Subscribers, Emails, Journeys and Automations. You can find the SMS Channel Data Views in [Mobile Connect Data Views article](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx) and the push channel ones in [MobilePush Data Views article](/docs/salesforce/marketing-cloud-engagement/config/mobile-push-data-views.mdx).

Be sure to check the excellent [Dataviews.io](https://dataviews.io) - an interactive system tables relationship diagram created by [Zuzanna Jarczyńska](https://sfmarketing.cloud/author/zjarczynska/). It will for sure help you with multi-view [`JOIN`s](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx).
...
<details>
<summary>`Status` - 4 values</summary>
...
<details>
<summary>`SubscriberType` - 2 values</summary>
...
2. If you need high-level data on your subscribers, this Data View might be enough. If you want to learn more (for example, why a subscriber bounced, where did he unsubscribe), you will need to [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) other Data Views.
...
3. Despite the `Email Address` field is visible in the Salesforce :product[MCE] UI when you go to Profile Attributes, it is not part of the `_EnterpriseAttribute` Data View. To get the Email Address of your Subscriber, query the `_Subscribers` Data View.
...
PickupTime | The time the :product[MCE] application started the job | Date | X
...
<details>
<summary>`JobType` - 2 values</summary>
...
<details>
<summary>`JobStatus` - 2 values</summary>
...
<details>
<summary>`Category` - 4 values</summary>
...
<details>
<summary>`CharacterSet` - 1 value</summary>
...
<details>
<summary>`SendType` - 1 value</summary>
...
<details>
<summary>`SendClassificationType` - 2 values</summary>
...
Stores infomation about your Email and SMS final delivery status for transactional sends.
    ...
    - It's __reconcilable__, because using this feature allows you to reconcile (match) your transactional sends in :product[MCE] with the source system (e.g. e-commerce platform) based on the MessageKey and limit duplicate sends in case there is retriggering issue.
    - It's __disposition view__, because it shows the final disposition (status) of your transactional messages - whether they were delivered, are still in queue or were not sent at all. Remember that delivered doesn't mean delivered to the main inbox. Spam placement technically is also successful delivery.
...
<details>
<summary>`Channel` - 2 values</summary>
...
<details>
<summary>`Disposition` - 3 values</summary>
...
1. To leverage this Data View you first need to send __transactional__ message using a Sendable Reconcilable Data Extension:
...
3. When triggering transactional message provide a MessageKey (up to 100 characters) along with other data to allow for matching with your transactional source system (e.g. e-commerce platform). Thanks to the reconciliation feature, even if source system triggers multiple messages with the same MessageKey, :product[MCE] will only process the first one protecting recipients from duplicate messaging. If you do not provide a MessageKey, :product[MCE] will generate the MessageKey for you.
4. Extend [Email and SMS Send Logs](/docs/salesforce/marketing-cloud-engagement/config/enhanced-send-log.mdx) with the MessageKey to simplify the process of matching transactional sends delivery status with various events from multiple Data Views like [`_Open`](#_open), [`_Click`](#_click), [`_Bounce`](#_bounce).
5. The message will get Disposition `0` (Queued) when it is triggered and stay in this state until it is processed for delivery and left :product[MCE] (then it will get updated to `1` (Sent)) or if it was not sent at all or is still in queue after the expiration window (then it will get updated to `2` (NotSent)).
...
1. Use `JobID`, `ListID`, `BatchID` and `SubscriberId` or `SubscriberKey` to match various events from multiple Data Views like [`_Sent`](#_sent), [`_Click`](#_click), [`_Bounce`](#_bounce) for a single engagement with a particular subscriber. Example: [Query for Debugging Email Sends](/docs/salesforce/marketing-cloud-engagement/sql/snippets/sql-debugging-email-sends.mdx#solution).
...
There is **no official engagement feedback loop** that informs :product[MCE] (or any other Marketing Automation Platform) that your recipient opened the email.

All Marketing Automation Platforms put an **invisible 1px wide image in the email** body that is unique for every email you send. When someone opens an email, its images are downloaded from the server. :product[MCE] treats the download of this 1px wide image as proof that the recipient opened the email. And in most cases, it is correct. But...
...
1. Use `JobID`, `ListID`, `BatchID` and `SubscriberId` or `SubscriberKey` to match various events from multiple Data Views like [`_Sent`](#_sent), [`_Open`](#_open), [`_Bounce`](#_bounce) for a single engagement with a particular subscriber. Example: [Query for Debugging Email Sends](/docs/salesforce/marketing-cloud-engagement/sql/snippets/sql-debugging-email-sends.mdx#solution).
...
Stores information about your Email Bounces. Best place in :product[MCE] to understand why your messages are not getting delivered.
...
TriggererSendDefinitionObjectID | The object ID for the triggered send definition | Text | X
TriggeredSendCustomerKey | The customer key for the triggered send | Text | X
IsFalseBounce | Indicates a false bounce if [:product[MCE] received delivery receipt after receiving bounce](https://help.salesforce.com/s/articleView?id=000390396&type=1). | Boolean | X

</TabItem>
<TabItem value='values'>

<details>
<summary>`BounceCategoryID` - 5 values</summary>
...
<details>
<summary>`BounceCategory` - 5 values</summary>
...
<details>
<summary>`BounceSubcategoryID` - 15 values</summary>
...
<details>
<summary>`BounceSubcategory` - 14 values</summary>
...
<details>
<summary>`BounceTypeId` - 2 values</summary>
...
<details>
<summary>`BounceType` - 2 values</summary>
...
1. Use `JobID`, `ListID`, `BatchID` and `SubscriberId` or `SubscriberKey` to match various events from multiple Data Views like [`_Sent`](#_sent), [`_Open`](#_open), [`_Click`](#_click) for a single engagement with a particular subscriber. Example: [Query for Debugging Email Sends](/docs/salesforce/marketing-cloud-engagement/sql/snippets/sql-debugging-email-sends.mdx#solution).
...
6. If you get errors when trying to select `IsFalseBounce` - it might be missing on the :product[MCE] backend. Create a Support Ticket to resolve it.

:::note You Should Know

`SMTPBounceReason` field is `nvarchar(max)` SQL type, and due to that might be longer than the 4000 characters limit for the Data Extension string length in Salesforce :product[MCE]. If you want to save this data outside of Data View, be sure to use [`LEFT(SMTPBounceReason, 4000)`](/docs/salesforce/marketing-cloud-engagement/sql/sql-string-functions.mdx#left--right).
...
Subscription management in Salesforce :product[MCE] is complex. Very complex.
...
3. Master unsubscribe = no more sends from your :product[MCE].
4. Global unsubscribe = no more sends from :product[MCE]. Any, globally.
...
Spam Complaint and List-Unsubscribe Header are pretty particular unsubscription use cases. They are added to [`_Unsubscribe`](#_unsubscribe) and [`_BusinessUnitUnsubscribe`](#_businessunitunsubscribes) as unsubscriptions from a list used in send (for example, ID of Publication List selected for the send). In reality, :product[MCE] will perform automatic unsubscribe from the All Subscriber list (depending on Email Studio settings - Child BU's or Enterprise one), not the list mentioned in `ListID`.
...
This Data View stores data only for six-months, so retention-less `_Subscribers` Data View might show more unsubscribed records if you are not cleaning them from :product[MCE] regularly.
...
<details>
<summary>`UnsubReason` - 6 values</summary>

- Spam Complaint
- Unsubscribed from list details on subscriber properties dialog.
- Unsubscribed by Salesforce :product[MCE] RMM service based on subscriber Leave/Reply email.
...
3. `UnsubDateUTC`, as name suggests, is in UTC instead of the MCE-standard UTC-6. Remember to use [`DATEADD`](/docs/salesforce/marketing-cloud-engagement/sql/sql-date-functions.mdx#dateadd) to normalize it before comparing to other system dates.
...
`UnsubDateUTC` not only uses a different timezone from the rest of the system dates. It differs by a few milliseconds from [`_Unsubscribe`](#_unsubscribe) `EventDate`. When querying (:product[MCE] doesn't show seconds for date fields), you won't see it, but the comparison is using the full datetime value.
...
- `_Subscribers` has data about All Subscribers list only. `_ListSubscribers` shows status for all Lists available in Salesforce :product[MCE].
...
<details>
<summary>`AddMethod` - 5 values</summary>
...
<details>
<summary>`ListType` - 6 values</summary>
...
Learn more about Mobile Connect [SMS Data Views here](docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx), and about the undocumented [MobilePush Data Views](docs/salesforce/marketing-cloud-engagement/config/mobile-push-data-views.mdx) that hold push devices and tags.
...
<details>
<summary>`JourneyStatus` - 5 values</summary>
...
2. You can also connect it to SMS channel using [_SMSMessageTracking](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views/#_smsmessagetracking) Data View by making a [`JOIN`](/docs/salesforce/marketing-cloud-engagement/sql/sql-join.mdx) `ON journey.VersionID = smsTracking.JBDefinitionID`.
3. Use `JourneyStatus` to check what is the current Journey Status. It is fantastic for both the [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) statement and a data point for engagement analysis. Think about scheduled Automation that finds all emails performing below expectations and checks which are in the Active Journeys. For all found, it sends you a notification to check and improve. For awful results, you can even use this data in Script Activity to automatically stop the Journey with the :product[MCE] REST API.
...
<details>
<summary>`ActivityType` - 21 values</summary>
...
<details>
<summary>`AutomationType` - 3 values</summary>
...
<details>
<summary>`AutomationInstanceStatus` - 6 values</summary>
...
<details>
<summary>`ActivityType` - 28 values</summary>
...
<details>
<summary>`ActivityInstanceStatus` - 5 values</summary>
