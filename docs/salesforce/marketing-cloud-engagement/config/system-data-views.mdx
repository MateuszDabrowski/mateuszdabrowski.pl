...
Below you can find selected Salesforce Marketing Cloud Data Views focused on Subscribers, Emails, Journeys and Automations. You can find the SMS Channel Data Views in [Mobile Connect Data Views article](/docs/salesforce/marketing-cloud-engagement/config/mobile-connect-data-views.mdx).
...
2. When triggering transactional message provide a MessageKey (up to 100 characters) along with other data to allow for matching with your transactional source system (e.g. e-commerce platform). Thanks to the reconciliation feature, even if source system triggers multiple messages with the same MessageKey, MCE will only process the first one protecting recipients from duplicate messaging. If you do not provide a MessageKey, MCE will generate the MessageKey for you.
3. Extend [Email and SMS Send Logs](/docs/salesforce/marketing-cloud-engagement/config/enhanced-send-log.mdx) with the MessageKey to simplify the process of matching transactional sends delivery status with various events from multiple Data Views like [`_Open`](#_open), [`_Click`](#_click), [`_Bounce`](#_bounce).
4. The message will get Disposition `0` (Queued) when it is triggered and stay in this state until it is processed for delivery and left Marketing Cloud (then it will get updated to `1` (Sent)) or if it was not sent at all or is still in queue after the expiration window (then it will get updated to `2` (NotSent)).
4. `ErrorCodeID`, `ErrorName`, and `ErrorDescription` will be populated only if `Disposition` is equal to `2` (NotSent). If your transactional messages are not being delivered, check these fields to understand why.
5. `StartTime` field allows you to track trigger time for transactional messages. For Data Extension sends, it will be the time when the job started. For API sends, it will be the time when the API call was received. It's fun data point to compare against [`_Sent`](#_sent) `EventDate` to check the time it took for the message to be sent after the trigger.
6. `SubscriberID` field might be null if there are issues with the subscriber record.
7. Data retention for this Data View is only 7 days.
