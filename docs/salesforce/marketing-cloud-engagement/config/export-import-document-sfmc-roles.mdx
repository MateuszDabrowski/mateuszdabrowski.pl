...
image: img/og/og-image-sf-mce-export-import-document-roles.png
tags: [JavaScript, Marketing Cloud Engagement, Setup]
...
While you get a set of standard user Roles in :product[Marketing Cloud Engagement], those are rarely enough. Instead, they are the base on which you can create custom-tailored solutions that align with your business architecture.

:::note You Should Know

The best way to work with Roles and Permissions in :product[Marketing Cloud Engagement] is to leverage standard roles and build on top of them with custom permissions/roles.
...
:product[Marketing Cloud Engagement] doesn't make it easy. There is no option to clone existing Roles to serve as a starting point for a new custom one. There is no option to export and import them between :product[MCE] accounts. There is no option to create good documentation on them (unless you have a kink for super long print screens).
...
:product[Marketing Cloud Engagement] goes with the most restrictive resulting permission possible:
...
To start using those :product[MCE] Role-related, [DOM](/docs/js/js-dom.mdx)-fueled solutions:

1. Go to :product[MCE] Setup » Users » Roles.
...
To use it, go to the Role in :product[Marketing Cloud Engagement] setup and:
...
Once you have your Role export saved, go to the new Role. It can be either an existing Role you want to overwrite or a new Role (in the same or different :product[MCE] account).
...
As the default System Roles permissions in :product[Marketing Cloud Engagement] can be edited freely and [currently there is no easy way to revert those changes](/docs/salesforce/marketing-cloud-engagement/ideas/#restore-default-permissions-for-mce-standard-system-roles), I created a backup repository for you with all out-of-the-box configurations.
