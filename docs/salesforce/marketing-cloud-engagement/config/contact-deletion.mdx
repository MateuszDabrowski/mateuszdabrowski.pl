---
id: contact-deletion
title: MCE Contact Deletion
sidebar_label: Contact Deletion Process
description: Marketing Automation is as powerful as the database quality behind it. To make it good you must first remove the bad. Enable, run and monitor Contact Deletion in Marketing Cloud Engagement, and choose which contacts to delete.
image: img/og/og-image-sf-mce-contact-deletion.png
tags: [Marketing Cloud Engagement, Snippet, SSJS, SQL, Data Extensions, Data Management]
...
1. [Learn everything about MCE Contact Deletion Process](#contact-deletion-in-mce)
...
:product[Marketing Cloud Engagement] makes Contact Deletion not only easy but also powerful. The out-of-the-box solution does most of the heavy lifting, leaving us the decision of who and when should be deleted from our platform.
...
Once we know why we should delete, it's time to see what exactly is the built-in :product[MCE] Contact Deletion process doing behind the scenes:
...
There is also a second reason for the suppression period. Depending on the parallel processes run in your :product[MCE], the number of contacts requested for deletion, as well as the volume of data in the system, :product[MCE] Contact Deletion can be a very time-consuming process. Suppression makes sure that even if it takes longer than you would like, the Contact will not get any unwanted communication.

:::note You Should Know

Few places are not targeted by the out-of-the-box :product[MCE] Contact Deletion process:
...
- Triggered Send Lists » _Can be removed using [API method](#mce-api-contact-deletion)_
...
To use the Contact Deletion Process in :product[Marketing Cloud Engagement], you must enable it first. To do this, you need to have a Marketing Cloud Administrator role.
...
For the most popular :product[MCE] tenant type - Enterprise 2.0 - the deletion must happen at the parent level and automatically apply across all Business Units.
...
Manual Contact Deletion in Email Studio will remove the contact only from Email Studio. To trigger the Contact Deletion Process, you must leverage Contact Builder or [API method](#mce-api-contact-deletion).
...
This method will be better if you want to remove a significant number of contacts once but don't want the hassle of the most potent [API method](#mce-api-contact-deletion).
...
7. Decide whether the Data Extension itself should stay in the system after :product[MCE] deletes the contacts.
...
Previous methods are nice, but they require manual triggering in :product[MCE] each time you run the Contact Deletion Process. There are, however, many use cases where you might want to trigger it automatically - for example, from a different system responsible for managing privacy.

Thankfully, :product[MCE] offers a solution for that as well with REST API. To implement it:
...
With the [API Contact Deletion](#mce-api-contact-deletion) we are not limited to external triggers. We can also leverage AMPScript or SSJS to trigger it directly from :product[MCE].

The most popular use case is a Contact Deletion Automation scheduled to clean up the database regularly. It's a great idea to keep your :product[MCE] instance clean without much of a hassle.
...
Below you can find ready-to-use SSJS snippet for Script Activity. Change `clientSecret`, `clientID` and `clientBase` variables to data you saved during [Installed Package configuration](#mce-api-contact-deletion). If needed, change also the `contactDeletionDEKey`, `errorDE` and `automationName` variables to reflect your configuration.

To learn more about `errorDE` check my [debugging guide](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#error-logging). You will find there also details about [helper functions](/docs/salesforce/marketing-cloud-engagement/ssjs/debugging-ssjs.mdx#error-handler-function) from the script.
...
Remember to test those recommendations and queries (for example, using [Query Studio](docs/salesforce/marketing-cloud-engagement/config/appexchange-solutions.mdx#query-studio)) before implementing them in your Contact Deletion Process, as even a small mistake might lead to irreversible data loss.

<Button link='/docs/salesforce/marketing-cloud-engagement/sql/snippets/sql-debugging-all-contacts'
 text='Find Problematic Contacts'
 newTab='true'
/>

### Contacts deleted in Salesforce CRM

If you use Marketing Cloud Connect to integrate your Salesforce CRM with :product[MCE], you should think about synchronizing deletions between those two systems. It does not happen automatically. By default, contacts deleted in SF CRM are removed from Synchronized Data Extensions and stay in other :product[MCE] tables.
...
If you are also using other channels, you can include their [System Data Views](docs/salesforce/marketing-cloud-engagement/config/system-data-views.mdx) to the query.
...
First, to remove unused Leads and Users, you must first stop them from synchronizing via Marketing Cloud Connect. Otherwise, they will come back to :product[MCE] at the next scheduled synchronization. The best way to do it is to create a boolean field on those objects (for example, `ShouldSyncToSFMC`) that would be `true` only for records you need in your system. Depending on your use case, it might be neither (so just set `false` as the default value), or it might be complicated (the calculated field will be your best friend). With such a boolean, you will be able to select it as a filter for your Marketing Cloud Connect object synchronization.

Second is the problem of finding those records in :product[MCE]. The easy way is to create a copy of Synchronized Data Extensions before filtering them. However, this will not always be possible, and it won't capture Leads and Users that were removed in the past from CRM.

If you have Mobile Studio in your :product[MCE] licence, you have a nice workaround:
...
You might wonder what is happening within the [`WHERE`](/docs/salesforce/marketing-cloud-engagement/sql/sql-where.mdx) filter above - this is magic that is possible thanks to Salesforce ID structure. The ID is not entirely random. Instead, it has a specific format.
...
Deleting a Contact that still has a MobilePush device is not a simple cleanup. :product[MCE] treats it as a compliance deletion, and the SDK blocks any future push registration from that device. The only fix is the user reinstalling the app - Salesforce Support can't restore it for you.
