...
title: MCE Behavioral Triggers
sidebar_label: Behavioral Triggers
description: Win-back your e-commerce customers with MCE Behavioral Triggers.
image: img/og/og-image-sf-mce-behavioral-triggers.png
tags: [Marketing Cloud Engagement, Setup, Email, Journey, Personalisation]
...
<LeadText content="Win-back your e-commerce customers with MCE Behavioral Triggers." />

## MCE Behavioral Triggers

Behavioral Triggers are a solution for smoothly moving abandoned engagement data from your e-commerce to Salesforce :product[Marketing Cloud Engagement]. They enable powerful win-back communication you can create with minimal technical knowledge in express time.
...
In each case, what Behavioral Triggers does is push relevant data (items left in the cart, browsed without conversion or added to wishlist) to the Einstein Backend and - after configured time - to :product[MCE] Data Extension.
...
5. Finally, **configure Behavioral Triggers** to bring relevant data to :product[MCE] Data Extensions.
...
2. Customer clicks on a product page - you trigger the [Page View data layer](#track-page-view) with a link to that product (Abandoned Browse scenario).
3. Customer finds an exciting product and adds it to the wishlist - you trigger the [Wishlist data layer](#track-wishlist) (Abandoned Wishlist scenario).
4. A customer decides on one of the products and adds it to the cart - you trigger the [Cart data layer](#track-cart) (Abandoned Cart scenario).
5. Customer goes through the purchase process and buys the product - you trigger the [Cart data layer](#track-cart) with explicit parameter (This blocks Abandoned Cart from launching communication).
6. Each of the above triggers is sent to Einstein Backend (IgoDigital) that stores the information.
7. :product[Marketing Cloud Engagement] Behavioral Triggers query the Einstein Backend for any events that occurred some time ago - you can configure it in the range from 15 minutes to 3 hours (in 15 minutes increments).
...
The next step allows you to configure Custom Profile Attributes. It is beneficial for general Einstein purposes, but it is also crucial for Behavioral Triggers if you want to personalise your win-back email for unknown (to :product[MCE]) customers.
...
When you confirm the setup, :product[MCE] creates the system Data Extension to store Behavioral Trigger data (one per scenario).
...
Once you have it, you may start building the data layers that will move the information from your e-commerce to :product[MCE]. For simplicity, I won't be writing the above code in the snippets below.

### Set Org ID

The first data layer you need to push contains, again, the MID of the Business Unit. Just as with the [Collect Code Script](#collect-code-script), be sure to change the `MID` to your :product[MCE] BU's actual ID.
...
1. Subscriber Key (if you plan on sending only to contacts known to :product[MCE] or have an option to generate Subscriber straight from your e-commerce) or
2. Email Address (recommended if you want to push win-back emails to unknown users in your :product[MCE] and you do not have an option to create them properly.)

:::note You Should Know

If you decide to push an Email Address to communicate with people not yet in :product[MCE], be sure to make them Subscribers. You can easily do it using the classic Export-Transfer-Import Automation on the data stored in the Behavioral Triggers Data Extension. Leverage SQL Activity to copy the needed data to another Data Extension using this query:
...
With this data layer, you can be sure that the Catalog stored in :product[MCE] is always up-to-date. It is crucial, as everything in the Behavioral Trigger Content Block uses the Catalog data.
...
<ButtonRow
 linkLeft='https://joernberkefeld.github.io/SFMC-Cookbook/einstein/recommendation/'
 textLeft="Jörn's Cookbook"
 newTabLeft='true'
 linkRight='https://resources.docs.salesforce.com/248/latest/en-us/sfdc/pdf/mc_pb_implementation_guide.pdf'
 textRight="Salesforce's Guide"
 newTabRight='true'
/>
...
The current version of Behavioral Trigger Content Block is not rendering correctly on Yahoo, AOL and Windows Outlook. Learn more [here](/docs/salesforce/marketing-cloud-engagement/ideas/#behavioral-trigger-content-block-rendering-on-yahoo-aol-windows-outlook).
...
Once all of the above is validated, the correct data should be available to Salesforce :product[MCE] Einstein and Behavioral Triggers.

### Debugging Data Flow

Now it's time to check whether the data is coming through to your Salesforce :product[MCE].
