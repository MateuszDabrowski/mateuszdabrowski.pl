---
id: ideas
title: Marketing Cloud Engagement Ideas
description: Here you can find all my Salesforce Marketing Cloud Engagement IdeaExchange contributions. Let's make the change!
image: img/og/og-image-sf-mce-ideas.png
...
<LeadText content="Here you can find all my Salesforce Marketing Cloud Engagement IdeaExchange contributions. Let's make the change!" />
...
Salesforce :product[Marketing Cloud Engagement] Journey Builder offers many out-of-the-box Activities and the possibility to create or download custom ones from :product[AppExchange]{as-written}. However, there is a space between those solutions that Salesforce can cover to empower :product[MCE] users. An out-of-the-box Script Activity, similar to the one available in Automation Studio.

Flexible enough to offer countless possibilities to :product[MCE] power users without the complexity of creating a custom solution with the whole hosting, scalability and security considerations.
...
2. __Solution Deluxe__: Extension of the previous idea to define various paths based on a predefined value in the script. Think about custom decision splits using out-of-MCE data that allow you to change the Journey path based on real-time information from external systems.
...
Salesforce :product[Marketing Cloud Engagement] offers many different Wait Activities, but one basic is missing - Wait Until Time Range.
...
The [Behavioral Triggers Content Block](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/#behavioral-trigger-content-block) for Emails is an excellent tool for easy drag-and-drop creation of Abandoned Engagement communication. But it is missing one powerful feature — the ability to personalise the email with customer data.

It is already available in the form of:

- Einstein User Attributes that we can pass through the `setUserInfo` data layer.
- The [request](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/#5-building-the-api-query) used in the back end of the current Behavioral Triggers Content Block; by adding "&user_attributes=attributeName" to the endpoint.
- [Code](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/#6-response-parsing) of the existing Behavioral Triggers Content Block responsible for creating AMPScript variables based on the Einstein User Attributes data.
...
2. __Solution Deluxe__: On top of the above solution, :product[MCE] should consider the Parameter Management to allow for a mix of global UTM's from Web Analytics Connector and local from the new text box in Behavioral Trigger Content Block configuration.
...
The [Behavioral Triggers](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/) are a terrific solution for Abandoned Engagement. The [Abandoned Cart](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/#track-cart) scenario already has multiple great features that make it easy to implement.
...
It is already available in the form of an optional `price` attribute of [`trackCart` Collect.js Data Layer](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/#track-cart) that contains individual price that does not impact the Product Catalog.  It would be great to have the possibility to add a few more, available in [`trackCart` Collect.js](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/#track-cart) and the [response](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/#api-responses) from Einstein Backend (IgoDigital).
...
And for Behavioral Trigger Content block that parent is a `<td>` parameter within header [`markupFragment`](/docs/salesforce/marketing-cloud-engagement/config/behavioral-triggers/#3-settings--markup-fragments). Unfortunately, it has `font-size: 0`:
...
Content Builder is great for creating and neatly organising assets in :product[MCE], but it has one huge issue. Folder access management.
...
When settings permissions for an :product[MCE] User or Role, there are options that control the rights to Shared Folders of Content Builder. As with Shared Data Extensions Folder permissions, you can decide what the user or role can do with Shared Content Folders.
...
As :product[MCE] applies the permissions to read and edit content to both local and shared assets, every person with edit rights can change assets in a Shared Content Folder - event if they do not have permission to that folder.
...
While Salesforce updates the Standard Roles (Email Studio and :product[Marketing Cloud Engagement] ones) during Releases, users can also modify them within the :product[MCE] Setup freely (permissions of those System Roles are not locked).
...
- Checking it against another, unchanged :product[MCE] (which is not an option for most) or
...
Salesforce :product[Marketing Cloud Engagement]'s Business Unit structure is essential for organising marketing operations across different brands, regions, or teams. However, its effectiveness is severely undermined by the current licence model, where key limits are shared across all Business Units. This creates a critical bottleneck, especially for Yearly Automation Executions, where a single active BU can consume the entire enterprise-wide allowance, crippling all other BUs.
...
<Button
    link='https://ideas.salesforce.com/s/idea/a0BHp000016LlDJMA0/increase-licence-limits-with-additional-business-units'
    text='Vote on IdeaExchange'
    newTab='true'
/>