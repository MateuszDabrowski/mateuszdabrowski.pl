...
There are multiple areas to apply this approach when developing in Marketing Cloud.
...
Yet another area where you can make your SFMC development more explicit is comments. Yes, good naming convention and purposeful choice of functions are already great for creating understandable code, but you can make it so much better with adding a bit of context.
...
It might sound contradictory to what the [Simple is better than complex](#3-simple-is-better-than-complex) recommendation just covered, but only on the surface. There is a limit to what can be simplified before it starts getting complicated. In such case, complex is the lesser evil.
...
This approach is natural during development, but it quickly leads to a complicated outcome. And [complicated solution should be avoided](#4-complex-is-better-than-complicated). There are two ways to do it:
